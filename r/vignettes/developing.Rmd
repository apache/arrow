...
If you're looking to contribute to __arrow__, this vignette can help you set up a development environment that will enable you to write code and run tests locally. It outlines:
...
zip file before installing the `arrow` R package. On Linux, you'll need to create a `libarrow` directory inside the R package directory and unzip that file into it. Version numbers in that repository correspond to dates.
...
There are five major steps to the process — the first three are relevant to all Arrow developers, and the last one is specific to the R bindings:
...
## Step 2 - Configure the Arrow build {.tabset}
