...
Building on Windows requires RTools 4.0 or newer. Open RTools Bash and install CMake with:
...
You will need to add mingw64 binaries and R to your path. For a user-level installation, R will be at something like `~/Documents/R/R-4.1.2/bin`.

# TODO(wjones127): Add R path of global install
# TODO(wjones127): What about path for mingw32?

```{bash}
export PATH=/c/rtools40/mingw64/bin:~/Documents/R/R-4.1.2/bin:$PATH
```
...
pacman --sync --refresh --noconfirm \
  ...
  mingw-w64-${MSYSTEM_CARCH}-aws-sdk-cpp \
  mingw-w64-${MSYSTEM_CARCH}-libutf8proc
```
