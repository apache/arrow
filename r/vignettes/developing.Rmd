...
For all setups, now change to be inside `arrow/cpp/build`:
...
Change to be inside `arrow/cpp/build`:
...
* `configure` and `configure.win` These scripts are triggered during `R CMD INSTALL .` on non-windows and windows platforms respectively. They handle finding the Arrow library, setting up the build variables necesary, and writing the package Makevars file that is used to compile the C++ code in the R package.
