...
Now, we can begin writing a stream containing some number of these batches. For this we use `RecordBatchStreamWriter`:
...
When creating the stream writer, we pass the schema, since the schema (column names and types) must be the same 
for all of the batches sent in this particular stream. Now we can do:
...
Now `buf` contains the complete stream as an in-memory byte buffer. We can read such a stream with `RecordBatchStreamReader`: 

```{r}
reader <- RecordBatchStreamReader(buf)
# TODO change to reader$schema after #3104 is merged
reader$schema
...
```{r}
all(purrr::map_lgl(batches, ~ .x == batch))
```
...
The difference between `RecordBatchFileReader` and `RecordBatchStreamReader` is that the input source
must have a seek method for random access.
...
RecordBatchFileReader and RecordBatchStreamReader may be used by 
 
  - `read_table()` to create an `arrow::Table` from the record batches
  - `read_arrow()` to create a data frame 
...
Feather is a lightweight file-format for data frames that uses the Arrow memory layout for data representation on disk. 
It was created early in the Arrow project as a [proof of concept](https://github.com/wesm/feather) 
for fast, language-agnostic data frame storage for Python (pandas) and R.
...
The `read_feather()` and `write_feather()` functions read and write files in the feather format: 
