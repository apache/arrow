...
C++ is an object-oriented language, so the core logic of the Arrow library is encapsulated in classes and methods. In the R package, these classes are implemented using the `R6` object-oriented programming system, and most of them are exported from the namespace.
