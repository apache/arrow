...
to build the Arrow libraries from source with optional features such as S3 
support enabled. This will increase the build time but provides many useful features.
...
Some features are optional when you build Arrow from source. With the exception 
of `ARROW_S3`, these are all `ON` by default in the bundled C++ build, but you 
can set them to `OFF` to disable them.
