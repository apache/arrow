...
If you have `arrow` installed and want to upgrade to a different version,
install a development build, or try to reinstall and fix issues with Linux
C++ binaries, you can call `install_arrow()`. `install_arrow()` will install
from CRAN, while `install_arrow(nightly = TRUE)` will give you a development build.
...
Binaries will only be retrieved if you have set the environment variable
`LIBARROW_BINARY` or `NOT_CRAN`.
(Note that in the 0.16.0.1 CRAN release, `LIBARROW_BINARY_DISTRO` is the variable you need to set.)
...
If you experience compilation errors as described below and want to install
`arrow` without the C++ libraries in order to access the `install_arrow()`
function, set the environment variables `ARROW_USE_PKG_CONFIG=FALSE`,
`LIBARROW_DOWNLOAD=FALSE`, and `LIBARROW_BUILD=FALSE`, then reinstall the package.
...
* `LIBARROW_BINARY`: If set to `FALSE` (case insensitive),
