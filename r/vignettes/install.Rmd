...
* `ARROW_S3`: If set to `true` S3 support will be built (as long as the 
  dependencies are met, if they are not met, this may be automatically turned off).
* `ARROW_JEMALLOC`: If set to `true` jemalloc support will be included.
* `ARROW_PARQUET`: If set to `true` parquet support will be included.
* `ARROW_DATASET`: If set to `true` dataset support will be included.
* `ARROW_WITH_RE2`: If set to `true` RE2 regular expression library support will be included.
* `ARROW_WITH_UTF8PROC`: If set to `true` UTF8Proc string library support will be included.
