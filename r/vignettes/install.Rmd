...
A workaround would be to set the environment variable `ARROW_BINARY_DISTRO=FALSE`
...
A workaround would be to set the environment variable `ARROW_BINARY_DISTRO`
to a `distribution-version` that exists in the Ursa Labs repository.
<!-- give example, once one exists -->
Setting `ARROW_BINARY_DISTRO` is also an option when there's not an exact match
...
The only system dependency the library needs beyond what R requires is `cmake`.
Provided that `cmake` is found, if the C++ library fails to compile,
please [report an issue](https://issues.apache.org/jira/projects/ARROW/issues).
