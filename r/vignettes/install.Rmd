...
* `ARROW_USE_PKGCONFIG`: If set to `FALSE` (case sensitive), the configure script
  won't look for Arrow libraries on your system and instead will look to download/build them.
* `LIBARROW_DOWNLOAD`
* `LIBARROW_BUILD`
* `LIBARROW_BINARY_DISTRO`: When unset, the build script will try to discover
  from the OS what distribution-version we're on,
  but you can override this by setting the env var LIBARROW_BINARY_DISTRO to `FALSE` (not case-sensitive),
  which tells the script not to download a binary,
  or some other string, presumably a related "distro-version" that has binaries built that work for your OS
