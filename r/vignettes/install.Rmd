...
In order for `install.packages()` to download C++ dependencies for `arrow` on
Linux, first set the environment variable `LIBARROW_DOWNLOAD=true`. If you have
set the `NOT_CRAN` environment variable, that will be used as a default value
for `LIBARROW_DOWNLOAD`.
...
* `LIBARROW_DOWNLOAD`: Unless set to `TRUE` (case insensitive), the build script
