...
capability.
One approach to add more capabilities in an offline install is to pre-download
the required files.
...
allowing the project to build without JSON support.
...
* `TEST_OFFLINE_BUILD`: When set to `true`, the build script will not download
  ...
  in `ARROW_THIRDPARTY_DEPENDENCY_DIR`.
  Regardless of this flag's value, `cmake` will be downloaded if it's unavailable.
  (Currently `RapidJSON` will also be downloaded.
  See discussion [here](https://issues.apache.org/jira/browse/ARROW-13768).)
