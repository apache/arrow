...
Which connects to AWS, and the same can be adapted for other providers.

Just as an example for the non-Amazon case, we provide 
[nyc-taxi.sfo3.digitaloceanspaces.com](https://nyc-taxi.sfo3.digitaloceanspaces.com), 
which is exactly the same as `s3://ursa-labs-taxi-data` (See
[Working with Arrow Datasets and dplyr](dataset.html) for the details).
...
In order to list the files in an S3 bucket, you can just type:

```r
minio$ls('nyc-taxi', recursive = TRUE)
```
...
```r
minio$cd()
june2019 <- minio$path("nyc-taxi/2019/06")
df <- read_parquet(june2019$path("data.parquet"))
```

From here, the same example from the 
[Working with Arrow Datasets and dplyr](dataset.html) vignette can be completed 
with a single change:

```r 
copy_files(minio$path("nyc-taxi"), "nyc-taxi")
```
...
Note that you can copy a single year or month, say
```r 
copy_files("s3://ursa-labs-taxi-data/2019/06", "nyc-taxi/2019/06")
```

Of course, this also applies to the `space` object or any S3 connection to your
