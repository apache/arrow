...
Another way to connect to S3 is to create an `S3Bucket` object once and pass
that to the read/write functions.
An `S3Bucket` object is a subclass of `FileSystem`, containing an `S3FileSystem`
that holds the AWS connection details and a path to the bucket.
...
We can create an `S3Bucket` object and point to specific files in it with the `$path()` method.
In the previous example, this would look like:

```r
bucket <- S3Bucket$create("ursa-labs-taxi-data")
...
An `S3Bucket` is more generally a type of `SubTreeFileSystem` class, which holds a path and a file system to which it corresponds. `SubTreeFileSystem`s can be useful for holding a reference to a subdirectory somewhere, on S3 or elsewhere.
...
## Copying files across file systems
...
2. Pass them as `access_key` and `secret_key` to `S3FileSystem$create()` or `S3Bucket$create()`
...
for temporary access by passing the `role_arn` identifier to `S3FileSystem$create()` or `S3Bucket$create()`.
