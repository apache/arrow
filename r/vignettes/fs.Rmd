...
you have R 4.0 and rtools 40 installed (rtools35 is not supported).
...
With a `FileSystem` object, you can point to specific files in the S3 bucket with the 
`$path()` method.
...
[AWS documentation](https://docs.aws.amazon.com/sdk-for-cpp/v1/developer-guide/credentials.html).
...
and object-storage server that emulates the S3 API.

In order to configure and run MinIO in a Unix-type system, you need to run
these steps from the command line:

```bash
cd ~/Downloads 
wget https://dl.min.io/server/minio/release/linux-amd64/minio
chmod +x minio
MINIO_ROOT_USER=minioadmin MINIO_ROOT_PASSWORD=minioadmin ./minio server ~/nyc-taxi
```

If you were to
run `minio server` locally with its default settings, you can connect to
...
can subtree exactly as in the previous section.

Among other applications, this can be useful for testing out code locally before
running on a remote S3 bucket.

### Connecting to a cloud S3

*This section adapts some elements from [Analyzing Room Temperature Data](https://www.jaredlander.com/2021/03/analyzing-room-temperature-data/#getting-the-data)
by Jared Lander.*
...
MinIO locally. Please note that the use of DigitalOcean here is just an example,
as it can be any other S3 compatible service.

As an example, to connect to DigitalOcean you need to change the endpoint
in the previous MinIO example to something like:

```r
endpoint_override = "sfo3.digitaloceanspaces.com"
```

If you were to connect to a private space (i.e. with sensitive data), you would 
need to provide a token, but please avoid typing passwords in scripts and 
adapt this instead:

```r
space <- arrow::S3FileSystem$create(
  access_key = Sys.getenv('DO_ACCESS_KEY_ID'),
  secret_key = Sys.getenv('DO_SECRET_ACCESS_KEY'),
  scheme = "https",
  endpoint_override = "sfo3.digitaloceanspaces.com"
)
```

Look for `usethis::edit_r_environ()` to put your passwords into environment
variables.

### Copying from/to S3 filesystems

While there are efficient tools such as [rclone](https://rclone.org/), you can
copy a year or a month from a remote S3 by using `arrow`:

```r
mydir <- "~/nyc-taxi/2016/01"
try(dir.create(mydir, recursive = T))
arrow::copy_files("s3://ursa-labs-taxi-data/2016/01", "~/nyc-taxi/2016/01")
```

This works in both directions:
```r
arrow::copy_files("~/nyc-taxi/2016/01", "s3://ursa-labs-taxi-data/2016/01")
```
