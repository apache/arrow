...
On linux, you will need to set `LD_LIBRARY_PATH` to this same directory before launching R and using Arrow. One way to do this is to add it to your profile. # TODO: do we want to recommend this? 

```{bash, save=run & ubuntu & !sys_install}
echo "export LD_LIBRARY_PATH=$(pwd)/dist/lib" >> ~/.profile
```
