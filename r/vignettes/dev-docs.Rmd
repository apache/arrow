...
On linux, you will need to set `LD_LIBRARY_PATH` to the same directory as `LIB_DIR` before launching R and using Arrow. One way to do this is to add it to your profile. # TODO: do we want to recommend this? Is there any other alternative?
...
If you are installing from a user-level directory (like XXX above) and you already have a previous installation of libarrow in a system directory, you may get errors like the following when you install the R package:
...
To solve errors like these, make sure that your arrow library is up to date and make sure that you don't let R link to your system library when building arrow. You can do this a number of different ways: 
...
```{bash, save=run & !sys_install & macos, include=FALSE}
...
```{bash, save=run & !sys_install & ubuntu, include=FALSE}
# Setup troubleshooting section
# install a system-level arrow on macOS
sudo apt update
