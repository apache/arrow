...
columns are not inferred and you may want to recover them from the path. Note
that the new column can't be referenced by later steps of the same query until
you call `compute()` or `collect()`; see `?add_filename` for details.
...
efficient binary columnar format like Parquet or Arrow IPC and partitioning based on
...
You can write it to a new location and translate the files to the Arrow IPC format
