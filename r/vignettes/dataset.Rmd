...
The other thing we can do when writing datasets is select a subset of columns,
reorder columns, and/or rename columns. Suppose we never care about `vendor_id`,
and being a string column, it can take up a lot of space when we read it in, so
let's drop it:
