...
If you have Amazon S3 and/or Google Cloud Storage support enabled in `arrow` (true for most users; see links at the end of this vignette if you need to troubleshoot this), you can connect to the "tiny taxi data" with either of the following commands:
...
- To learn about cloud storage, see `vignette("cloud_storage", package = "arrow")`.
- To learn about `dplyr` with `arrow`, see `vignette("data_wrangling", package = "arrow")`.
- To learn about reading and writing data, see `vignette("read_write", package = "arrow")`.
- To manually enable cloud support on Linux, see `vignette("install_linux", package = "arrow")`.
- To learn about schemas and metadata, see `vignette("metadata", package = "arrow")`.
