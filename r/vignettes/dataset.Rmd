...
recover them from the path, for example with `sub()`. Note that using the new
column in `filter()`, or in some functions such as `substr()` or `%in%`,
requires calling `compute()` or `collect()` first; see `?add_filename` for
details.
