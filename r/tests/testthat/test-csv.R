...
test_that("Can read csv file", {
  ...
  tab1 <- read_csv_arrow(tf, as_dataframe = FALSE)
  tab2 <- read_csv_arrow(mmap_open(tf), as_dataframe = FALSE)
  tab3 <- read_csv_arrow(ReadableFile$create(tf), as_dataframe = FALSE)
...
test_that("read_csv_arrow(as_dataframe=TRUE)", {
  tf <- tempfile()
  on.exit(unlink(tf))

  write.csv(iris, tf, row.names = FALSE)

  tab1 <- read_csv_arrow(tf, as_dataframe = TRUE)
  tab2 <- read_csv_arrow(mmap_open(tf), as_dataframe = TRUE)
  tab3 <- read_csv_arrow(ReadableFile$create(tf), as_dataframe = TRUE)
...
test_that("read_csv_arrow() respects col_select", {
  ...
  tab <- read_csv_arrow(tf, col_select = starts_with("Sepal"), as_dataframe = FALSE)
  expect_equal(tab, Table$create(Sepal.Length = iris$Sepal.Length, Sepal.Width = iris$Sepal.Width))

  tib <- read_csv_arrow(tf, col_select = starts_with("Sepal"), as_dataframe = TRUE)
