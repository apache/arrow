...
test_that("RecordBatch", {
  ...
  expect_equal(
    ...
    schema(
      ...
      fct = dictionary(int32(), arrow_array(letters[1:10]))
    )
  )
  ...
  expect_equal(col_fct$type, dictionary(int32(), arrow_array(letters[1:10])))

  batch2 <- batch$RemoveColumn(0)
  expect_equal(
    batch2$schema,
    schema(dbl = float64(), lgl = boolean(), chr = utf8(), fct = dictionary(int32(), arrow_array(letters[1:10])))
  )
...
test_that("RecordBatch with 0 rows are supported", {
  ...
  expect_equal(
    ...
    schema(
      ...
      lgl = boolean(),
      chr = utf8(),
      fct = dictionary(int32(), arrow_array(c("a", "b")))
    )
  )
})
