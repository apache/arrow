...
test_that("Can use across() within summarise()", {
  ...
  compare_dplyr_binding(
    .input %>%
      group_by(some_grouping) %>%
      summarise(across(c(int, dbl), mean)) %>%
      collect(),
    tbl
  )
  ...
  compare_dplyr_binding(
    .input %>%
      group_by(some_grouping) %>%
      summarise(across(c(int, dbl), list(mean = mean, sd))) %>%
      collect(),
    tbl
  )
  ...
  compare_dplyr_binding(
    .input %>%
      group_by(some_grouping) %>%
      summarise(across(c(int, dbl), list(mean = mean, sd = sd))) %>%
      collect(),
    tbl
  )

  # we don't support purrr-style formulas yet here
  # but do we support them elsewhere in Arrow?
  # TODO: check this
  # compare_dplyr_binding(
  #   .input %>%
  #     group_by(some_grouping) %>%
  #     summarise(across(c(int, dbl),  ~ mean(.x, na.rm = TRUE))) %>%
  #     collect(),
  #   tbl
  # )

  # # this fails as .names isn't used here
  # compare_dplyr_binding(
  #   .input %>%
  #     group_by(some_grouping) %>%
  #     summarise(across(starts_with("dbl"), mean, .names = "mean_{.col}")) %>%
  #     collect(),
  #   tbl
  # )
  #
  ...
  #   .input %>%
  #     group_by(some_grouping) %>%
  #     summarise(across(c(int, dbl), list(mean = mean, sd = sd), .names = "{.col}.{.fn}")) %>%
  #     collect(),
  #   tbl
  # )
  #
  # # this doesn't work yet either, also because .names
  # compare_dplyr_binding(
  #   .input %>%
  #     group_by(some_grouping) %>%
  #     summarise(across(starts_with("dbl"), list(mean, sd), .names = "{.col}.fn.{.fn}")) %>%
  #     collect(),
  #   tbl
  # )
