...
test_that("Functions that take ... but we only accept a single arg", {
  ...
  expect_error(call_translation_agg("n_distinct"), "n_distinct() with 0 arguments", fixed = TRUE)
  expect_error(call_translation_agg("sum"), "sum() with 0 arguments", fixed = TRUE)
  expect_error(call_translation_agg("any"), "any() with 0 arguments", fixed = TRUE)
  expect_error(call_translation_agg("all"), "all() with 0 arguments", fixed = TRUE)
  expect_error(call_translation_agg("min"), "min() with 0 arguments", fixed = TRUE)
  expect_error(call_translation_agg("max"), "max() with 0 arguments", fixed = TRUE)
  expect_error(call_translation_agg("n_distinct", 1, 2), "Multiple arguments to n_distinct()")
  expect_error(call_translation_agg("sum", 1, 2), "Multiple arguments to sum")
  expect_error(call_translation_agg("any", 1, 2), "Multiple arguments to any()")
  expect_error(call_translation_agg("all", 1, 2), "Multiple arguments to all()")
  expect_error(call_translation_agg("min", 1, 2), "Multiple arguments to min()")
  expect_error(call_translation_agg("max", 1, 2), "Multiple arguments to max()")
})
