...
test_that("head/tail", {
  ...
  expect_data_frame(head(ds), head(df1))
  expect_data_frame(tail(ds), tail(df2))
  ...
  expect_data_frame(as.data.frame(head(ds, 0))[, names(ds) != "fct"], zero_df)
  expect_data_frame(as.data.frame(tail(ds, 0))[, names(ds) != "fct"], zero_df)

  # Two more cases: more than 1 batch, and more than nrow
  for (n in c(12, 1000)) {
    expect_data_frame(head(ds, n), head(big_df, n))
    expect_data_frame(tail(ds, n), tail(big_df, n))
  }
...
test_that("Scanner$ScanBatches", {
  ...
  expect_data_frame(table, rbind(df1, df2))
