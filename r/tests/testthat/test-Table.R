...
test_that("Table cast (ARROW-3741)", {
  tab <- to_arrow(data.frame(x = 1:10, y = 1:10))
...
test_that("table_from_dots() handles record batches with splicing", {
  # TODO: reimplement as something like rbind for record batches?
  batch <- record_batch(x = 1:2, y = letters[1:2])
  tab <- table_from_dots(batch, batch, batch)
...
test_that("table_from_dots() handles ... of arrays, chunked arrays, vectors", {
  a <- to_arrow(1:10)
  ca <- chunked_array(1:5, 6:10)
  v <- rnorm(10)
  tbl <- tibble::tibble(x = 1:10, y = letters[1:10])

  tab <- table_from_dots(a = a, b = ca, c = v, !!!tbl)
  expect_equal(
    tab$schema,
