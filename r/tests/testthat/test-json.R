...
test_that("Can read json file with scalars columns (ARROW-5503)", {
  ...
  tab1 <- read_json_arrow(tf, as_dataframe = FALSE)
  tab2 <- read_json_arrow(mmap_open(tf), as_dataframe = FALSE)
  tab3 <- read_json_arrow(ReadableFile$create(tf), as_dataframe = FALSE)
