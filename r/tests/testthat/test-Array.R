...
test_that("Array supports NA", {
  x_int <- to_arrow(as.integer(c(1:10, NA)))
  x_dbl <- to_arrow(as.numeric(c(1:10, NA)))
...
test_that("empty arrays are supported", {
  x <- character()
  expect_equal(to_arrow(x)$as_vector(), x)
...
test_that("to_arrow() supports the type= argument. conversion from INTSXP and int64 to all int types", {
  num_int32 <- 12L
  ...
  for(type in types) {
    expect_equal(to_arrow(num_int32, type = type)$type, type)
    expect_equal(to_arrow(num_int64, type = type)$type, type)
  }
})

test_that("to_arrow() aborts on overflow", {
  expect_error(to_arrow(128L, type = int8())$type, "Invalid.*downsize")
  expect_error(to_arrow(-129L, type = int8())$type, "Invalid.*downsize")

  expect_error(to_arrow(256L, type = uint8())$type, "Invalid.*downsize")
  expect_error(to_arrow(-1L, type = uint8())$type, "Invalid.*downsize")

  expect_error(to_arrow(32768L, type = int16())$type, "Invalid.*downsize")
  expect_error(to_arrow(-32769L, type = int16())$type, "Invalid.*downsize")

  expect_error(to_arrow(65536L, type = uint16())$type, "Invalid.*downsize")
  expect_error(to_arrow(-1L, type = uint16())$type, "Invalid.*downsize")

  expect_error(to_arrow(65536L, type = uint16())$type, "Invalid.*downsize")
  expect_error(to_arrow(-1L, type = uint16())$type, "Invalid.*downsize")

  expect_error(to_arrow(bit64::as.integer64(2^31), type = int32()), "Invalid.*downsize")
  expect_error(to_arrow(bit64::as.integer64(2^32), type = uint32()), "Invalid.*downsize")
})

test_that("to_arrow() does not convert doubles to integer", {
  types <- list(
    int8(), int16(), int32(), int64(),
    uint8(), uint16(), uint32(), uint64()
  )
  for(type in types) {
    expect_error(to_arrow(10, type = type)$type, "Cannot convert.*REALSXP")
  }
})

test_that("to_arrow() converts raw vectors to uint8 arrays (ARROW-3794)", {
  expect_equal(to_arrow(as.raw(1:10))$type, uint8())
})

test_that("Array<int8>$as_vector() converts to integer (ARROW-3794)", {
  a <- to_arrow((-128):127)$cast(int8())
  expect_equal(a$type, int8())
  expect_equal(a$as_vector(), (-128):127)

  a <- to_arrow(0:255)$cast(uint8())
  expect_equal(a$type, uint8())
  expect_equal(a$as_vector(), 0:255)
})

test_that("to_arrow() recognise arrow::Array (ARROW-3815)", {
  a <- to_arrow(1:10)
  expect_equal(a, to_arrow(a))
})

test_that("to_arrow() handles data frame -> struct arrays (ARROW-3811)", {
...
test_that("to_arrow() can handle data frame with custom struct type (not infered)", {
  df <- tibble::tibble(x = 1:10, y = 1:10)
  type <- struct(x = float64(), y = int16())
  a <- to_arrow(df, type = type)
  expect_equal(a$type, type)

  type <- struct(x = float64(), y = int16(), z = int32())
  expect_error(to_arrow(df, type = type), regexp = "Number of fields in struct.* incompatible with number of columns in the data frame")

  type <- struct(y = int16(), x = float64())
  expect_error(to_arrow(df, type = type), regexp = "Field name in position.*does not match the name of the column of the data frame")

  type <- struct(x = float64(), y = utf8())
  expect_error(to_arrow(df, type = type), regexp = "Cannot convert R object to string array")
})
