...
write_dataset <- function(dataset,
  ...
  #   # This does not exist yet (in the R bindings at least)
  #   dataset <- InMemoryDataset$create(dataset)
  # }
  if (!inherits(format, "FileFormat")) {
    format <- FileFormat$create(format, ...)
  }
  if (!inherits(partitioning, "Partitioning") && !is.null(partitioning)) {
    assert_is(partitioning, "character")
    partition_schema <- dataset$schema[partitioning]
    if (identical(partitioning_type, "hive")) {
