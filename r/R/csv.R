...
#' @param col_types A [schema][Schema] or a named list of [types][DataType],
#' indicating optional per-column types (disabling type inference on these columns).
...
CsvConvertOptions$create <- function(check_utf8 = TRUE,
  ...
  } else if (inherits(col_types, "Schema")) {
    # all good
  } else if (is.null(col_types)){
    # all good
  } else {
    abort("Unsupported `col_types` specification")
  }
