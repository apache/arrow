...
def get_dfs(object_ids):
    ...
    return [
        pa.RecordBatchStreamReader(buf).read_next_batch().to_pandas()
        for buf in buffers
    ]
...
if __name__ == '__main__':
    ...
    df = pd.DataFrame(
        np.random.randn(num_rows, num_cols), columns=column_names
    )

    partition_ids = [
        put_df(partition) for partition in np.split(df, num_cores)
    ]
    ...
    sorted_df_ids, pivot_groups = list(
        zip(*pool.map(local_sort, partition_ids))
    )

    # Choose the pivots.
    all_pivots = np.concatenate(pivot_groups)
    indices = np.linspace(
        0, len(all_pivots) - 1, num=num_cores, dtype=np.int64
    )
    ...
    results = list(
        zip(
            *pool.map(
                local_partitions,
                zip(sorted_df_ids, len(sorted_df_ids) * [pivots]),
            )
        )
    )
    ...
    resulting_ids = [
        object_id for object_id in object_ids if object_id is not None
    ]
    ...
    print(
        'Parallel sort took {} seconds.'.format(
            parallel_sort_end - parallel_sort_start
        )
    )
    ...
    print(
        'Serial sort took {} seconds.'.format(
            serial_sort_end - serial_sort_start
        )
    )
