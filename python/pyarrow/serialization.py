...
def _pickle_to_buffer(x):
    pickled = builtin_pickle.dumps(
        x, protocol=builtin_pickle.HIGHEST_PROTOCOL
    )
...
def _register_custom_pandas_handlers(context):
    ...
    def _serialize_pandas_dataframe(obj):
        if pdcompat._pandas_api.has_sparse and isinstance(
            obj, pd.SparseDataFrame
        ):
...
def register_default_serialization_handlers(serialization_context):
    ...
    serialization_context.register_type(
        type(lambda: 0), "function", pickle=True
    )
