...
def _deserialize_numpy_matrix(data):
    if data[1] != "|O":
        assert data[0].dtype == np.uint8
        return np.matrix(data[0].view(descr_to_dtype(data[1])), copy=False)
    else:
...
def _serialize_pyarrow_recordbatch(batch):
    ...
    writer = pyarrow.RecordBatchStreamWriter(output_stream, schema=batch.schema)
...
def _serialize_pyarrow_table(table):
    output_stream = pyarrow.BufferOutputStream()
    writer = pyarrow.RecordBatchStreamWriter(output_stream, schema=table.schema)
...
def _register_custom_pandas_handlers(context):
    ...
    def _serialize_pandas_dataframe(obj):
        if isinstance(obj, pd.SparseDataFrame):
            raise NotImplementedError(sparse_type_error_msg.format("SparseDataFrame"))
    ...
    def _serialize_pandas_series(obj):
        if isinstance(obj, pd.SparseSeries):
            raise NotImplementedError(sparse_type_error_msg.format("SparseSeries"))
...
def register_default_serialization_handlers(serialization_context):
    ...
    if sys.version_info < (3, 0):
        serialization_context.register_type(
            ...
            "long",  # noqa: F821
            custom_serializer=lambda obj: str(obj),
            custom_deserializer=lambda data: long(data),
        )  # noqa: F821

    serialization_context.register_type(type(lambda: 0), "function", pickle=True)
