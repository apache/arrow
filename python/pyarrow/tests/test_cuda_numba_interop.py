...
@pytest.mark.parametrize(
    "c", range(len(context_choice_ids)), ids=context_choice_ids
)
def test_context(c):
...
def test_from_object(c, dtype, size):
    ctx, nb_ctx = context_choices[c]
    arr, cbuf = make_random_buffer(
        size, target='device', dtype=dtype, ctx=ctx
    )
    ...
    if size >= 8:
        ...
        with pytest.raises(
            ValueError, match=('array data is non-contiguous')
        ):
