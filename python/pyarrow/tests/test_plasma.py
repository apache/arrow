...
class TestPlasmaClient:
    ...
    def test_create_with_metadata(self):
        for length in range(0, 1000, 3):
            ...
            # Create a new buffer and write to it.
            memory_buffer = np.frombuffer(
                self.plasma_client.create(object_id, length, metadata),
                dtype="uint8",
            )
            for i in range(length):
                memory_buffer[i] = i % 256
    ...
    def test_create_existing(self):
        ...
        for _ in range(1000):
            ...
            self.plasma_client.create(
                object_id, length, generate_metadata(length)
            )
            try:
                self.plasma_client.create(
                    object_id, length, generate_metadata(length)
    ...
    def test_create_and_seal(self):
        ...
        for i in range(1000):
            ...
            assert (
                self.plasma_client.get_metadata([object_ids[i]])[
                    0
                ].to_pybytes()
                == i * b'b'
            )
    ...
    def test_get(self):
        ...
        assert (
            self.plasma_client.get_buffers(
                [object_id], timeout_ms=1, with_meta=True
            )[0][1]
            is None
        )
        self.plasma_client.seal(object_id)
    ...
    def test_hash(self):
        ...
        metadata = generate_metadata(length)
        memory_buffer = np.frombuffer(
            self.plasma_client.create(object_id1, length, metadata),
            dtype="uint8",
        )
        for i in range(length):
            memory_buffer[i] = i % 256
        ...
        memory_buffer = np.frombuffer(
            self.plasma_client.create(object_id2, length, metadata),
            dtype="uint8",
        )
        for i in range(length):
            memory_buffer[i] = i % 256
        ...
        metadata = generate_metadata(length)
        memory_buffer = np.frombuffer(
            self.plasma_client.create(object_id3, length, metadata),
            dtype="uint8",
        )
        for i in range(length):
        ...
        memory_buffer = np.frombuffer(
            self.plasma_client.create(object_id4, length, metadata4),
            dtype="uint8",
        )
        for i in range(length):
            memory_buffer[i] = (i + 1) % 256
    ...
    def test_subscribe_deletions(self):
        ...
        for i in self.SUBSCRIBE_TEST_SIZES:
            ...
            for j in range(i):
                notification_info = (
                    self.plasma_client2.get_next_notification()
                )
                recv_objid, recv_dsize, recv_msize = notification_info
                assert object_ids[j] == recv_objid
                assert data_sizes[j] == recv_dsize
    ...
    def test_client_death_during_get(self):
        ...
        p = multiprocessing.Process(
            target=self._client_blocked_in_get,
    ...
    def test_client_getting_multiple_objects(self):
        ...
        p = multiprocessing.Process(
            target=self._client_get_multiple,
        ...
        while True:
            if time.time() - start_time > 5:
                raise Exception(
                    "Timing out while waiting for blocked client "
                    "to finish."
                )
            if not p.is_alive():
                break
...
def test_plasma_list():
    ...
    with plasma.start_plasma_store(
        ...
        # Test sizes
        u, _, _ = create_object(
            plasma_client, 11, metadata_size=7, seal=False
        )
