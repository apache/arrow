...
def _check_series_roundtrip(s, type_=None, expected_pa_type=None):
    ...
    if pa.types.is_timestamp(arr.type) and arr.type.tz is not None:
        result = result.dt.tz_localize("utc").dt.tz_convert(arr.type.tz)
...
class TestConvertMetadata(object):
    ...
    def test_string_column_index(self):
        ...
        assert column_indexes["pandas_type"] == ("bytes" if six.PY2 else "unicode")
    ...
    def test_multiindex_duplicate_values(self):
        ...
        index = pd.MultiIndex.from_arrays(
            [["foo", "foo", "bar"], numbers], names=["foobar", "some_numbers"]
        )
...
class TestConvertDateTimeLikeTypes(object):
    ...
    @pytest.mark.skipif(
        six.PY2, reason="datetime.timezone is available since " "python version 3.2"
    )
    ...
    def test_pandas_datetime_to_date64(self, mask):
        s = pd.to_datetime(
            ["2018-05-10T00:00:00", "2018-05-11T00:00:00", "2018-05-12T00:00:00"]
        )
        arr = pa.Array.from_pandas(s, type=pa.date64(), mask=mask)

        data = np.array([date(2018, 5, 10), date(2018, 5, 11), date(2018, 5, 12)])
    ...
    def test_pandas_datetime_to_date64_failures(self, mask):
        s = pd.to_datetime(
            ["2018-05-10T10:24:01", "2018-05-11T10:24:01", "2018-05-12T10:24:01"]
        )
    ...
    def test_array_types_date_as_object(self):
        ...
        expected = np.array(
            ["2000-01-01", None, "1970-01-01", "2040-02-26"], dtype="datetime64"
        )
    ...
    def test_date_objects_typed(self):
        ...
        expected_pandas = pd.DataFrame(
            {"date32": ex_datetime64ns, "date64": ex_datetime64ns}, columns=colnames
        )
    ...
    def test_timedelta(self):
        ...
        df = pd.DataFrame(
            {
                "timedelta": np.arange(
                    start=0, stop=3 * 86400000, step=86400000, dtype="timedelta64[ms]"
                )
            }
        )
        pa.Table.from_pandas(df)
...
class TestConvertListTypes(object):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (pa.string, [[u"aaa", u"bb"], [u"c"], None], [None, [u"c"], None]),
...
class TestConvertMisc(object):
    ...
    def test_strided_data_import(self):
        ...
        # strings
        cases.append(
            np.array([tm.rands(10) for i in range(N * K)], dtype=object)
            .reshape(N, K)
            .copy()
        )
    ...
    def test_partial_schema(self):
        ...
        _check_pandas_roundtrip(
            df, schema=partial_schema, expected_schema=partial_schema
        )
...
@pytest.mark.skipif(
    LooseVersion(np.__version__) >= "0.16", reason="Until numpy/numpy#12745 is resolved"
)
