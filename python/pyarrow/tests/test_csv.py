...
def test_read_options():
    ...
    opts = cls(
        block_size=1234, use_threads=False, skip_rows=42, column_names=["a", "b", "c"]
    )
...
class BaseTestCSVRead:
    ...
    def test_custom_nulls(self):
        ...
        assert table.schema == schema
        assert table.to_pydict() == {"a": [u"#N/A"], "b": [u""]}
    ...
    def test_options_delimiter(self):
        rows = b"a;b,c\nde,fg;eh\n"
        table = self.read_bytes(rows)
        assert table.to_pydict() == {"a;b": [u"de"], "c": [u"fg;eh"]}
        opts = ParseOptions(delimiter=";")
        table = self.read_bytes(rows, parse_options=opts)
        assert table.to_pydict() == {"a": [u"de,fg"], "b,c": [u"eh"]}
