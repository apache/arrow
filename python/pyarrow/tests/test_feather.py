...
def test_read_table(version):
    ...
    df = pd.DataFrame(values, columns=['col_' + str(i) for i in range(100)])
...
def test_float_nulls(version):
    ...
    ex_frame = pd.DataFrame(dict(zip(dtypes, expected_cols)), columns=dtypes)
...
def test_integer_with_nulls(version):
    ...
    ex_frame = pd.DataFrame(
        dict(zip(int_dtypes, expected_cols)), columns=int_dtypes
    )
...
def test_buffer_bounds_error(version):
    ...
    for i in range(16, 256):
        ...
        write_feather(
            pa.table([values], names=['arr']), path, version=version
        )
        result = read_feather(path)
...
def test_multithreaded_read(version):
    data = {'c{}'.format(i): [''] * 10 for i in range(100)}
    df = pd.DataFrame(data)
...
def test_timestamp(version):
    ...
    df['with_tz'] = df.naive.dt.tz_localize('utc').dt.tz_convert(
        'America/Los_Angeles'
    )
...
def test_timestamp_with_nulls(version):
    df = pd.DataFrame(
        {'test': [pd.Timestamp(2016, 1, 1), None, pd.Timestamp(2016, 1, 3)]}
    )
...
def test_timedelta_with_nulls_v1():
    df = pd.DataFrame(
        {'test': [pd.Timedelta('1 day'), None, pd.Timedelta('3 day')]}
    )
    ...
    df = pd.DataFrame(
        {
            'test': pd.DatetimeIndex(
                [1451606400000000001, None, 14516064000030405]
            )
        }
    )
    df['with_tz'] = df.test.dt.tz_localize('utc')
...
def test_non_string_columns(version):
    df = pd.DataFrame({0: [1, 2, 3, 4], 1: [True, False, True, False]})
...
def test_unicode_filename(version):
    ...
    _check_pandas_roundtrip(
        df, path=random_path(prefix=name), version=version
    )


def test_read_columns(version):
    df = pd.DataFrame(
        {'foo': [1, 2, 3, 4], 'boo': [5, 6, 7, 8], 'woo': [1, 3, 5, 7]}
    )
    expected = df[['boo', 'woo']]

    _check_pandas_roundtrip(
        df, expected, version=version, columns=['boo', 'woo']
    )
...
def test_filelike_objects(version):
    ...
    df = pd.DataFrame(
        np.arange(12).reshape(4, 3), columns=['a', 'b', 'c']
    ).copy()
...
def test_v2_compression_options():
    ...
    for compression, compression_level in cases:
        _check_pandas_roundtrip(
            df, compression=compression, compression_level=compression_level
        )
    ...
    with pytest.raises(
        pa.ArrowInvalid, match="doesn't support setting a compression level"
    ):
        write_feather(df, buf, compression='lz4', compression_level=10)

    # Trying to compress with V1
    with pytest.raises(
        ValueError, match="Feather V1 files do not support compression option"
    ):
        write_feather(df, buf, compression='lz4', version=1)

    # Trying to set chunksize with V1
    with pytest.raises(
        ValueError, match="Feather V1 files do not support chunksize option"
    ):
        write_feather(df, buf, chunksize=4096, version=1)

    # Unsupported compressor
    with pytest.raises(
        ValueError, match='compression="snappy" not supported'
    ):
...
def test_chunked_binary_error_message():
    ...
    with pytest.raises(
        ValueError,
        match="'byte_col' exceeds 2GB maximum "
