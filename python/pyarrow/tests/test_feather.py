...
class TestFeatherReader(unittest.TestCase):
    ...
    def test_dataset(self):
        ...
        for index, path in enumerate(paths):
            ...
            for col in range(num_values[1]):
                writer.write_array(df.columns[col], df.iloc[rows[0] : rows[1], col])
    ...
    def test_num_columns_attr(self):
        ...
        df2 = pd.DataFrame({"foo": [1, 2, 3, 4, 5], "bar": [1, 2, 3, 4, 5]})
    ...
    def test_multithreaded_read(self):
        data = {"c{0}".format(i): [""] * 10 for i in range(100)}
        df = pd.DataFrame(data)
    ...
    def test_timestamp_with_nulls(self):
        df = pd.DataFrame(
            {"test": [pd.datetime(2016, 1, 1), None, pd.datetime(2016, 1, 3)]}
        )
        df["with_tz"] = df.test.dt.tz_localize("utc")

        self._check_pandas_roundtrip(df, null_counts=[1, 1])

    @pytest.mark.xfail(reason="not supported ATM", raises=NotImplementedError)
    ...
    def test_out_of_float64_timestamp_with_nulls(self):
        df = pd.DataFrame(
            {"test": pd.DatetimeIndex([1451606400000000001, None, 14516064000030405])}
        )
        df["with_tz"] = df.test.dt.tz_localize("utc")
        self._check_pandas_roundtrip(df, null_counts=[1, 1])
    ...
    @pytest.mark.skipif(
        not os.path.supports_unicode_filenames, reason="unicode filenames not supported"
    )
    ...
    def test_read_columns(self):
        data = {"foo": [1, 2, 3, 4], "boo": [5, 6, 7, 8], "woo": [1, 3, 5, 7]}
