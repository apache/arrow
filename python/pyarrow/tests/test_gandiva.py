...
def test_tree_exp_builder():
    ...
    condition = builder.make_function(
...
def test_filter():
    ...
    table = pa.Table.from_arrays(
        [pa.array([1.0 * i for i in range(10000)])], ['a']
    )
...
def test_in_expr_todo():
    ...
    builder = gandiva.TreeExprBuilder()
    node_a = builder.make_field(table.schema.field("a"))
    cond = builder.make_in_expression(
        node_a, [datetime_2], pa.timestamp('ms')
    )
    condition = builder.make_condition(cond)
