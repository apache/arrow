...
def test_variable_shape_tensor_class_methods(value_type):
    ...
    for x, y in zip(arr, ndarray_list):
        np.testing.assert_array_equal(x.to_numpy(), y)
...
def test_variable_shape_tensor_from_row_splits_validation():
    ...
    # Nulls in values
    with pytest.raises(ValueError, match="values must not contain nulls"):
        pa.VariableShapeTensorArray.from_row_splits(
            pa.array([1, None, 3], pa.int32()),
            pa.array([0, 3], pa.int32()),
    ...
    # Null shape row
    with pytest.raises(ValueError, match=r"shapes\[0\] must not be null"):
        null_shapes = pa.array([None, [2, 1]], pa.list_(pa.int32(), 2))
        pa.VariableShapeTensorArray.from_row_splits(
            pa.array([1, 2, 3, 4, 5, 6, 7, 8], pa.int32()),
            pa.array([0, 6, 8], pa.int32()),
            null_shapes)

    # Null dimension within shape
    with pytest.raises(ValueError, match=r"shapes\[0\]\[1\] must not be null"):
        pa.VariableShapeTensorArray.from_row_splits(
            pa.array([1, 2, 3, 4, 5, 6, 7, 8], pa.int32()),
            pa.array([0, 6, 8], pa.int32()),
            pa.array([[2, None], [2, 1]], pa.list_(pa.int32(), 2)))

    # Negative dimension value
    with pytest.raises(ValueError, match=r"shapes\[0\]\[1\] must be non-negative"):
        pa.VariableShapeTensorArray.from_row_splits(
            pa.array([1, 2, 3, 4, 5, 6, 7, 8], pa.int32()),
            pa.array([0, 6, 8], pa.int32()),
            pa.array([[2, -3], [2, 1]], pa.list_(pa.int32(), 2)))
...
def test_variable_shape_tensor_type_ipc(tensor_type):
    ...
    assert result.storage.to_pylist() == [
        {"data": [1, 2, 3, 4, 5, 6], "shape": [2, 3]},
        {"data": [7, 8], "shape": [1, 2]},
    ]
...
@pytest.mark.numpy
def test_variable_shape_tensor_scalar():
    tensor_type = pa.variable_shape_tensor(pa.int32(), 2)
    values = pa.array([[1, 2, 3, 4, 5, 6], [7, 8]], pa.list_(pa.int32()))
    shapes = pa.array([[2, 3], [2, 1]], pa.list_(pa.int32(), 2))
    storage = pa.StructArray.from_arrays(
    ...
    scalar = arr[0]
    assert isinstance(scalar, pa.VariableShapeTensorScalar)
    np.testing.assert_array_equal(
        scalar.to_numpy(),
        np.array([[1, 2, 3], [4, 5, 6]], dtype=np.int32))

    tensor = scalar.to_tensor()
    ...
    storage_with_null = pa.StructArray.from_arrays(
        [values, shapes], names=["data", "shape"], mask=mask)
    arr_with_null = pa.ExtensionArray.from_storage(tensor_type, storage_with_null)
    null_scalar = arr_with_null[0]

    assert not null_scalar.is_valid
    with pytest.raises(ValueError, match="Cannot convert null scalar to numpy array"):
        null_scalar.to_numpy()
    with pytest.raises(ValueError, match="Cannot convert null scalar to Tensor"):
        null_scalar.to_tensor()


@pytest.mark.numpy
def test_fixed_shape_tensor_scalar_null():
    tensor_type = pa.fixed_shape_tensor(pa.int32(), [2, 3])
    storage = pa.array([[1, 2, 3, 4, 5, 6], None],
                       pa.list_(pa.int32(), 6))
    arr = pa.ExtensionArray.from_storage(tensor_type, storage)
    null_scalar = arr[1]
...
def test_variable_shape_tensor_type_cast():
    tensor_type = pa.variable_shape_tensor(pa.int32(), 2)
    values = pa.array([[1, 2, 3, 4, 5, 6], [7, 8]], pa.list_(pa.int32()))
    shapes = pa.array([[2, 3], [2, 1]], pa.list_(pa.int32(), 2))
    storage = pa.StructArray.from_arrays(
...
@pytest.mark.parametrize(("tensor_type", "text"), [
    (
        pa.variable_shape_tensor(pa.int8(), 2),
        'variable_shape_tensor[value_type=int8, ndim=2]'
    ),
    (
        pa.variable_shape_tensor(pa.int32(), 3, permutation=[0, 2, 1]),
        'variable_shape_tensor[value_type=int32, ndim=3, permutation=[0,2,1]]'
    ),
    (
        pa.variable_shape_tensor(pa.int64(), 3, dim_names=['C', 'H', 'W']),
        'variable_shape_tensor[value_type=int64, ndim=3, dim_names=[C,H,W]]'
    ),
    (
        pa.variable_shape_tensor(pa.int32(), 2, uniform_shape=[2, None]),
        'variable_shape_tensor[value_type=int32, ndim=2, uniform_shape=[2,null]]'
    ),
])
def test_variable_shape_tensor_type_str(tensor_type, text):
    tensor_type_str = tensor_type.__str__()
    assert text in tensor_type_str


@pytest.mark.numpy
def test_variable_shape_tensor_from_numpy_dim_names_validation():
    arrays = [np.array([[1, 2, 3], [4, 5, 6]], dtype=np.int32)]

    # Wrong-length dim_names
    with pytest.raises(ValueError, match="length of dim_names"):
        pa.VariableShapeTensorArray.from_numpy_ndarray(
            arrays, dim_names=["only_one"])

    # Non-string dim_names
    with pytest.raises(TypeError, match="Each element of dim_names must be a string"):
        pa.VariableShapeTensorArray.from_numpy_ndarray(
            arrays, dim_names=[1, 2])
