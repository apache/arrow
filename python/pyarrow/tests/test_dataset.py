...
def test_dataset_join(tempdir):
    ...
    ds.write_dataset(t1, tempdir / "t1", format="csv")
    ds1 = ds.dataset(tempdir / "t1", format="csv")

    t2 = pa.table({
        "colB": [99, 2, 1],
        "col3": ["Z", "B", "A"]
    })
    ds.write_dataset(t2, tempdir / "t2", format="csv")
    ds2 = ds.dataset(tempdir / "t2", format="csv")
