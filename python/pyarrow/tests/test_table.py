...
def test_recordbatch_basics():
    ...
    assert pydict == OrderedDict([("c0", [0, 1, 2, 3, 4]), ("c1", [-10, -5, 0, 5, 10])])
...
def test_recordbatch_pickle():
    data = [pa.array(range(5)), pa.array([-10, -5, 0, 5, 10])]
    schema = pa.schema(
        [pa.field("ints", pa.int8()), pa.field("floats", pa.float32())]
...
def test_recordbatch_slice_getitem():
    ...
    expected2 = pa.RecordBatch.from_arrays([x.slice(2, 2) for x in data], names)
...
def test_table_from_pydict():
    ...
    data = OrderedDict(
        [
            ("strs", pa.chunked_array([[u""], [u"foo", u"bar"]])),
            ("floats", pa.chunked_array([[4.5], [5.0, None]])),
        ]
    )
    table = pa.Table.from_pydict(data)
    assert table.num_columns == 2
    ...
    data = OrderedDict([("strs", [u"", u"foo", u"bar"]), ("floats", [4.5, 5, None])])
    table = pa.Table.from_pydict(data)
    assert table.num_columns == 2
    assert table.num_rows == 3
    assert table.schema == schema
