...
def test_recordbatch_from_pydict():
    table = pa.RecordBatch.from_pydict({})
    assert table.num_columns == 0
    assert table.num_rows == 0
    assert table.schema == pa.schema([])
    assert table.to_pydict() == {}
    ...
    metadata = {b'foo': b'bar'}
    schema = schema.with_metadata(metadata)
    table = pa.RecordBatch.from_pydict(data, metadata=metadata)
    assert table.schema == schema
    assert table.schema.metadata == metadata
    assert table.to_pydict() == data

    # With explicit schema
    table = pa.RecordBatch.from_pydict(data, schema=schema)
    assert table.schema == schema
    assert table.schema.metadata == metadata
    assert table.to_pydict() == data
    ...
    # Cannot pass both schema and metadata
    with pytest.raises(ValueError):
        pa.RecordBatch.from_pydict(data, schema=schema, metadata=metadata)

    # Non-convertible values given schema
    with pytest.raises(TypeError):
        pa.RecordBatch.from_pydict({'c0': [0, 1, 2]},
                                   schema=pa.schema([("c0", pa.string())]))
    ...
    # Passed wrong schema type
    with pytest.raises(TypeError):
        pa.RecordBatch.from_pydict({'a': [1, 2, 3]}, schema={})
