...
def test_table_from_batches_and_schema():
    ...
    batch = pa.RecordBatch.from_arrays([pa.array([1]), pa.array(['hello'])],
                                       names=['a', 'b'])
    table = pa.Table.from_batches([batch], schema)
    assert table.schema.equals(schema)
    assert table.column(0) == pa.column('a', pa.array([1]))
    assert table.column(1) == pa.column('b', pa.array(['hello']))

    incompatible_schema = pa.schema([pa.field('c', pa.int64())])
    with pytest.raises(pa.ArrowInvalid):
