...
def test_no_memory_map(tempdir):
    ...
    table = pa.Table.from_pandas(df)
    _check_roundtrip(table, read_table_kwargs={"memory_map": False}, version="2.0")
...
def test_pandas_parquet_configuration_options(tempdir):
    ...
    for use_dictionary in [True, False]:
        _write_table(
            arrow_table, filename, version="2.0", use_dictionary=use_dictionary
        )
        table_read = _read_table(filename)
        df_read = table_read.to_pandas()
        tm.assert_frame_equal(df, df_read)
    ...
    for write_statistics in [True, False]:
        _write_table(
            arrow_table, filename, version="2.0", write_statistics=write_statistics
        )
        table_read = _read_table(filename)
        df_read = table_read.to_pandas()
        tm.assert_frame_equal(df, df_read)
    ...
    for compression in ["NONE", "SNAPPY", "GZIP", "LZ4", "ZSTD"]:
        _write_table(arrow_table, filename, version="2.0", compression=compression)
        table_read = _read_table(filename)
        df_read = table_read.to_pandas()
        tm.assert_frame_equal(df, df_read)
...
def make_sample_file(table_or_df):
    ...
    buf = io.BytesIO()
    _write_table(
        a_table, buf, compression="SNAPPY", version="2.0", coerce_timestamps="ms"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([-1.1, 2.2, 2.3, None, 4.4], pa.float32(), "FLOAT", -1.1, 4.4, 1, 4, 0),
        ([-1.1, 2.2, 2.3, None, 4.4], pa.float64(), "DOUBLE", -1.1, 4.4, 1, 4, 0),
        (
            [u"", u"b", unichar(1000), None, u"aaa"],
            pa.binary(),
            "BYTE_ARRAY",
            b"",
            unichar(1000).encode("utf-8"),
            1,
            4,
            0,
        ),
        ([True, False, False, True, True], pa.bool_(), "BOOLEAN", False, True, 0, 5, 0),
        (
...
def test_validate_schema_write_table(tempdir):
    ...
    with pq.ParquetWriter(
        path, simple_schema, version="2.0", compression="snappy", flavor="spark"
    ) as w:
...
def test_coerce_timestamps(tempdir):
    ...
    with pytest.raises(ValueError):
        _write_table(arrow_table, filename, version="2.0", coerce_timestamps="unknown")
...
def test_column_of_lists(tempdir):
    ...
    if PY2:
        ...
        for col in ["date32[day]_list", "date64[ms]_list"]:
            df[col] = df[col].apply(lambda x: list(map(np.datetime64, x)) if x else x)
...
def test_date_time_types(tempdir):
    ...
    _write_table(table, filename, version="2.0", use_deprecated_int96_timestamps=True)
    parquet_schema = pq.ParquetFile(filename).schema
    for i in range(3):
...
def test_read_single_row_group():
    ...
    buf = io.BytesIO()
    _write_table(
        a_table, buf, row_group_size=N / K, compression="snappy", version="2.0"
    )
...
def test_cutoff_exclusive_integer(tempdir):
    ...
    df = pd.DataFrame(
        {"index": np.arange(N), "integers": np.array(integer_keys, dtype="i4")},
    ...
    dataset = pq.ParquetDataset(
        base_path, filesystem=fs, filters=[("integers", "<", 4), ("integers", ">", 1)]
    )
    table = dataset.read()
...
@pytest.mark.pandas
@pytest.mark.xfail(
    raises=TypeError, reason="Loss of type information in creation of categoricals."
)
def test_cutoff_exclusive_datetime(tempdir):
    ...
    df = pd.DataFrame(
        {"index": np.arange(N), "dates": np.array(date_keys, dtype="datetime64")},
        columns=["index", "dates"],
    )
...
def test_inclusive_integer(tempdir):
    ...
    dataset = pq.ParquetDataset(
        base_path, filesystem=fs, filters=[("integers", "<=", 3), ("integers", ">=", 2)]
    )
    table = dataset.read()
    result_df = table.to_pandas().sort_values(by="index").reset_index(drop=True)
...
def test_invalid_pred_op(tempdir):
    ...
    with pytest.raises(ValueError):
        pq.ParquetDataset(base_path, filesystem=fs, filters=[("integers", "in", set())])
...
def test_filters_read_table(tempdir):
    ...
    table = pq.read_table(base_path, filesystem=fs, filters=[("integers", "<", 3)])
...
def test_noncoerced_nanoseconds_written_without_exception(tempdir):
    ...
    df = pd.DataFrame(
        {"x": range(n)},
        index=pd.DatetimeIndex(start="2017-01-01", freq="1n", periods=n),
    )
...
def _test_write_to_dataset_with_partitions(
    ...
    # partitioned dataset
    dataset = pq.ParquetDataset(base_path, filesystem=filesystem, validate_schema=True)
...
def test_write_to_dataset_with_partitions_and_custom_filenames(tempdir):
    ...
    pq.write_to_dataset(output_table, path, partition_by, partition_filename_callback)
...
def test_parquet_nested_convenience(tempdir):
    # ARROW-1684
    df = pd.DataFrame(
        {"a": [[1, 2, 3], None, [4, 5], []], "b": [[1.0], None, None, [6.0, 7.0]]}
    )
...
def test_parquet_writer_with_caller_provided_filesystem():
    ...
    with pq.ParquetWriter(fname, table.schema, filesystem=fs, version="2.0") as writer:
        writer.write_table(table)
...
def test_write_nested_zero_length_array_chunk_failure():
    ...
    my_batches = [
        pa.RecordBatch.from_arrays(batch, pa.schema(cols)) for batch in my_arrays
    ]
...
def test_direct_read_dictionary():
    ...
    data = [[tm.rands(10) for i in range(nunique)] * repeats]
...
def test_direct_read_dictionary_subfield():
    ...
    pq.write_table(table, bio)
    contents = bio.getvalue()
    result = pq.read_table(pa.BufferReader(contents), read_dictionary=["f0.list.item"])
...
def test_dataset_metadata(tempdir):
    ...
    df = pd.DataFrame(
        {"one": [1, 2, 3], "two": [-1, -2, -3], "three": [[1, 2], [2, 3], [3, 4]]}
    )
    table = pa.Table.from_pandas(df)
...
def test_parquet_file_too_small(tempdir):
    ...
    with pytest.raises(pa.ArrowIOError, match="size is 0 bytes"):
        with open(path, "wb") as f:
            pass
        pq.read_table(path)

    with pytest.raises(pa.ArrowIOError, match="size is 4 bytes"):
        with open(path, "wb") as f:
            f.write(b"ffff")
        pq.read_table(path)
