...
def assert_equal(obj1, obj2):
    ...
    if module_numpy:
        ...
        for i in range(len(obj1)):
            assert_equal(obj1[i], obj2[i])
    elif pa.lib.is_named_tuple(type(obj1)) or pa.lib.is_named_tuple(type(obj2)):
...
def large_memory_map(tmpdir_factory, size=100 * 1024 * 1024):
    path = tmpdir_factory.mktemp("data").join("pyarrow-serialization-tmp-file").strpath

    # Create a large memory mapped file
    with open(path, "wb") as f:
        f.write(np.random.randint(0, 256, size=size).astype("u1").tobytes()[:size])
    return path
...
def test_datetime_serialization(large_buffer):
    data = [
        ...
        # Some random date
        datetime.datetime(year=1911, month=6, day=3, hour=4, minute=55, second=44),
        # End of WWI
        ...
        # Another random date
        datetime.datetime(year=2011, month=6, day=3, hour=4, minute=0, second=3),
        # Another random date
        datetime.datetime(year=1970, month=1, day=3, hour=4, minute=0, second=0),
    ]
    for d in data:
...
def test_torch_serialization(large_buffer):
    ...
    for t in ["float32", "float64", "uint8", "int16", "int32", "int64"]:
        ...
        serialization_roundtrip(obj, large_buffer, context=serialization_context)
...
@pytest.mark.skipif(
    not torch or not torch.cuda.is_available(), reason="requires pytorch with CUDA"
)
def test_torch_cuda():
...
def test_pyarrow_objects_serialization(large_buffer):
    ...
    pyarrow_objects = [
        pa.array([1, 2, 3, 4]),
        pa.array(["1", u"never U+1F631", "", u"233 * U+1F600"]),
        pa.array([1, None, 2, 3]),
...
def test_fallback_to_subclasses():
    ...
    reconstructed_object = serialized_object.deserialize(serialization_context)
...
def test_serialize_to_components_invalid_cases():
    ...
    components = {"num_tensors": 0, "num_ndarrays": 0, "num_buffers": 1, "data": [buf]}
...
def test_deserialize_buffer_in_different_process():
    ...
    subprocess.check_call([sys.executable, python_file, f.name], env=subprocess_env)
