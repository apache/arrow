...
cdef class SparseSplitCOOTensor(_Weakrefable):
    """
    A sparse split-COO tensor.
    """

    def __init__(self):
        raise TypeError(
            "Do not call SparseSplitCOOTensor's constructor directly, "
            "use one of the `pyarrow.SparseSplitCOOTensor.from_*` "
            "functions instead.")
    ...
    def __repr__(self):
        return """<pyarrow.SparseSplitCOOTensor>
type: {0.type}
shape: {0.shape}""".format(self)
    ...
    def from_dense_numpy(cls, obj, dim_names=None):
        """
        Convert numpy.ndarray to arrow::SparseSplitCOOTensor
        """
        return cls.from_tensor(Tensor.from_numpy(obj, dim_names=dim_names))

    @staticmethod
    def from_numpy(data, indices, shape, dim_names=None):
        """
        Create arrow::SparseSplitCOOTensor from numpy.ndarrays
        """
        cdef shared_ptr[CSparseSplitCOOTensor] sp_sparse_tensor
        cdef vector[int64_t] c_shape
        cdef vector[c_string] c_dim_names
        ...
        for dim, index in enumerate(indices):
            if index.ndim != 1:
                raise ValueError("Expected 1-dimensional array for "
                                 "each index of SparseSplitCOOTensor")

        check_status(NdarraysToSparseSplitCOOTensor(
            c_default_memory_pool(), data, indices, c_shape, c_dim_names,
            &sp_sparse_tensor))
        return pyarrow_wrap_sparse_split_coo_tensor(sp_sparse_tensor)

    @staticmethod
    def from_scipy(obj, dim_names=None):
        """
        Convert scipy.sparse.coo_matrix to arrow::SparseSplitCOOTensor
        """
        import scipy.sparse
        if not isinstance(obj, scipy.sparse.coo_matrix):
            raise TypeError(
        ...
        indices = [obj.row, obj.col]

        check_status(NdarraysToSparseSplitCOOTensor(
            c_default_memory_pool(), obj.data, indices, c_shape, c_dim_names,
            &sp_sparse_tensor))
        return pyarrow_wrap_sparse_split_coo_tensor(sp_sparse_tensor)
    ...
    def from_pydata_sparse(obj, dim_names=None):
        """
        Convert pydata/sparse.COO to arrow::SparseSplitCOOTensor.
        """
        import sparse
        if not isinstance(obj, sparse.COO):
            raise TypeError(
    ...
    def from_tensor(obj):
        """
        Convert arrow::Tensor to arrow::SparseSplitCOOTensor.
        """
        cdef shared_ptr[CSparseSplitCOOTensor] sp_sparse_tensor
        cdef shared_ptr[CTensor] sp_ctensor = pyarrow_unwrap_tensor(obj)

        with nogil:
            check_status(TensorToSparseSplitCOOTensor(
                sp_ctensor, &sp_sparse_tensor))

        return pyarrow_wrap_sparse_split_coo_tensor(sp_sparse_tensor)

    def to_numpy(self):
        """
        Convert arrow::SparseSplitCOOTensor to numpy.ndarrays with zero copy.
        """
        cdef PyObject* out_data
        cdef PyObject* out_indices

        check_status(SparseSplitCOOTensorToNdarray(self.sp_sparse_tensor, self,
                                                   &out_data, &out_indices))
        return PyObject_to_object(out_data), PyObject_to_object(out_indices)

    def to_scipy(self):
        """
        Convert arrow::SparseSplitCOOTensor to scipy.sparse.coo_matrix.
        """
        from scipy.sparse import coo_matrix
        cdef PyObject* out_data
        cdef PyObject* out_indices
        ...
        result = coo_matrix((data[:, 0], (indices[0], indices[1])),
                            shape=self.shape)
        return result

    def to_pydata_sparse(self):
        """
        Convert arrow::SparseSplitCOOTensor to pydata/sparse.COO.
        """
        from sparse import COO
        cdef PyObject* out_data
        cdef PyObject* out_coords
    ...
    def to_tensor(self):
        """
        Convert arrow::SparseSplitCOOTensor to arrow::Tensor.
        """
    ...
    def equals(self, SparseSplitCOOTensor other):
        """
        Return true if sparse tensors contains exactly equal data.
        """
        return self.stp.Equals(deref(other.stp))
    ...
    def __eq__(self, other):
        if isinstance(other, SparseSplitCOOTensor):
            return self.equals(other)
        else:
            return NotImplemented
