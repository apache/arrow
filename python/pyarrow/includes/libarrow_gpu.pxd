...
cdef extern from "arrow/gpu/cuda_api.h" namespace "arrow::gpu" nogil:
    ...
    cdef cppclass CCudaDeviceManager" arrow::gpu::CudaDeviceManager":
        ...
        CStatus CreateNewContext(int gpu_number, shared_ptr[CCudaContext]* ctx)
        CStatus AllocateHost(int64_t nbytes, shared_ptr[CCudaHostBuffer]* buffer)
    ...
    cdef cppclass CCudaIpcMemHandle" arrow::gpu::CudaIpcMemHandle":
        @staticmethod
        CStatus FromBuffer(const void* opaque_handle, shared_ptr[CCudaIpcMemHandle]* handle)
    ...
    cdef cppclass CCudaBuffer" arrow::gpu::CudaBuffer"(CBuffer):
        ...
        CCudaBuffer(const shared_ptr[CCudaBuffer]& parent, const int64_t offset, const int64_t size)
    ...
    cdef cppclass CCudaBufferReader" arrow::gpu::CudaBufferReader"(CBufferReader):
    ...
    cdef cppclass CCudaBufferWriter" arrow::gpu::CudaBufferWriter"(WriteableFile):
