...
from pyarrow.compat import builtin_pickle, PY2, zip_longest, Sequence, u_utf8  # noqa
...
def get_column_metadata(column, name, arrow_type, field_name):
    ...
    assert field_name is None or isinstance(field_name, six.string_types), str(
        type(field_name)
    )
    return {
        "name": name,
...
def construct_metadata(
    df, column_names, index_levels, index_descriptors, preserve_index, types
):
    ...
    column_metadata = []
    for col_name, sanitized_name, arrow_type in zip(df.columns, column_names, df_types):
    ...
    if preserve_index is not False:
        for level, arrow_type, descriptor in zip(
            ...
            if isinstance(descriptor, dict):
                ...
                continue
            metadata = get_column_metadata(
                level, name=level.name, arrow_type=arrow_type, field_name=descriptor
            )
...
def dataframe_to_types(df, preserve_index, columns=None):
    ...
    metadata = construct_metadata(
        df, column_names, index_columns, index_descriptors, preserve_index, types
    )
...
def dataframe_to_arrays(
    ...
    if nthreads == 1:
        ...
        with futures.ThreadPoolExecutor(nthreads) as executor:
            arrays = list(
                executor.map(convert_column, columns_to_convert, convert_fields)
            )
...
def dataframe_to_serialized_dict(frame):
    ...
    for block in block_manager.blocks:
        ...
        if isinstance(block, _int.DatetimeTZBlock):
            ...
            block_data.update(dictionary=values.categories, ordered=values.ordered)
            values = values.codes
...
def serialized_dict_to_dataframe(data):
    ...
    reconstructed_blocks = [_reconstruct_block(block) for block in data["blocks"]]
...
def _reconstruct_block(item):
    ...
    if "dictionary" in item:
        ...
        block = _int.make_block(cat, placement=placement, klass=_int.CategoricalBlock)
    elif "timezone" in item:
        dtype = make_datetimetz(item["timezone"])
        block = _int.make_block(
            block_arr, placement=placement, klass=_int.DatetimeTZBlock, dtype=dtype
        )
    elif "object" in item:
        block = _int.make_block(
            builtin_pickle.loads(block_arr), placement=placement, klass=_int.ObjectBlock
        )
    else:
...
def table_to_blockmanager(options, table, categories=None, ignore_metadata=False):
    ...
    if not ignore_metadata and pandas_metadata is not None:
        ...
        table, index = _reconstruct_index(table, index_descriptors, all_columns)
    else:
        index = _pandas_api.pd.RangeIndex(table.num_rows)

    _check_data_column_metadata_consistency(all_columns)
    blocks = _table_to_blocks(options, table, pa.default_memory_pool(), categories)
...
def _reconstruct_index(table, index_descriptors, all_columns):
    ...
    for descr in index_descriptors:
        if isinstance(descr, six.string_types):
            ...
            raise ValueError("Unrecognized index kind: {0}".format(descr["kind"]))
...
def _extract_index_level(table, result_table, field_name, field_name_to_metadata):
    ...
    if isinstance(col.type, pa.lib.TimestampType):
        index_level = pd.Series(values).dt.tz_localize("utc").dt.tz_convert(col.type.tz)
    else:
...
def _table_to_blocks(options, block_table, memory_pool, categories):
    ...
    result = pa.lib.table_to_blocks(options, block_table, memory_pool, categories)
