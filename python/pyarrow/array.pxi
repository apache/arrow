...
cdef class VariableShapeTensorArray(ExtensionArray):
    ...
    def from_row_splits(values, row_splits, shapes, dim_names=None, permutation=None,
        ...
        if values_arr.null_count > 0:
            raise ValueError("values must not contain nulls")

        if len(values_arr) == 0 and value_type is None and values_arr.type == null():
        ...
        # Materialize shapes early if needed for ndim inference, and keep
        # the materialized list so we don't re-read an exhausted iterator.
        shapes_materialized = None
        if ndim is None:
            if isinstance(shapes, Array):
                ...
                shapes_materialized = list(shapes)
                if len(shapes_materialized) == 0:
                    raise ValueError("ndim must be provided when shapes is empty")
                if shapes_materialized[0] is None:
                    raise ValueError("shapes[0] must not be null")
                ndim = len(shapes_materialized[0])
        elif ndim < 0:
            raise ValueError("ndim must be non-negative")
        ...
        shape_type = list_(int32(), list_size=ndim)
        shape_arr = asarray(
            shapes_materialized if shapes_materialized is not None else shapes,
            type=shape_type)
        ...
        for i in range(len(splits) - 1):
            if shape_rows[i] is None:
                raise ValueError(f"shapes[{i}] must not be null")
            expected_size = 1
            for j, dim in enumerate(shape_rows[i]):
                if dim is None:
                    raise ValueError(
                        f"shapes[{i}][{j}] must not be null")
                if dim < 0:
                    raise ValueError(
                        f"shapes[{i}][{j}] must be non-negative, got {dim}")
                expected_size *= dim
