...
def generate_chunks(total_size, nchunks, ncols, dtype=np.dtype("int64")):
    ...
    def make_column(col, chunk):
        return np.frombuffer(
            common.get_random_bytes(rowsize, seed=col + 997 * chunk)
        ).view(dtype)

    return [
        pd.DataFrame({"c" + str(col): make_column(col, chunk) for col in range(ncols)})
...
class StreamReader(object):
    ...
    def setup(self, chunk_size):
        ...
        chunks = generate_chunks(
            self.total_size, nchunks=self.total_size // chunk_size, ncols=self.ncols
        )
