...
./bootstrap.sh
./b2 tools/bcp
./dist/bin/bcp --namespace=arrow_boost --namespace-alias filesystem date_time system thread regex build algorithm locale format variant /arrow_boost
popd

pushd /arrow_boost
ls -l
./bootstrap.sh
./bjam dll-path="'\$ORIGIN/'" cxxflags='-std=c++11 -fPIC' cflags=-fPIC linkflags="-std=c++11" variant=release link=shared --prefix=/arrow_boost_dist --with-filesystem --with-date_time --with-system --with-thread --with-regex install
popd
