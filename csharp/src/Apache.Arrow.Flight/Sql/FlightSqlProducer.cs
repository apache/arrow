...
public abstract class FlightSqlProducer
{
    ...
    public static readonly Schema SqlInfoSchema = new(new List<Field>
    {
        ...
        //TODO: once we have union serialization in Arrow Flight for .Net we need to add these fields
    ...
    public static Schema GetTableSchema(bool includeTableSchemaField)
    {
        ...
        var fields = TableSchema_.Fields.Values.ToList();
    ...
    public List<FlightActionType> ListActions()
    {
        return FlightSqlUtils.FlightSqlActions;
    }
    ...
    public class PreparedStatementResult
    {
        private readonly string statementHandleId;
        private readonly Schema datasetSchema;
        private readonly Schema? parameterSchema;

        public PreparedStatementResult(string statementHandleId, Schema datasetSchema, Schema? parameterSchema = null)
        {
            this.statementHandleId = statementHandleId;
            this.datasetSchema = datasetSchema;
            this.parameterSchema = parameterSchema;
        }

        internal ActionCreatePreparedStatementResult ToProtocol()
        {
            return new ActionCreatePreparedStatementResult
            {
                DatasetSchema = datasetSchema.ToByteString(),
                ParameterSchema = parameterSchema?.ToByteString() ?? ByteString.Empty,
                PreparedStatementHandle = ByteString.CopyFromUtf8(statementHandleId)
            };
        }
    }
}
