...
namespace Apache.Arrow.Acero
{
    public class LiteralExpression : Expression
    {
        ...
        {
            var dataPtr = (nint)StringUtil.ToCStringUtf8(literal);
            var bufferPtr = CLib.garrow_buffer_new(dataPtr, literal.Length);
            var scalarPtr = CLib.garrow_string_scalar_new(bufferPtr);
            var datumPtr = (nint)CLib.garrow_scalar_datum_new((nint)scalarPtr);

            _ptr = (nint)CLib.garrow_literal_expression_new((GArrowDatum*)datumPtr);
        }
