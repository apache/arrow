...
DECODERS = ["seq_scal", "seq_simd", "intlv", "fl_unpk", "fl_tpos"]
POINTS = ["L1", "L2", "DRAM"]
...
for p in POINTS:
    ...
    G[p] = {a: gm([float(r[a]) for r in sel]) for a in DECODERS}
    G[p]["n"] = len(sel)
...
w('<p>Geometric mean across %d real columns, GiB/s measured on <em>output</em> bytes (4 bytes per\n'
  'value decoded), higher is better. Ratios are against <code>seq_simd</code>, which is what\n'
  'Parquet decodes at today.</p>\n' % NDS)
w('<div class="tablewrap">\n<table>\n')
w('<caption>Table 2. Geometric mean over %d columns. The <code>fl_unpk/intlv</code> column is the\n'
  'self-check described in Table 1: two decoders running identical kernel code on identically-shaped\n'
...
for a in DECODERS: w('<th class="num">%s</th>' % a)
...
for p in POINTS:
    g = G[p]
    w('<tr><td>%s</td>' % LBL[p])
    for a in DECODERS: w('<td class="num">%.1f</td>' % g[a])
...
w('<p><b>First, the DRAM row.</b> At a 32 MiB working set every layout converges: %.1f, %.1f, %.1f,\n'
  '%.1f, %.1f GiB/s across the five, a spread of %.0f%% between the fastest and slowest of the\n'
  'four SIMD decoders. The kernel is no longer instruction-bound, it is waiting on memory, and no\n'
  ...
  % (G["DRAM"]["seq_scal"], G["DRAM"]["seq_simd"], G["DRAM"]["intlv"], G["DRAM"]["fl_unpk"],
     G["DRAM"]["fl_tpos"],
     100 * (max(G["DRAM"][a] for a in DECODERS[1:]) / min(G["DRAM"][a] for a in DECODERS[1:]) - 1)))
...
w('<caption>Table 3. Per-column consistency of the layout effect, raw. "Wins" counts columns where\n'
  'the decoder is strictly faster than <code>seq_simd</code>.</caption>\n')
...
w('<p>Every measurement, unaggregated. <code>W</code> is the mean per-block bit width the encoder\n'
  ...
  'of the variation between rows. Speeds are GiB/s on output bytes; the best of the four SIMD decoders in\n'
...
for d in order:
    ...
    for k, p in enumerate(POINTS):
        ...
        w('<td>%s</td>' % p)
        vals = {a: float(r[a]) for a in DECODERS}
        best = max(vals[a] for a in DECODERS[1:])
        for a in DECODERS:
            v = vals[a]
            cell = '%.1f' % v
