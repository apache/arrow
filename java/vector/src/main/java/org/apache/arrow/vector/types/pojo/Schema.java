...
public class Schema {
   ...
   *
   * @param unsafe a ignored argument. Its only purpose is to prevent using the constructor by
   ...
   * #deserializeMessage() to rebuild the Schema.}
   */
  public byte[] serializeAsMessage() {
    ByteArrayOutputStream out = new ByteArrayOutputStream();
    try (WriteChannel channel = new WriteChannel(Channels.newChannel(out))) {
      long size = MessageSerializer.serialize(new WriteChannel(Channels.newChannel(out)), this);
