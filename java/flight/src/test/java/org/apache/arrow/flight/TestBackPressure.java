...
public class TestBackPressure {
  ...
  private final static int BATCH_SIZE = 4095;
  ...
  public void ensureIndependentSteams() throws Exception {
    ...
    try(
      ...
      FlightStream fs1 = client.getStream(client.getInfo(TestPerf.getPerfFlightDescriptor(110l * BATCH_SIZE, BATCH_SIZE)).getEndpoints().get(0).getTicket());
      consume(fs1, 10);

      // stop consuming fs1 but make sure we can consume a large amount of fs2.
      FlightStream fs2 = client.getStream(client.getInfo(TestPerf.getPerfFlightDescriptor(200l * BATCH_SIZE, BATCH_SIZE)).getEndpoints().get(0).getTicket());
      consume(fs2, 100);
  ...
  public void ensureWaitUntilProceed() throws Exception {
    ...
    try(BufferAllocator allocator = new RootAllocator(Long.MAX_VALUE)){
      ...
      Assert.assertTrue(String.format("Expected a sleep of at least %dms but only slept for %d", expected, sleepTime.get()), sleepTime.get() > expected);
