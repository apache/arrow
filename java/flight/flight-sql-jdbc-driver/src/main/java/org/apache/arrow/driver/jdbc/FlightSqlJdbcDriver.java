...
/**
 * JDBC driver for querying data from an Apache Arrow Flight SQL server.
 */
public class FlightSqlJdbcDriver extends UnregisteredDriver {

  // Accept both URI schemes, but prefer the one that explicitly mentions Flight SQL.
  ...
  static {
    ...
    new FlightSqlJdbcDriver().register();
  }

  @Override
  public FlightSqlConnection connect(final String url, final Properties info)
      throws SQLException {
  ...
  protected String getFactoryClassName(final JdbcVersion jdbcVersion) {
    return FlightSqlJdbcFactory.class.getName();
  }
  ...
  public Meta createMeta(final AvaticaConnection connection) {
    return new FlightSqlMetaImpl(connection);
  }
  ...
  Map<Object, Object> getUrlsArgs(String url)
    ...
    if (!Objects.equals(uri.getScheme(), "arrow-flight") &&
      ...
      throw new SQLException("URL Scheme must be 'arrow-flight-sql'. Expected format: " +
          CONNECTION_STRING_EXPECTED);
    }
