...
/**
 * {@link ResultSet} implementation for Arrow Flight SQL used to access the results of multiple {@link FlightStream}
 * objects.
 */
public final class FlightStreamResultSet extends VectorSchemaRootResultSet {

  private final FlightSqlConnection connection;
  ...
  FlightStreamResultSet(final AvaticaStatement statement,
    ...
    this.connection = (FlightSqlConnection) statement.connection;
  }

  FlightStreamResultSet(final FlightSqlConnection connection,
                        final QueryState state,
                        final Meta.Signature signature,
                        final ResultSetMetaData resultSetMetaData,
                        final TimeZone timeZone,
   ...
   */
  static FlightStreamResultSet fromFlightInfo(
    ...
    final FlightStreamResultSet resultSet =
        new FlightStreamResultSet(connection, state, signature, resultSetMetaData,
            timeZone, null);
  ...
  protected AvaticaResultSet execute() throws SQLException {
    final FlightInfo flightInfo = ((FlightInfoStatement) statement).executeFlightInfoQuery();
