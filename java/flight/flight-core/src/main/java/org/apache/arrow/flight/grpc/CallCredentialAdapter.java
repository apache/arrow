...
public class CallCredentialAdapter extends CallCredentials {
  ...
  public CallCredentialAdapter(CredentialWriter credentialWriter) {
    this.credentialWriter = credentialWriter;
  }

  @Override
  public void applyRequestMetadata(RequestInfo requestInfo, Executor executor, MetadataApplier metadataApplier) {
    final CallInfo callInfo = new CallInfo(FlightMethod.fromProtocol(
        requestInfo.getMethodDescriptor().getFullMethodName()));
    executor.execute(() ->
    {
      final Metadata headers = new Metadata();
      credentialWriter.writeCredentials(callInfo, new MetadataAdapter(headers));
