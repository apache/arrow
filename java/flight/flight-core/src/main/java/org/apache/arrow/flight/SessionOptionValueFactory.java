...
public class SessionOptionValueFactory {
  ...
  /** Only use on Proto-generated ByteStrings that cannot fail parsing. */
  private static ByteStringToStringValueToStringOrNull(ByteString bs) {
    try {
      return StringValue.parseFrom(bs).getValue();
    ...
    switch (proto.getOptionValueCase()) {
      ...
      case STRING_LIST_VALUE:
        ...
        return new SessionOptionValueStringList(proto.getStringListValue().getValuesList().asByteStringList().stream()
          .map(ByteStringToStringValueToStringOrNull).toArray(String[]::new));
      default:
        // Unreachable
