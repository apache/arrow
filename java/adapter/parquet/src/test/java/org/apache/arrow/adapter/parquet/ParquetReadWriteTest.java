...
public class ParquetReadWriteTest {
  ...
  public void testParquetReadWrite() throws Exception {
    ...
    ParquetReaderJniWrapper readerHandler = new ParquetReaderJniWrapper(allocator);
    ParquetWriterJniWrapper writerHandler = new ParquetWriterJniWrapper();
    ...
    ParquetWriter writer = new ParquetWriter(writerHandler, path, schema);
    writer.writeNext(batch);
    releaseArrowRecordBatch(batch);
    writer.close();

    ParquetReader reader =
        new ParquetReader(readerHandler, path, columnIndices, rowGroupIndices, 16);
