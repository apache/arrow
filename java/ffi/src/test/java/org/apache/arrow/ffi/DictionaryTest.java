...
public class DictionaryTest {
  ...
  void roundtrip(FieldVector vector, DictionaryProvider provider, Class<?> clazz) {
    ...
    try (ArrowSchema consumerArrowSchema = ArrowSchema.allocateNew(allocator);
      ...
      try (FFIDictionaryProvider ffiDictionaryProvider = new FFIDictionaryProvider();
          FieldVector imported = FFI.importVector(allocator, consumerArrowArray, consumerArrowSchema,
              ffiDictionaryProvider);) {
        ...
        for (long id : ffiDictionaryProvider.getDictionaryIds()) {
          ValueVector exportedDictionaryVector = provider.lookup(id).getVector();
          ValueVector importedDictionaryVector = ffiDictionaryProvider.lookup(id).getVector();
  ...
  public void testRoundtripMultipleBatches() throws IOException {
    try (ArrowStreamReader reader = createMultiBatchReader();
      ...
      FFI.exportSchema(allocator, reader.getVectorSchemaRoot().getSchema(), reader, consumerArrowSchema);
      // Consumer loads it as an empty vector schema root
      try (FFIDictionaryProvider consumerDictionaryProvider = new FFIDictionaryProvider();
          VectorSchemaRoot consumerRoot = FFI.importVectorSchemaRoot(allocator, consumerArrowSchema,
              consumerDictionaryProvider)) {
        do {
          try (ArrowArray consumerArray = ArrowArray.allocateNew(allocator)) {
            // Producer exports next data
            FFI.exportVectorSchemaRoot(allocator, reader.getVectorSchemaRoot(), reader, consumerArray);
            // Consumer loads next data
            FFI.importIntoVectorSchemaRoot(allocator, consumerArray, consumerRoot, consumerDictionaryProvider);
