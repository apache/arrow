...
The `Apache ORC <http://parquet.apache.org/>`_ project provides a
...
.. ipython:: python
   ...
   t = pq.read_table('example_noindex.orc')
   t.to_pandas()
...
Inspecting the ORC File Metadata
------------------------------------

The ``FileMetaData`` of a Parquet file can be accessed through
:class:`~.ORCFile` as shown above:

.. ipython:: python

   orc_file = pq.ORCFile('example.orc')
