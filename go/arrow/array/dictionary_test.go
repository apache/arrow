...
func TestDictionaryUnifierChunkedArrayNested(t *testing.T) {
	...
	expectedIndices, _, _ := array.FromJSON(mem, arrow.PrimitiveTypes.Int16, strings.NewReader(`[0, 1, 1]`))
	defer expectedIndices.Release()
	checkDictionaryArray(t, unified1.ListValues(), expectedDict, expectedIndices)

	unified2 := unified.Chunk(1).(*array.List)
	assert.Equal(t, []int32{0, 0, 3}, unified2.Offsets())
	expectedIndices, _, _ = array.FromJSON(mem, arrow.PrimitiveTypes.Int16, strings.NewReader(`[2, 1, 2]`))
	defer expectedIndices.Release()
	checkDictionaryArray(t, unified2.ListValues(), expectedDict, expectedIndices)
}
