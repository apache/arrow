...
func RunEndEncodedOf(runEnds, values DataType) *RunEndEncodedType {
	return &RunEndEncodedType{ends: runEnds, values: values, ValueNullable: true}
}
...
func (t *RunEndEncodedType) String() string {
	return t.Name() + "<run_ends: " + t.ends.String() + ", values: " + t.values.String() + ">"
}

func (t *RunEndEncodedType) Fingerprint() string {
	return typeFingerprint(t) + "{" + t.ends.Fingerprint() + ";" + t.values.Fingerprint() + ";}"
}
