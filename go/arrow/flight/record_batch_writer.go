...
var (
	ErrDataStreamClosed error = errors.New("the server unexpectedly closed the FlightData stream")
)
...
func (f *flightPayloadWriter) WritePayload(payload ipc.Payload) error {
	...
	if err == io.EOF {
		...
		// We return a more informative error to clients that hides this gRPC idiosyncrasy.
		// The error can be retrieved subsequently by calling RecvMsg on the stream
