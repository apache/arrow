...
func ExecCopyArray(ctx *KernelCtx, batch *ExecSpan, out *ExecResult) error {
...
func ExecComputedBitmap(ctx *KernelCtx, batch *ExecSpan, out *ExecResult) error {
...
func ExecNoPreallocatedData(ctx *KernelCtx, batch *ExecSpan, out *ExecResult) error {
...
func ExecNoPreallocatedAnything(ctx *KernelCtx, batch *ExecSpan, out *ExecResult) error {
...
func InitStateful(_ *KernelCtx, args KernelInitArgs) (KernelState, error) {
	value := args.Options.(*ExampleOptions).Value
	return &ExampleState{Value: value}, nil
}

func ExecStateful(ctx *KernelCtx, batch *ExecSpan, out *ExecResult) error {
	state := ctx.State.(*ExampleState)
	multiplier := state.Value.(*scalar.Int32).Value

	arg0 := batch.Values[0].Array
	arg0Data := GetSpanValues[int32](&arg0, 1)
	dst := GetSpanValues[int32](out, 1)
...
func ExecAddInt32(ctx *KernelCtx, batch *ExecSpan, out *ExecResult) error {
	left := GetSpanValues[int32](&batch.Values[0].Array, 1)
	right := GetSpanValues[int32](&batch.Values[1].Array, 1)
	outValues := GetSpanValues[int32](out, 1)
...
func (c *CallScalarFuncSuite) addCopyFuncs() {
	...
	for _, t := range types {
		c.NoError(fn.AddNewKernel([]InputType{NewExactInput(t)},
			NewOutputType(t), ExecCopyArray, nil))
	}
	...
	kernel := NewScalarKernel([]InputType{NewExactInput(arrow.PrimitiveTypes.Uint8)},
		NewOutputType(arrow.PrimitiveTypes.Uint8), ExecComputedBitmap, nil)
	kernel.NullHandling = NullComputedPrealloc
...
func (c *CallScalarFuncSuite) addNoPreallocFuncs() {
	...
	kernel := NewScalarKernel(
		[]InputType{NewExactInput(arrow.PrimitiveTypes.Uint8)},
		NewOutputType(arrow.PrimitiveTypes.Uint8),
		ExecNoPreallocatedData, nil)
	kernel.MemAlloc = MemNoPrealloc
	c.NoError(f1.AddKernel(*kernel))

	kernel.exec = ExecNoPreallocatedAnything
	kernel.NullHandling = NullComputedNoPrealloc
...
func (c *CallScalarFuncSuite) addStatefulFunc() {
	...
	c.NoError(fn.AddNewKernel([]InputType{NewExactInput(arrow.PrimitiveTypes.Int32)},
		NewOutputType(arrow.PrimitiveTypes.Int32), ExecStateful, InitStateful))
...
func (c *CallScalarFuncSuite) addScalarFunc() {
	...
	c.NoError(fn.AddNewKernel([]InputType{
		NewExactInput(arrow.PrimitiveTypes.Int32),
		NewExactInput(arrow.PrimitiveTypes.Int32)},
		NewOutputType(arrow.PrimitiveTypes.Int32), ExecAddInt32, nil))
	c.True(registry.AddFunction(fn, false))
}
...
func (c *CallScalarFuncSuite) TestArgumentValidation() {
	...
	c.Run("too many args", func() {
		args := []Datum{d1, d1}
		_, err := CallFunction(c.execCtx, "test_copy", nil, args...)
		c.ErrorIs(err, arrow.ErrInvalid)
	})

	c.Run("too few args", func() {
		_, err := CallFunction(c.execCtx, "test_copy", nil)
		c.ErrorIs(err, arrow.ErrInvalid)
	})

	d1Scalar := NewDatum(int32(5))
	result, err := CallFunction(c.execCtx, "test_copy", nil, d1)
	c.NoError(err)
	result.Release()
	result, err = CallFunction(c.execCtx, "test_copy", nil, d1Scalar)
	c.NoError(err)
	result.Release()
}
	...
	for _, funcName := range funcNames {
		c.Run(funcName, func() {
			...
			c.Run("single output default", func() {
				result, err := CallFunction(c.execCtx, funcName, nil, &ArrayDatum{arr.Data()})
				c.NoError(err)
			...
			c.Run("chunked", func() {
				...
				result, err := CallFunction(c.execCtx, funcName, nil, &ChunkedDatum{carr})
				c.NoError(err)
				defer result.Release()
...
func (c *CallScalarFuncSuite) TestBasicNonStandardCases() {
	...
	for _, funcName := range []string{"test_nopre_data", "test_nopre_validity_or_data"} {
		c.Run("funcName", func() {
			c.resetCtx()
			c.Run("single output default", func() {
				result, err := CallFunction(c.execCtx, funcName, nil, args...)
				c.NoError(err)
				defer result.Release()
				c.Equal(KindArray, result.Kind())
				c.assertDatumEqual(arr, result)
...
func (c *CallScalarFuncSuite) TestStatefulKernel() {
	...
	result, err := CallFunction(c.execCtx, "test_stateful", options, &ArrayDatum{input.Data()})
	c.NoError(err)
	defer result.Release()
...
func (c *CallScalarFuncSuite) TestScalarFunction() {
	...
	result, err := CallFunction(c.execCtx, "test_scalar_add_int32", nil, args...)
	c.NoError(err)
	defer result.Release()
