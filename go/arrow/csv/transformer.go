...
func (w *Writer) transformColToStringArr(typ arrow.DataType, col arrow.Array, stringReplacer func(string)string) []string {
	...
	case *arrow.StringType:
		...
		for i := 0; i < arr.Len(); i++ {
			if arr.IsValid(i) {
				res[i] = stringReplacer(arr.Value(i))
			} else {
				res[i] = w.nullValue
			}
		}
	...
	case *arrow.ListType:
		...
		for i := 0; i < arr.Len(); i++ {
			if arr.IsValid(i) {
				...
				writer.Write(w.transformColToStringArr(list.DataType(), list, stringReplacer))
				writer.Flush()
				b.Truncate(b.Len() - 1)
				b.Write([]byte{'}'})
				res[i] = b.String()
