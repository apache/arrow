...
// 	nRows := 200
// 	nCols := 14
// 	require.Equal(b, 1, fileReader.NumRowGroups())
// 	rgr := fileReader.RowGroup(0)
// 	require.EqualValues(b, nRows, rgr.NumRows())
// 	require.EqualValues(b, nCols, rgr.NumColumns())

// 	// Read ByteStreamSplit-encoded column
// 	rdrByteStreamSplit, err := rgr.Column(7)
// 	require.NoError(b, err)

// 	// Decoded values match
// 	r, ok := rdrByteStreamSplit.(*file.Int32ColumnChunkReader)
// 	require.True(b, ok)
...
// 	// b.SetBytes(int64(len(values) * arraySize))
// 	for n := 0; n < b.N; n++ {
// 		total, _, _ := r.ReadBatch(int64(nRows), values, nil, nil)
// 	}
