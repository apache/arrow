...
func OpenParquetFile(filename string, opts ...ReadOption) (*Reader, error) {
	ctx := context.Background()
	bucket := "writo-test-bucket"

	// var source source.ParquetFile
	source, err := s3.NewS3FileReader(ctx, bucket, filename)
	if err != nil {
		return nil, err
	}
	src := S3file { source }
	return NewParquetReader(src, opts...)
}
