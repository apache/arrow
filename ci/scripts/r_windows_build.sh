...
if [ -d mingw64/lib/ ]; then
  ...
  cp $MSYS_LIB_DIR/mingw64/lib/lib{zstd,lz4,brotli*,bzip2,crypto,curl,ss*,utf8proc,re2,aws*}.a $DST_DIR/lib/x64
fi
...
if [ -d ucrt64/lib/ ]; then
  ...
  cp $MSYS_LIB_DIR/ucrt64/lib/lib{thrift,snappy,zstd,lz4,brotli*,bzip2,crypto,curl,ss*,utf8proc,re2,aws*}.a $DST_DIR/lib/x64-ucrt
fi
