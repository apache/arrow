...
if [[ "$(uname -s)" == "Linux" ]] && [[ "$(uname -m)" == "s390x" ]]; then
  ...
  mvn_install="mvn clean install:install-file -DrerunGoals"
  wget="wget"
...
if [ "${BUILD_DOCS_JAVA}" == "ON" ]; then
  ...
  ${mvn} -Dcheckstyle.skip=true -Dhttp.keepAlive=false -Dmaven.wagon.http.pool=false -DrerunGoals clean install site
