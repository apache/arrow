...
% rootPaths = convertCharsToStrings({dirContents.name});
% 
...
%     cmd = compose("cp -P %s %s", sharedLibrariesToCopy(ii), sharedLibraryTargetFolder);
%     system(cmd);
% end
% 
...
% 
% % Re-zip the MLTBX source files 
% cmd = compose("pushd %s; zip -y -r %s %s; popd", tmpFolder, opts.OutputFile, strjoin(rootPaths));
% system(cmd);
