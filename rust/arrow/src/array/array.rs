...
impl UnionArray {
    ...
    pub fn try_new(
        ...
        if (type_ids.len() * 4) != value_offsets.len() {
...
mod tests {
    ...
    fn test_union_i32_dense() {
        let type_ids = vec![0_i8, 1, 2, 0, 2, 0, 1];
        let value_offsets = vec![0_i32, 0, 0, 1, 1, 2, 1];
        let data = vec![
            ("a".to_string(), vec![1_i32, 4, 6]),
            ("b".to_string(), vec![2_i32, 7]),
            ("c".to_string(), vec![3_i32, 5]),
        ];
        let expected_array_values = [1_i32, 2, 3, 4, 5, 6, 7];

        let array = create_test_union(type_ids.clone(), value_offsets.clone(), data);

        // Check type ids
        assert_eq!(
            Buffer::from(&type_ids.clone().to_byte_slice()),
        ...
        // Check offsets
        assert_eq!(
            Buffer::from(value_offsets.clone().to_byte_slice()),
    ...
    fn test_union_i32_sparse() {
        let type_ids = vec![0_i8, 1, 2, 0, 2, 0, 1];
        let value_offsets = vec![0_i32, 1, 2, 3, 4, 5, 6];
        let data = vec![
            ("a".to_string(), vec![1_i32, 0, 0, 4, 0, 6, 0]),
            ("b".to_string(), vec![0_i32, 2_i32, 0, 0, 0, 0, 7]),
            ("c".to_string(), vec![0_i32, 0, 3_i32, 0, 5, 0, 0]),
        ];
        let expected_array_values = [1_i32, 2, 3, 4, 5, 6, 7];
    ...
    // Helper fn used in `UnionArray` tests
    fn create_test_union(
        type_ids: Vec<i8>,
        offsets: Vec<i32>,
        data: Vec<(String, Vec<i32>)>,
    ) -> UnionArray {
        let type_id_buffer = Buffer::from(&type_ids.to_byte_slice());
        let value_offsets_buffer = Buffer::from(offsets.to_byte_slice());
        let mut children = Vec::new();
        for (name, arr_data) in data.into_iter() {
            children.push((
                Field::new(&name, DataType::Int32, false),
                Arc::new(Int32Array::from(arr_data)) as Arc<Array>,
            ))
        }
        UnionArray::try_new(type_id_buffer, value_offsets_buffer, children).unwrap()
    }
