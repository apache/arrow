...
use std::io::Write;
use crate::array::*;
use crate::datatypes::{DataType, ArrowNumericType, TimeUnit};
use crate::error::{ArrowError, Result};
use crate::{buffer::{MutableBuffer, Buffer}, bitmap::Bitmap, util::bit_util};
...
impl CopyNullBit for NullBitNoop {
    ...
    fn null_buffer(&mut self) -> Buffer { Buffer::from([0u8; 0]) }
}
...
impl<'a> NullBitSetter<'a> {
    fn new(null_bitmap: &'a Bitmap) -> Self {
        ...
        let null_buffer = MutableBuffer::new(null_bytes.len())
            .with_bitset(null_bytes.len(), true);
        NullBitSetter {
...
impl<'a> CopyNullBit for NullBitSetter<'a> {
    ...
    fn copy_null_bit(&mut self, source_index: usize) {
        if bit_util::get_bit(self.source_bytes, source_index) == false {
            ...
            unsafe { bit_util::unset_bit_raw(self.target_buffer.raw_data_mut(), self.target_index); }
            self.null_count += 1;
        }
        self.target_index += 1;
    }

    fn null_count(&self) -> usize { self.null_count }
...
fn filter_array_impl(filter_context: &FilterContext, data_array: &impl Array, array_type: DataType, value_size: usize) -> Result<ArrayDataBuilder> 
{
    ...
    for i in 0..filter_u64.len() {
        ...
        for j in 0..64 {
            ...
            if (filter_batch & filter_mask[j]) != 0 {
                ...
                copy_buffer[(value_size * copy_count)..((value_size * copy_count) + value_size)]
                    .copy_from_slice(&data_bytes[(value_size * data_index)..((value_size * data_index) + value_size)]);
                copy_count += 1;
            }
        }
...
impl FilterContext {
    ...
    pub fn new(filter_array: &BooleanArray) -> Self {
        ...
        u64_buffer.write_bytes(filter_bytes, u64_buffer.capacity() - filter_bytes.len()).unwrap();
    ...
    pub fn filter_primitive_array<T>(&self, data_array: &PrimitiveArray<T>) -> Result<PrimitiveArray<T>> 
    where
        T: ArrowNumericType
    {
    ...
    pub fn filter_dictionary_array<T>(&self, data_array: &DictionaryArray<T>) -> Result<DictionaryArray<T>> 
    where
        T: ArrowNumericType
    {
        let array_type = data_array.data_type().clone();
        let value_size = mem::size_of::<T::Native>();
        let mut array_data_builder = filter_array_impl(self, data_array, array_type, value_size)?;
        // copy dictionary values from input array
        array_data_builder = array_data_builder.add_child_data(data_array.values().data());
        let data = array_data_builder.build();
...
/// Returns a new PrimitiveArray<T> containing only those values from the array passed as the data_array parameter, 
/// selected by the BooleanArray passed as the filter_array parameter
pub fn filter_primitive_array<T>(data_array: &PrimitiveArray<T>, filter_array: &BooleanArray) -> Result<PrimitiveArray<T>> 
where
    T: ArrowNumericType
{
...
/// Returns a new DictionaryArray<T> containing only those keys from the array passed as the data_array parameter, 
/// selected by the BooleanArray passed as the filter_array parameter. The values are cloned from the data_array.
pub fn filter_dictionary_array<T>(data_array: &DictionaryArray<T>, filter_array: &BooleanArray) -> Result<DictionaryArray<T>> 
where
    T: ArrowNumericType
{
...
pub fn filter_record_batch(record_batch: &RecordBatch, filter_array: &BooleanArray) -> Result<RecordBatch> {
    let filter_context = FilterContext::new(filter_array);
    let filtered_arrays = record_batch.columns().iter()
        .map(|a| filter_context.filter(a.as_ref()).unwrap()).collect();
...
mod tests {
    ...
    fn test_filter_dictionary_array() {
        ...
        let c = filter(&a, &b).unwrap();
        let d = c.as_ref().as_any().downcast_ref::<Int8DictionaryArray>().unwrap();
        ...
        assert_eq!(2, d.len());
        assert_eq!(true, d.is_null(0));
        assert_eq!("world", values.value(d.keys().nth(1).unwrap().unwrap() as usize));
    }
}
