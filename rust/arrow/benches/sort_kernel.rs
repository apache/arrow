...
use arrow::array::*;
use arrow::compute::kernels::sort::{lexsort, sort, SortColumn};
...
fn bench_lexsort(arr_a: &ArrayRef, array_b: &ArrayRef) {
    let columns = vec![
        SortColumn {
            values: arr_a.clone(),
            options: None,
...
fn bench_sort(input: &ArrayRef) {
    criterion::black_box(sort(&input, Some(SortOptions::default())).unwrap());
}
