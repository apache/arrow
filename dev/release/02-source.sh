...
mkdir -p js-tmp && cd js-tmp
# run npm pack on `tmp-apache-arrow/js`
npm pack ../${extract_dir}/js
# unzip and remove the pack tarball
tar -xzf *.tgz && rm *.tgz
cd -
...
# replace ${tag}/js with the npm-packed files instead
rm -rf ${tag}/js
# npm pack puts the files in `$PWD/package`
mv js-tmp/package ${tag}/js
rm -rf ${tag}/c_glib
