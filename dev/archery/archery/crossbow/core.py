...
class File(object):
    ...
    def create_blob(self, repo):
        ...
        if self.sha:
            ...
            if '.jar' in self.content:
                print("Working with jar files")
                # Reading the binary file (rb)
                with open(self.content, 'rb') as open_artifact:
                    byte_content = open_artifact.read()
                # Encode bytes read data
                base64_bytes = base64.b64encode(byte_content)
                # Decode these bytes to text to obtain a string (in utf-8)
                self.content = base64_bytes.decode('utf-8')
                self.sha = repo.create_blob(self.content, encoding='base64')
            elif '.pom' in self.content:
                print("Working with pom files")
                with open(self.content, 'r') as open_artifact:
                    self.content = open_artifact.read()
