...
from .reports import ConsoleReport, EmailReport
...
@click.pass_context
def crossbow(
    ctx, github_token, arrow_path, queue_path, queue_remote, output_file
):
    """
...
def submit(
    ...
    if no_push:
        click.echo(
            f"Branches and commits created but not pushed: `{job.branch}`"
        )
    else:
        queue.push()
...
def render(
    ...
    job = Job.from_config(
        config=config, target=target, tasks=[task], params=params
    )
...
@click.option(
    "--recipient-email", "-r", help="Where to send the e-mail report"
)
@click.option("--smtp-user", "-u", help="E-mail address to use for SMTP login")
@click.option(
    "--smtp-password", "-P", help="SMTP password to use for report e-mail."
)
@click.option(
    "--smtp-server",
    "-s",
