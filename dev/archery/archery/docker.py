...
class DockerCompose(Command):
    def __init__(
        self, config_path, dotenv_path=None, compose_bin=None, params=None
    ):
        compose_bin = default_bin(compose_bin, "docker-compose")
        self.config = ComposeConfig(
            config_path, dotenv_path, compose_bin, params
        )
    ...
    def push(self, service_name, user=None, password=None, using_docker=False):
        ...
        if user is not None:
            try:
                ...
                # hide credentials
                msg = (
                    "Failed to push `{}`, check the passed credentials".format(
                        service_name
                    )
                )
