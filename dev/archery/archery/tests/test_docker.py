...
def test_compose_default_params_and_env(arrow_compose_path):
    compose = DockerCompose(
        arrow_compose_path, params=dict(UBUNTU="18.04", DASK="master")
    )
...
def test_compose_build(arrow_compose_path):
    ...
    with assert_compose_calls(compose, expected_calls):
        compose.build(
            "conda-python-pandas", use_cache=True, use_leaf_cache=False
        )
...
def test_compose_run(arrow_compose_path):
    ...
    expected_calls = [
        (
            format_run(
                "-e CONTAINER_ENV_VAR_A=a -e CONTAINER_ENV_VAR_B=b "
                "conda-python"
            )
        )
    ]
...
def test_compose_error(arrow_compose_path):
    compose = DockerCompose(
        arrow_compose_path, params=dict(PYTHON="3.8", PANDAS="master")
    )
