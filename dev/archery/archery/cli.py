    ...
    **kwargs,
    ...
    with tmpdir(preserve=preserve, preserve_dir=preserve_dir) as root:
        ...
        if runner_base.results_dir is not None:
            results_path = os.path.join(runner_base.results_dir, "benchmark.json")
            with open(results_path, "w") as f:
                f.write(json_out)
            metadata_path = os.path.join(runner_base.results_dir, "metadata.json")
...
@click.option(
    "--write-generated-json", default="", help="Generate test JSON to indicated path"
)
@click.option(
    "--write-gold-files", default="", help="Generate gold files to indicated path"
)
@click.option(
...
@click.option(
    "--arrow-token",
    envvar=["GH_TOKEN", "ARROW_GITHUB_TOKEN"],
...
add_optional_command("ci", module=".ci.cli", function="ci", parent=archery)
