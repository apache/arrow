...
// Argument-dependent lookup (with ToArrowStatus defined in its argument's namespace)
// should find this correctly in functions and macros implicitly converting to Status.
// Putting this in the arrow namespace instead would not work because the declaration
// would come after the definition of functions using it, such as Status::OrElse.
Status ToArrowStatus(StatusLike v) {
  if (v.value == 42) {
    return Status::OK();
  } else {
