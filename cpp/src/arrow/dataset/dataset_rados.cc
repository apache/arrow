...
Result<std::shared_ptr<DatasetFactory>> RadosDatasetFactory::Make(
  ...
  return std::shared_ptr<DatasetFactory>(
      new RadosDatasetFactory(std::move(objects), std::move(cluster), std::move(options)));
}
...
Result<std::shared_ptr<Dataset>> RadosDatasetFactory::Finish(FinishOptions finish_options) {
