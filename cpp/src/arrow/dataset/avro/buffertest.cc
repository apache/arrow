...
void TestGrow() {
  BOOST_TEST_MESSAGE("TestGrow");
  {
    ...
    BOOST_CHECK_EQUAL(ob.size(),
                      kDefaultBlockSize + kDefaultBlockSize / 2 + reserveSize + 1);
...
void TestSeek() {
  BOOST_TEST_MESSAGE("TestSeek");
  {
    ...
    cout << "Saved loc = " << loc << '\n';
    BOOST_CHECK_EQUAL(static_cast<std::string::size_type>(loc),
                      (str.size() - part2.size()));
...
void TestIterator() {
  BOOST_TEST_MESSAGE("TestIterator");
  {
    ...
    size_t acc = 0;
    for (OutputBuffer::const_iterator iter = ob.begin(); iter != ob.end(); ++iter) {
      acc += iter->size();
    }
...
#else
void TestAsioBuffer() { cout << "Skipping asio test\n"; }
...
void TestCopy() {
  ...
  // Test1, small data, small buffer
  {
    ...
    BOOST_CHECK_EQUAL(kDefaultBlockSize - msg.size(), wb.freeSpace());
    ...
    BOOST_CHECK_EQUAL(kDefaultBlockSize * 2 - msg.size(), wb.freeSpace());
  }
  ...
  // Test2, small data, large buffer
  {
    ...
    BOOST_CHECK_EQUAL(kMaxBlockSize * 3 - msg.size() + kMinBlockSize, wb.freeSpace());
  }
...
void TestForeign() {
  BOOST_TEST_MESSAGE("TestForeign");
  {
...

    {
      OutputBuffer buf;
      buf.writeTo(hello.c_str(), hello.size());
      buf.appendForeignData(there.c_str(), there.size(),
                            boost::bind(&deleteForeign, there));
...
void TestForeignDiscard() {
  BOOST_TEST_MESSAGE("TestForeign");
  {
    ...
    OutputBuffer buf;
    buf.writeTo(hello.c_str(), hello.size());
    buf.appendForeignData(again.c_str(), again.size(),
                          boost::bind(&deleteForeign, again));
    buf.appendForeignData(there.c_str(), there.size(),
                          boost::bind(&deleteForeign, there));
    buf.writeTo(world.c_str(), world.size());
