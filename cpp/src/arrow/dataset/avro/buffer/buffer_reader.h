...
class AVRO_DECL BufferReader : private boost::noncopyable {
 ...
 private:
  size_type chunkRemaining() const { return iter_->dataSize() - chunkPos_; }
  ...
  const data_type* addr() const { return iter_->tellReadPos() + chunkPos_; }
 ...
 public:
  ...
  size_type bytesRemaining() const { return bytesRemaining_; }

  /**
   * Read a block of data from the front of the buffer.
   **/

  size_type bytesRead() const { return bytes_ - bytesRemaining_; }
