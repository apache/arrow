...
struct InputIteratorHelper {
  ...
  const data_type* data() const { return iter_->tellReadPos(); }

  /// The size of valid data in this chunk.
  size_type size() const { return iter_->dataSize(); }
  ...
  operator ConstAsioBuffer() const { return ConstAsioBuffer(data(), size()); }
#endif
...
struct OutputIteratorHelper {
  ...
  data_type* data() const { return iter_->tellWritePos(); }

  /// The size of area that can be written in this chunk.
  size_type size() const { return iter_->freeSize(); }

  /// Conversion operator.   It doesn't check for null, because the only
  /// the only time the chunk should be null is when it's the iterator
  /// end(), which should never be dereferenced anyway.
#ifdef HAVE_BOOST_ASIO
  operator MutableAsioBuffer() const { return MutableAsioBuffer(data(), size()); }
#endif
...
class BufferIterator {
  ...
  */
  ...
  reference operator*() { return helper_; }

  /// Dereference iterator, returns const InputIteratorHelper or OutputIteratorHelper
  /// wrapper.
  const value_type& operator*() const { return helper_; }

  /// Dereference iterator, returns InputIteratorHelper or OutputIteratorHelper wrapper.
  pointer operator->() { return &helper_; }
