...
class AVRO_DECL OutputBuffer {
 public:
  ...
  void reserve(size_type reserveSize) { pimpl_->reserveFreeSpace(reserveSize); }
  ...
  bool empty() const { return (pimpl_->size() == 0); }
  ...
  size_type freeSpace() const { return pimpl_->freeSpace(); }
  ...
  const_iterator begin() const { return const_iterator(pimpl_->beginWrite()); }

  /**
   * Return the end iterator for writing.
   **/

  const_iterator end() const { return const_iterator(pimpl_->endWrite()); }

  /**
   * Discard any data in this buffer.
   **/

  void discardData() { pimpl_->discardData(); }
  ...
  void appendForeignData(const data_type* data, size_type size,
  ...
  int numChunks() const { return pimpl_->numFreeChunks(); }

  /**
   * Returns the number of chunks that contain data
   **/

  int numDataChunks() const { return pimpl_->numDataChunks(); }
...
class AVRO_DECL InputBuffer {
 public:
  ...
  const_iterator begin() const { return const_iterator(pimpl_->beginRead()); }

  /**
   * Return the end iterator.
   **/

  const_iterator end() const { return const_iterator(pimpl_->endRead()); }

  /**
   * Returns the number of chunks containing data.
   **/

  int numChunks() const { return pimpl_->numDataChunks(); }
