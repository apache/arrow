...
inline char toHex(unsigned int n) { return (n < 10) ? (n + '0') : (n + 'a' - 10); }
...
class AVRO_DECL JsonParser : boost::noncopyable {
 ...
 public:
  ...
  Token cur() const { return curToken; }

  double doubleValue() const { return dv; }

  int64_t longValue() const { return lv; }

  const std::string& rawString() const { return sv; }

  std::string stringValue() const { return decodeString(sv, false); }

  std::string bytesValue() const { return decodeString(sv, true); }
  ...
  static const char* toString(Token tk) { return tokenNames[tk]; }
};
...
class AVRO_DECL JsonGenerator {
  ...
  void escapeCtl(char c) { escapeUnicode(static_cast<uint8_t>(c)); }
 ...
 public:
  ...
  int64_t byteCount() const { return out_.byteCount(); }
