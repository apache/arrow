...
struct codec_traits<std::vector<uint8_t> > {
   ...
   * Encodes a given value.
   */
  static void encode(Encoder& e, const std::vector<uint8_t>& b) { e.encodeBytes(b); }
...
struct codec_traits<avro::null> {
   ...
   * Encodes a given value.
   */
  static void encode(Encoder& e, const avro::null&) { e.encodeNull(); }

  /**
   * Decodes into a given value.
   */
  static void decode(Decoder& d, avro::null&) { d.decodeNull(); }
};
