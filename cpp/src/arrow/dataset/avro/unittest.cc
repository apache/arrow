...
struct TestNested {
  ...
  void createSchema() {
    ...
    arrayTree.addField("children",
                       ArraySchema(SymbolicSchema(Name("ArrayTree"), arrayTree.root())));
  ...
  void readArrayRecord(Parser<ValidatingReader>& p) {
    ...
    for (int64_t bs = p.readArrayBlockSize(); bs > 0; bs = p.readArrayBlockSize()) {
  ...
  void testDecodeNoRecurse() {
    ...
    runEncodeDecode(*validatingEncoder(schema_, binaryEncoder()),
                    *validatingDecoder(schema_, binaryDecoder()), encodeNoRecurse);
  }

  void testDecodeRecurse() {
    std::cout << "DecodeRecurse\n";
    runEncodeDecode(*validatingEncoder(schema_, binaryEncoder()),
                    *validatingDecoder(schema_, binaryDecoder()), encodeRecurse);
  }

  void testDecodeNoRecurseJson() {
    std::cout << "DecodeNoRecurseJson\n";
    runEncodeDecode(*jsonEncoder(schema_), *jsonDecoder(schema_), encodeNoRecurse);
  }

  void testDecodeRecurseJson() {
    std::cout << "DecodeRecurseJson\n";
    runEncodeDecode(*jsonEncoder(schema_), *jsonDecoder(schema_), encodeRecurse);
  }
...
boost::unit_test::test_suite* init_unit_test_suite(int argc, char* argv[]) {
  ...
  test->add(
      BOOST_CLASS_TEST_CASE(&TestEncoding::test, boost::make_shared<TestEncoding>()));
  test->add(BOOST_CLASS_TEST_CASE(&TestSchema::test, boost::make_shared<TestSchema>()));
  test->add(BOOST_CLASS_TEST_CASE(&TestNested::test, boost::make_shared<TestNested>()));
  test->add(
      BOOST_CLASS_TEST_CASE(&TestGenerated::test, boost::make_shared<TestGenerated>()));
  test->add(
      BOOST_CLASS_TEST_CASE(&TestBadStuff::test, boost::make_shared<TestBadStuff>()));
  test->add(
      BOOST_CLASS_TEST_CASE(&TestResolution::test, boost::make_shared<TestResolution>()));
