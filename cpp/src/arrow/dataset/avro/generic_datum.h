...
class AVRO_DECL GenericDatum {
 protected:
  ...
  GenericDatum(Type t) : type_(t), logicalType_(LogicalType::NONE) {}

  GenericDatum(Type t, LogicalType logicalType) : type_(t), logicalType_(logicalType) {}
 ...
 public:
  ...
  GenericDatum(bool v) : type_(AVRO_BOOL), logicalType_(LogicalType::NONE), value_(v) {}

  /// Makes a new AVRO_INT datum whose value is of type int32_t.
  GenericDatum(int32_t v) : type_(AVRO_INT), logicalType_(LogicalType::NONE), value_(v) {}
  ...
  GenericDatum(float v) : type_(AVRO_FLOAT), logicalType_(LogicalType::NONE), value_(v) {}
...
class AVRO_DECL GenericContainer {
 ...
 protected:
   ...
   */
  GenericContainer(Type type, const NodePtr& s) : schema_(s) { assertType(s, type); }
...
class AVRO_DECL GenericUnion : public GenericContainer {
 ...
 public:
   ...
   * in this union.
   */
  GenericDatum& datum() { return datum_; }

  /**
   * Returns the datum corresponding to the currently selected branch
   * in this union.
   */
  const GenericDatum& datum() const { return datum_; }
};
...
class AVRO_DECL GenericRecord : public GenericContainer {
 ...
 public:
   ...
   */
  GenericDatum& field(const std::string& name) { return fieldAt(fieldIndex(name)); }
...
class AVRO_DECL GenericEnum : public GenericContainer {
 ...
 public:
   ...
   */
  GenericEnum(const NodePtr& schema) : GenericContainer(AVRO_ENUM, schema), value_(0) {}
...
inline LogicalType GenericDatum::logicalType() const { return logicalType_; }
