...
class NodeImpl : public Node {
 protected:
  ...
  // Ctor with "doc"
  NodeImpl(Type type, const NameConcept& name,
           const concepts::SingleAttribute<std::string>& doc, const LeavesConcept& leaves,
           const LeafNamesConcept& leafNames, const SizeConcept& size)
      : Node(type),
        nameAttribute_(name),
        docAttribute_(doc),
  ...
  void doSetDoc(const std::string& doc) { docAttribute_.add(doc); }

  const std::string& getDoc() const { return docAttribute_.get(); }
  ...
  const std::string& nameAt(int index) const { return leafNameAttributes_.get(index); }
  ...
  // Rem: NameConcept type is HasName (= SingleAttribute<Name>), we use std::string
  // instead
...
class AVRO_DECL NodeRecord : public NodeImplRecord {
 ...
 public:
  ...
  NodeRecord(const HasName& name, const HasDoc& doc, const MultiLeaves& fields,
             const LeafNames& fieldsNames, const std::vector<GenericDatum>& dv)
  ...
  const GenericDatum& defaultValueAt(int index) { return defaultValues[index]; }
...
class AVRO_DECL NodeEnum : public NodeImplEnum {
 public:
  ...
  NodeEnum(const HasName& name, const LeafNames& symbols)
    ...
    for (size_t i = 0; i < leafNameAttributes_.size(); ++i) {
      if (!nameIndex_.add(leafNameAttributes_.get(i), i)) {
        throw Exception(boost::format("Cannot add duplicate enum: %1%") %
                        leafNameAttributes_.get(i));
      }
    }
  }
...
inline std::string intToHex(T i) {
  ...
  stream << "\\u" << std::setfill('0') << std::setw(sizeof(T)) << std::hex << i;
  return stream.str();
}
