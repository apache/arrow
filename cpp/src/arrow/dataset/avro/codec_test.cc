...
void testCodecResolving(const TestData3& td) {
  ...
  BOOST_TEST_CHECKPOINT("Test: " << testNo << ' ' << " writer schema: " << td.writerSchema
...
void testCodecResolving2(const TestData4& td) {
  ...
  for (unsigned int i = 0; i <= td.depth; ++i) {
    ...
    BOOST_TEST_CHECKPOINT(
        "Test: " << testNo << ' ' << " writer schema: " << td.writerSchema
                 << " writer calls: " << td.writerCalls
                 << " reader schema: " << td.readerSchema
                 << " reader calls: " << td.readerCalls << " skip-level: " << skipLevel);
    unique_ptr<InputStream> in = memoryInputStream(*p);
    DecoderPtr d = CodecFactory::newDecoder(vs, rvs);
...
struct JsonCodec {
  static EncoderPtr newEncoder(const ValidSchema& schema) { return jsonEncoder(schema); }
  static DecoderPtr newDecoder(const ValidSchema& schema) { return jsonDecoder(schema); }
};
...
struct ValidatingEncoderResolvingDecoderFactory : public ValidatingEncoderFactory {
  static DecoderPtr newDecoder(const ValidSchema& schema) {
    return resolvingDecoder(schema, schema, validatingDecoder(schema, binaryDecoder()));
  }

  static DecoderPtr newDecoder(const ValidSchema& writer, const ValidSchema& reader) {
    return resolvingDecoder(writer, reader, validatingDecoder(writer, binaryDecoder()));
  }
};
...
boost::unit_test::test_suite* init_unit_test_suite(int argc, char* argv[]) {
  ...
  ts->add(BOOST_PARAM_TEST_CASE(&avro::testJson, avro::jsonData, ENDOF(avro::jsonData)));
