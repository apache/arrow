...
class PrimitiveParser : public Resolver {
 public:
  PrimitiveParser(const PrimitiveLayout& offset) : Resolver(), offset_(offset.offset()) {}
...
class RecordParser : public Resolver {
 public:
  ...
  RecordParser(ResolverFactory& factory, const NodePtr& writer, const NodePtr& reader,
...
class MapParser : public Resolver {
 public:
  ...
  MapParser(ResolverFactory& factory, const NodePtr& writer, const NodePtr& reader,
            const CompoundLayout& offsets);
...
class ArrayParser : public Resolver {
 public:
  ArrayParser(ResolverFactory& factory, const NodePtr& writer, const NodePtr& reader,
              const CompoundLayout& offsets);
...
class EnumParser : public Resolver {
 public:
  enum EnumRepresentation { VAL };

  EnumParser(ResolverFactory& factory, const NodePtr& writer, const NodePtr& reader,
             const CompoundLayout& offsets)
      : Resolver(), offset_(offsets.at(0).offset()), readerSize_(reader->names()) {
...
class UnionParser : public Resolver {
 public:
  ...
  UnionParser(ResolverFactory& factory, const NodePtr& writer, const NodePtr& reader,
              const CompoundLayout& offsets);
...
class FixedParser : public Resolver {
 public:
  FixedParser(ResolverFactory& factory, const NodePtr& writer, const NodePtr& reader,
              const CompoundLayout& offsets)
      : Resolver() {
    size_ = writer->fixedSize();
...
class ResolverFactory : private boost::noncopyable {
  template <typename T>
  unique_ptr<Resolver> constructPrimitiveSkipper(const NodePtr& writer) {
    return unique_ptr<Resolver>(new PrimitiveSkipper<T>());
  }

  template <typename T>
  unique_ptr<Resolver> constructPrimitive(const NodePtr& writer, const NodePtr& reader,
                                          const Layout& offset) {
    ...
    if (match == RESOLVE_NO_MATCH) {
      ...
      instruction = unique_ptr<Resolver>(
      ...
      const PrimitiveLayout& primitiveLayout =
          static_cast<const PrimitiveLayout&>(offset);
      instruction =
          unique_ptr<Resolver>(new PrimitivePromoter<T, int64_t>(primitiveLayout));
    } else if (match == RESOLVE_PROMOTABLE_TO_FLOAT) {
      const PrimitiveLayout& primitiveLayout =
          static_cast<const PrimitiveLayout&>(offset);
      instruction =
          unique_ptr<Resolver>(new PrimitivePromoter<T, float>(primitiveLayout));
    } else if (match == RESOLVE_PROMOTABLE_TO_DOUBLE) {
      const PrimitiveLayout& primitiveLayout =
          static_cast<const PrimitiveLayout&>(offset);
      instruction =
          unique_ptr<Resolver>(new PrimitivePromoter<T, double>(primitiveLayout));
    } else {
      assert(0);
    }
    return instruction;
  ...
  unique_ptr<Resolver> constructCompoundSkipper(const NodePtr& writer) {
    return unique_ptr<Resolver>(new Skipper(*this, writer));
  }

  template <typename Parser, typename Skipper>
  unique_ptr<Resolver> constructCompound(const NodePtr& writer, const NodePtr& reader,
                                         const Layout& offset) {
    unique_ptr<Resolver> instruction;
    ...
    if (match == RESOLVE_NO_MATCH) {
      ...
      instruction =
          unique_ptr<Resolver>(new UnionToNonUnionParser(*this, writer, reader, offset));
    } else {
      const CompoundLayout& compoundLayout = dynamic_cast<const CompoundLayout&>(offset);
      instruction =
          unique_ptr<Resolver>(new Parser(*this, writer, reader, compoundLayout));
    }
 ...
 public:
  unique_ptr<Resolver> construct(const NodePtr& writer, const NodePtr& reader,
                                 const Layout& offset) {
    typedef unique_ptr<Resolver> (ResolverFactory::*BuilderFunc)(
  ...
  unique_ptr<Resolver> skipper(const NodePtr& writer) {
