...
DataFileWriterBase::DataFileWriterBase(const char* filename, const ValidSchema& schema,
...
void DataFileWriterBase::sync() {
  ...
  if (codec_ == NULL_CODEC) {
    ...
    std::unique_ptr<InputStream> in =
        memoryInputStream(reinterpret_cast<const uint8_t*>(buf.data()), buf.size());
    ...
    // Now compress
    size_t compressed_size = snappy::Compress(reinterpret_cast<const char*>(temp.data()),
                                              temp.size(), &compressed);
    temp.clear();
    {
      boost::iostreams::filtering_ostream os;
      os.push(boost::iostreams::back_inserter(temp));
    ...
    std::unique_ptr<InputStream> in =
        memoryInputStream(reinterpret_cast<const uint8_t*>(temp.data()), temp.size());
...
DataFileReaderBase::DataFileReaderBase(const char* filename)
    : filename_(filename),
      stream_(fileSeekableInputStream(filename)),
...
void DataFileReaderBase::readDataBlock() {
  ...
  if (codec_ == NULL_CODEC) {
    ...
    if (!snappy::Uncompress(reinterpret_cast<const char*>(compressed_.data()), len - 4,
                            &uncompressed)) {
      throw Exception("Snappy Compression reported an error when decompressing");
    }
    ...
    os_->push(boost::iostreams::basic_array_source<char>(uncompressed.c_str(),
                                                         uncompressed.size()));
    ...
    os_->push(boost::iostreams::basic_array_source<char>(compressed_.data(),
                                                         compressed_.size()));
...
void DataFileReaderBase::sync(int64_t position) {
  ...
  while (i < SyncSize) {
    ...
    int len = std::min(static_cast<size_t>(SyncSize - i), n);
...
int64_t DataFileReaderBase::previousSync() { return blockStart_; }
