...
class Symbol {
 ...
 public:
  ...
  bool isTerminal() const { return kind_ > sTerminalLow && kind_ < sTerminalHigh; }
  ...
  static Symbol repeater(const ProductionPtr& read, const ProductionPtr& skip,
                         bool isArray) {
    std::stack<ssize_t> s;
  ...
  static Symbol defaultEndAction() { return Symbol(sDefaultEnd); }
  ...
  static Symbol fieldSymbol(const std::string& name) { return Symbol(sField, name); }
  ...
  static Symbol indirect(const ProductionPtr& p) { return Symbol(sIndirect, p); }
 ...
 */
template <typename T>
void fixup_internal(const ProductionPtr& p, const std::map<T, ProductionPtr>& m,
                    std::set<ProductionPtr>& seen) {
...
void fixup(Symbol& s, const std::map<T, ProductionPtr>& m,
  ...
  switch (s.kind()) {
    ...
    case Symbol::sAlternative: {
      ...
      for (std::vector<ProductionPtr>::const_iterator it = vv->begin(); it != vv->end();
           ++it) {
        ...
        switch (s.kind()) {
          ...
          default:
            if (s.isImplicitAction()) {
              ...
              std::ostringstream oss;
              oss << "Encountered " << Symbol::toString(s.kind()) << " while looking for "
                  << Symbol::toString(k);
              throw Exception(oss.str());
            }
        }
  ...
  switch (s.kind()) {
    ...
    case Symbol::sAlternative: {
      ...
      for (std::vector<ProductionPtr>::const_iterator it =
           ...
           it != s.extrap<std::vector<ProductionPtr> >()->end(); ++it) {
        os << ' ' << **it;
      }
      os << ')';
    } break;
