...
OneShotDataset::OneShotDataset(std::shared_ptr<RecordBatchReader> reader)
    : InMemoryDataset(reader->schema(),
                      std::make_shared<ReaderRecordBatchGenerator>(reader)) {}
...
Result<FragmentIterator> OneShotDataset::GetFragmentsImpl(compute::Expression predicate) {
  FragmentVector fragments{
      std::make_shared<OneShotFragment>(schema(), get_batches_->Get())};
