...
// Validate the batch, accounting for the -validate_decimals flag and -validate_date64
static Status ValidateFull(const RecordBatch& batch) {
  if (FLAGS_validate_decimals && FLAGS_validate_date64) {
    return batch.ValidateFull();
  }
  // Decimal or date64 validation disabled, so individually validate columns
  RETURN_NOT_OK(batch.Validate());
  for (const auto& column : batch.columns()) {
    if (!FLAGS_validate_decimals && is_decimal(column->type()->id())) {
      continue;
    }
    if (!FLAGS_validate_date64 && column->type()->id() == Type::DATE64) {
      continue;
    }
