...
TEST(SubstraitRoundTrip, BasicPlan) {
  ...
  auto comp_left_value = compute::field_ref(FieldPath({1}));
  auto comp_right_value = compute::field_ref(FieldPath({2}));
...
TEST(Substrait, FilterProjectPlanRoundTripping) {
  ...
  compute::Expression project_fp_expr = compute::call(
      "add", {compute::field_ref(FieldPath({1})), compute::field_ref(FieldPath({2}))});
  ...
  ASSERT_OK_AND_ASSIGN(auto expected_table,
                       GetTableFromPlan(declarations, exec_context, project_schema));
  ...
  auto subs_project_expr = {compute::field_ref(FieldPath({0})),
                            compute::field_ref(FieldPath({1})),
                            compute::field_ref(FieldPath({2})), project_fp_expr};
  auto roundtripped_project =
      std::get<compute::Declaration>(roundtripped_emit_project.inputs[0]);
  AssertProjectRelation(roundtripped_project, subs_project_expr, project_schema);

  // assert scan declaration
  auto roundtripped_scan = std::get<compute::Declaration>(roundtripped_project.inputs[0]);
  AssertScanRelation(roundtripped_scan, dataset, dummy_schema);
}
