...
Result<std::shared_ptr<Table>> Table::MakeEmptyTable(std::shared_ptr<Schema> schema,
                                                     MemoryPool* memory_pool) {
  ChunkedArrayVector empty_table(schema->num_fields());
  for (int i = 0; i < schema->num_fields(); i++) {
    ARROW_ASSIGN_OR_RAISE(empty_table[i], ChunkedArray::MakeEmptyChunkedArray(
                                              schema->field(i)->type(), memory_pool));
  }
