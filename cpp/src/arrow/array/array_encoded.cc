...
Result<std::shared_ptr<RunLengthEncodedArray>> RunLengthEncodedArray::Make(
    ...
    int64_t logical_length, int64_t null_count, int64_t offset) {
  ...
  return std::make_shared<RunLengthEncodedArray>(
      run_length_encoded(values_array->type()), logical_length, values_array,
      std::move(run_ends_buffer), null_count, offset);
}
