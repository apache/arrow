...
struct ValidateArrayImpl {
  ...
  Status Visit(const RunLengthEncodedType& type) {
    switch (type.run_ends_type()->id()) {
      ...
      case Type::INT32:
        return ValidateRunLengthEncoded<int32_t>(type);
      case Type::INT64:
        return ValidateRunLengthEncoded<int64_t>(type);
      default:
 ...
 private:
  ...
  Status ValidateRunLengthEncoded(const RunLengthEncodedType& type) {
    // overflow was already checked at this point
    if (data.offset + data.length > std::numeric_limits<RunEndsType>::max()) {
      return Status::Invalid(
          "Offset + length of an RLE array must fit in a value of the run ends type ",
          *type.run_ends_type(), ", but offset + length was ", data.offset + data.length,
    ...
    if (*run_ends_data.type != *type.run_ends_type()) {
      return Status::Invalid("Run ends array of ", type, " must be ",
                             *type.run_ends_type(), ", but is ", *run_ends_data.type);
    }
    ...
    ArraySpan span(data);
    const RunEndsType* run_ends = rle_util::RunEnds<RunEndsType>(span);
