...
class WindowGroupByNode : public ExecNode, public TracedNode {
 public:
  WindowGroupByNode(ExecNode* input, std::shared_ptr<Schema> output_schema,
      ...
      : ExecNode(input->plan(), {input}, {"groupby"}, std::move(output_schema)),
        ...
        left_inclusive(window_args->left_inclusive ? "greater_equal" : "greater"),
        right_inclusive(window_args->right_inclusive ? "greater" : "greater_equal"),
        left(window_args->left_boundary),
        right(window_args->right_boundary) {}
  ...
  const char* kind_name() const override { return "WindowGroupByNode"; }
 ...
 private:
  Status OutputResult(bool flush = false);

  Status Consume(KernelContext ctx, ExecSpan& run_batch);

  Status Finalize(KernelContext ctx, ExecBatch& batch, std::size_t base);

  Status InitStatesVector(KernelContext& kernel_ctx);

  Result<std::pair<uint32_t, uint32_t>> FindLeftIndex(const Datum& data,
                                                      ExecContext* ctx);
  Result<std::pair<uint32_t, uint32_t>> FindRightIndex(const Datum& data,
                                                       ExecContext* ctx, bool flush);
  ...
  // Window content(s)
  std::vector<ExecBatch> window_batches;
  std::vector<std::vector<Datum>> window_segment_values;
  std::vector<std::vector<Segment>> window_segments;

  // Window parameters
  std::optional<WindowAggregateArgs> window_args;
  const std::string left_inclusive;
  const std::string right_inclusive;
  const Datum left;
  const Datum right;
  ...
  std::pair<uint32_t, uint32_t> current_index;
...
class WindowScalarNode : public ExecNode, public TracedNode {
 public:
  WindowScalarNode(ExecPlan* plan, std::vector<ExecNode*> inputs,
                   std::shared_ptr<Schema> output_schema,
                   std::unique_ptr<RowSegmenter> segmenter,
                   std::vector<int> segment_field_ids,
                   std::vector<std::vector<int>> target_fieldsets,
  ...
  const char* kind_name() const override { return "WindowScalarNode"; }
 ...
 private:
  Status Consume(KernelContext ctx,
                 std::vector<std::vector<std::unique_ptr<KernelState>>>& states_,
                 ExecSpan& run_batch);

  Status Finalize(KernelContext ctx,
                  std::vector<std::vector<std::unique_ptr<KernelState>>>& states_,
                  ExecBatch& batch, std::size_t base);

  Status OutputResult(bool flush = false);

  Status InitStatesVector(std::vector<std::vector<std::unique_ptr<KernelState>>>& states,
