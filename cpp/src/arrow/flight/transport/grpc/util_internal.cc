...
TransportStatus CombinedTransportStatus(const ::grpc::Status& grpc_status,
                                        arrow::Status arrow_status,
                                        ::grpc::ClientContext* ctx) {
  TransportStatus status = TransportStatusFromGrpc(grpc_status);
  if (status.ok() && !arrow_status.ok()) {
    status = TransportStatus{TransportStatusCode::kInternal, arrow_status.ToString()};
  }

  std::vector<std::unique_ptr<StatusDetail>> details;

  if (!arrow_status.ok()) {
    details.emplace_back(std::make_unique<ArrowStatusDetail>(std::move(arrow_status)));
  }
  if (!status.ok() && ctx) {
    ...
    // const auto code_val = trailers.find(kGrpcStatusCodeHeader);
    // const auto message_val = trailers.find(kGrpcStatusMessageHeader);
    // // TODO: refactor out of ReconstructStatus

    // gRPC error details
    auto [it, end] = trailers.equal_range(kBinaryErrorDetailsKey);
    for (; it != end; it++) {
      details.emplace_back(std::make_unique<BinaryStatusDetail>(
  ...
  if (!details.empty()) {
    ...
    return TransportStatus(code, std::move(status).MoveMessage(), std::move(details));
  }
  return status;
}
...
static ::grpc::Status ToRawGrpcStatus(const Status& arrow_status) {
  ...
  return {grpc_code, std::move(transport_status).MoveMessage()};
}
