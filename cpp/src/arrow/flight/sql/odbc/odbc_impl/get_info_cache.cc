...
inline int32_t ScalarToInt32(UnionScalar* scalar) {
  return reinterpret_cast<Int32Scalar*>(scalar->child_value().get())->value;
}

inline int64_t ScalarToInt64(UnionScalar* scalar) {
  return reinterpret_cast<Int64Scalar*>(scalar->child_value().get())->value;
}
...
inline void SetDefaultIfMissing(std::unordered_map<uint16_t, Connection::Info>& cache,
                                uint16_t info_type, Connection::Info default_value) {
...
bool GetInfoCache::LoadInfoFromServer() {
  if (sql_client_ && !has_server_info_.exchange(true)) {
    ...
    while (chunk_iter.GetNext(&chunk)) {
      ...
      UInt32Array* info_type_array = static_cast<UInt32Array*>(name_array.get());
      UnionArray* value_union_array = static_cast<UnionArray*>(value_array.get());
      for (int64_t i = 0; i < chunk.data->num_rows(); ++i) {
        if (!value_array->IsNull(i)) {
          auto info_type =
              static_cast<SqlInfoOptions::SqlInfo>(info_type_array->Value(i));
          auto result_scalar = value_union_array->GetScalar(i);
          util::ThrowIfNotOK(result_scalar.status());
          std::shared_ptr<Scalar> scalar_ptr = result_scalar.ValueOrDie();
          UnionScalar* scalar = reinterpret_cast<UnionScalar*>(scalar_ptr.get());
          switch (info_type) {
            // String properties
            ...
            case ARROW_SQL_NUMERIC_FUNCTIONS: {
              std::shared_ptr<Array> list_value =
                  reinterpret_cast<BaseListScalar*>(scalar->child_value().get())->value;
              uint32_t result_val = 0;
              for (int64_t list_index = 0; list_index < list_value->length();
                   ++list_index) {
                if (!list_value->IsNull(list_index)) {
                  ReportNumericFunction(reinterpret_cast<StringArray*>(list_value.get())
            ...
            case ARROW_SQL_STRING_FUNCTIONS: {
              ...
              for (int64_t list_index = 0; list_index < list_value->length();
                   ++list_index) {
                if (!list_value->IsNull(list_index)) {
                  ReportStringFunction(reinterpret_cast<StringArray*>(list_value.get())
                                           ->GetString(list_index),
                                       result_val);
                }
              }
            ...
            case ARROW_SQL_SYSTEM_FUNCTIONS: {
              ...
              for (int64_t list_index = 0; list_index < list_value->length();
                   ++list_index) {
                if (!list_value->IsNull(list_index)) {
                  ReportSystemFunction(reinterpret_cast<StringArray*>(list_value.get())
                                           ->GetString(list_index),
            ...
            case SqlInfoOptions::SQL_DATETIME_FUNCTIONS: {
              ...
              for (int64_t list_index = 0; list_index < list_value->length();
                   ++list_index) {
                if (!list_value->IsNull(list_index)) {
                  ReportDatetimeFunction(reinterpret_cast<StringArray*>(list_value.get())
                                             ->GetString(list_index),
                                         result_val);
                }
              }
            ...
            case ARROW_SQL_KEYWORDS: {
              ...
              for (int64_t list_index = 0; list_index < list_value->length();
                ...
                if (!list_value->IsNull(list_index)) {
                  ...
                  result_str += reinterpret_cast<StringArray*>(list_value.get())
                                    ->GetString(list_index);
                }
              }
            ...
            case SqlInfoOptions::SQL_SUPPORTS_CONVERT: {
              MapScalar* map_scalar =
                  reinterpret_cast<MapScalar*>(scalar->child_value().get());
              auto data_array = map_scalar->value;
              StructArray* map_contents =
                  reinterpret_cast<StructArray*>(data_array.get());
              ...
              for (int64_t map_index = 0; map_index < map_contents->length();
                ...
                if (!map_values->IsNull(map_index)) {
                  ...
                  int32_t map_key_scalar =
                      reinterpret_cast<Int32Scalar*>(map_key_scalar_ptr.get())->value;
                  auto map_value_scalar =
                      reinterpret_cast<BaseListScalar*>(map_value_scalar_ptr.get())
                          ->value;
                  ...
                  for (int64_t map_value_array_index = 0;
                    ...
                    if (!map_value_scalar->IsNull(map_value_array_index)) {
                      ...
                      info_bitmask_value_to_write |= GetCvtBitForArrowConvertEntry(
                          reinterpret_cast<Int32Scalar*>(list_entry_scalar.get())->value);
                    }
                  }
