...
inline void Utf8ToWcs(const char* utf8_string, size_t length,
  ...
  unsigned long length_in_bytes = string.size() * GetSqlWCharSize();  // NOLINT
  const uint8_t* data = (uint8_t*)string.data();                      // NOLINT
...
inline void WcsToUtf8(const void* wcs_string, size_t length_in_code_units,
  ...
  unsigned long length_in_bytes = byte_string.size();  // NOLINT
  const uint8_t* data = (uint8_t*)byte_string.data();  // NOLINT
