...
void DsnConfigurationWindow::OnCreate()
{
    ...
    commonGroupPosY += INTERVAL + CreateConnectionSettingsGroup(MARGIN, commonGroupPosY, groupSizeY);
    commonGroupPosY += INTERVAL + CreateAuthSettingsGroup(MARGIN, commonGroupPosY, groupSizeY);
...
int DsnConfigurationWindow::CreateConnectionSettingsGroup(int posX, int posY, int sizeX)
{
    ...
    serverEdit = CreateEdit(editPosX, rowPos, editSizeX, ROW_HEIGHT, val, ChildId::SERVER_EDIT);
...
int DsnConfigurationWindow::CreateAuthSettingsGroup(int posX, int posY, int sizeX)
{
    ...
    labels.push_back(CreateLabel(labelPosX, rowPos, LABEL_WIDTH, ROW_HEIGHT, "User:", ChildId::USER_LABEL));
    ...
    authTokenEdit = CreateEdit(editPosX, rowPos, editSizeX, ROW_HEIGHT,
        val, ChildId::AUTH_TOKEN_EDIT);
...
int DsnConfigurationWindow::CreateEncryptionSettingsGroup(int posX, int posY, int sizeX)
{
    ...
    labels.push_back(CreateLabel(labelPosX, rowPos, LABEL_WIDTH, ROW_HEIGHT, "Certificate:", ChildId::CERTIFICATE_LABEL));
    certificateEdit = CreateEdit(editPosX, rowPos, editSizeX - MARGIN - BUTTON_WIDTH, ROW_HEIGHT, val, ChildId::CERTIFICATE_EDIT);
    certificateBrowseButton = CreateButton(editPosX + editSizeX - BUTTON_WIDTH, rowPos - 2, BUTTON_WIDTH, BUTTON_HEIGHT,
        "Browse", ChildId::CERTIFICATE_BROWSE_BUTTON);
    ...
    encryptionSettingsGroupBox = CreateGroupBox(posX, posY, sizeX, rowPos - posY,
        "Encryption settings", ChildId::AUTH_SETTINGS_GROUP_BOX);
...
int DsnConfigurationWindow::CreatePropertiesGroup(int posX, int posY, int sizeX)
{
    ...
    propertyList = CreateList(labelPosX, rowPos, listSize, listHeight, ChildId::PROPERTY_LIST);
    ...
    SendMessage(propertyList->GetHandle(), LVM_SETEXTENDEDLISTVIEWSTYLE, LVS_EX_FULLROWSELECT, LVS_EX_FULLROWSELECT);

    rowPos += INTERVAL + listHeight;

    int deletePosX = width - INTERVAL - MARGIN - BUTTON_WIDTH;
    int addPosX = deletePosX - INTERVAL - BUTTON_WIDTH;
    addButton = CreateButton(addPosX, rowPos, BUTTON_WIDTH, BUTTON_HEIGHT, "Add", ChildId::ADD_BUTTON);
    deleteButton = CreateButton(deletePosX, rowPos, BUTTON_WIDTH, BUTTON_HEIGHT,
    ...
    switch (msg)
    {
        ...
        case WM_COMMAND:
        {
            switch (LOWORD(wParam))
            {
                ...
                {
                    useSystemCertStoreCheckBox->SetChecked(!useSystemCertStoreCheckBox->IsChecked());
                    break;
                }

                case ChildId::DISABLE_CERT_VERIFICATION_CHECKBOX:
                {
                    disableCertVerificationCheckBox->SetChecked(!disableCertVerificationCheckBox->IsChecked());
                    break;
                }
