...
TEST_F(TestTable, ToTensorNullToNanFloat16) {
  ...
  auto a0 = ChunkedArrayFromJSON(float16(), {"[null, 2, 3]", "[4, 5, 6, 7, 8, 9]"});
  auto a1 =
      ChunkedArrayFromJSON(float16(), {"[10, 20]", "[30, 40, null, 60, 70, 80, 90]"});
  auto table = Table::Make(schema, {a0, a1});
  ...
  CheckTableToTensor<HalfFloatType>(tensor, 18, shape, {f16_size, f16_size * shape[0]});
