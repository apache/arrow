...
/*
  Generates batches from data, then benchmark rows_per_second and batches_per_second for an
  isolated node. We do this by passing in batches through a task scheduler, and calling InputFinished
  and InputReceived.
*/
  ...
  for (auto _ : state) {
    ...
    ASSERT_OK_AND_ASSIGN(
        ...
        MakeExecNode("source", plan.get(), {},
                     arrow::compute::SourceNodeOptions{data.schema, data.gen(/*parallel=*/true,
                                                             /*slow=*/false)}));
    ...
    std::unique_ptr<arrow::compute::TaskScheduler> scheduler = arrow::compute::TaskScheduler::Make();
  ...
  void BenchmarkNodeOverhead(
    ...
    for (auto _ : state) {
      ...
      ASSERT_OK_AND_ASSIGN(std::shared_ptr<arrow::compute::ExecPlan> plan,
                          arrow::compute::ExecPlan::Make(&ctx));
      AsyncGenerator<util::optional<arrow::compute::ExecBatch>> sink_gen;
      arrow::compute::Declaration source = arrow::compute::Declaration({"source",
                        arrow::compute::SourceNodeOptions{data.schema,
                                          data.gen(/*parallel=*/true, /*slow=*/false)},
                        "custom_source_label"});
      arrow::compute::Declaration sink = arrow::compute::Declaration({"sink", arrow::compute::SinkNodeOptions{&sink_gen}, "custom_sink_label"});
