...
TEST(Decimal256Test, TestComparators) {
  ...
  for (size_t i = 0; i < num_values; ++i) {
    Decimal256 left(::arrow::BitUtil::FromLittleEndian(kSortedDecimal256Bits[i]));
    for (size_t j = 0; j < num_values; ++j) {
      Decimal256 right(::arrow::BitUtil::FromLittleEndian(kSortedDecimal256Bits[j]));
...
TEST(Decimal256Test, TestToBytesRoundTrip) {
  for (const std::array<uint64_t, 4>& bits : kSortedDecimal256Bits) {
    Decimal256 decimal(::arrow::BitUtil::FromLittleEndian(bits));
...
TYPED_TEST(Decimal256Test, ConstructibleFromAnyIntegerType) {
  ...
  EXPECT_EQ(UInt64Array({42, 0, 0, 0}),
            ::arrow::BitUtil::ToLittleEndian(value.native_endian_array()));

  TypeParam max = std::numeric_limits<TypeParam>::max();
  Decimal256 max_value(max);
  EXPECT_EQ(UInt64Array({static_cast<uint64_t>(max), 0, 0, 0}),
            ::arrow::BitUtil::ToLittleEndian(max_value.native_endian_array()));
  ...
  EXPECT_EQ(UInt64Array({static_cast<uint64_t>(min), high_bits, high_bits, high_bits}),
            ::arrow::BitUtil::ToLittleEndian(min_value.native_endian_array()));
}
