...
TEST(TestRleUtil, VisitMergedRuns) {
  ...
  ASSERT_OK_AND_ASSIGN(std::shared_ptr<Array> left_array,
                       RunLengthEncodedArray::Make(left_run_ends, left_child, 1026));
  ASSERT_OK_AND_ASSIGN(std::shared_ptr<Array> right_array,
                       RunLengthEncodedArray::Make(right_run_ends, right_child, 2026));
  ...
  rle_util::VisitMergedRuns(
      ArraySpan(*left_array->data()), ArraySpan(*left_array->data()),
