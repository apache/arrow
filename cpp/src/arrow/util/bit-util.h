...
class BitmapWriter {
 public:
  ...
  void Set() { current_byte_ |= BitUtil::kBitmask[bit_offset_ % 8]; }

  void Clear() { current_byte_ &= ~(BitUtil::kBitmask[bit_offset_ % 8]); }
  ...
  int64_t NumValues(int64_t start_offset) {
    return bit_offset_ + byte_offset_ * 8 - start_offset; }
