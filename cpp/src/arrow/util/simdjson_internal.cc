...
Status PrettyPrintJsonValueImpl(simdjson::ondemand::value value, std::string* out,
                                int indent) {
  ...
  switch (type) {
    case simdjson::ondemand::json_type::object: {
      ...
      for (auto field_result : object) {
        ...
        RETURN_NOT_OK(PrettyPrintJsonValueImpl(field_value, out, indent + 1));
      }
    ...
    case simdjson::ondemand::json_type::array: {
      ...
      for (auto element_result : array) {
        ...
        RETURN_NOT_OK(PrettyPrintJsonValueImpl(element, out, indent + 1));
      }
...
Result<std::string> PrettyPrintJsonValue(simdjson::ondemand::value value, int indent) {
  std::string out;
  RETURN_NOT_OK(PrettyPrintJsonValueImpl(value, &out, indent));
  return out;
}
