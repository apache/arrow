...
print('constexpr uint8_t kPdepTable[1 << kLookupBits][1 << kLookupBits] = {')
print(' ', end = '')
...
inline uint64_t DepositBitsSoftware(uint64_t bitmap, uint64_t mask) {
  // A software emulation of _pdep_u64
  ...
  while (mask != 0) {
    const uint64_t value = kPdepTable[mask & kLookupMask][bitmap & kLookupMask];
