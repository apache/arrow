...
absl::string_view BinaryBuilder::GetView(int64_t i) const {
  const int32_t* offsets = offsets_builder_.data();
  int32_t offset = offsets[i];
...
absl::string_view FixedSizeBinaryBuilder::GetView(int64_t i) const {
  const uint8_t* data_ptr = byte_builder_.data();
  return absl::string_view(reinterpret_cast<const char*>(data_ptr + i * byte_width_),
                           byte_width_);
}
