...
struct ChunkResolver {
  ...
  inline ChunkLocation Resolve(const int64_t index) const {
    ...
    const bool cache_hit = (index >= offsets_[static_cast<size_t>(cached_chunk)] &&
                            index < offsets_[static_cast<size_t>(cached_chunk) + 1]);
 ...
 protected:
  ...
  inline int64_t Bisect(const int64_t index) const {
    ...
    while (n > 1) {
      ...
      if (index >= offsets_[static_cast<size_t>(mid)]) {
        lo = mid;
        n -= m;
      } else {
        n = m;
