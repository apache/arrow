...
Status FuzzIpcFile(const uint8_t* data, int64_t size) {
  ...
  std::optional<Result<IpcReadResult>> maybe_stream_result;
  ...
  for (const bool pre_buffer : {false, true}) {
    ...
    if (maybe_file_result.ok()) {
      ...
      if (!maybe_stream_result.has_value()) {
        maybe_stream_result = do_stream_read();
        final_status &= maybe_stream_result->status();
      }
      if (maybe_stream_result->ok()) {
        if (maybe_file_result->schema->Equals((**maybe_stream_result).schema,
                                              /*check_metadata=*/true)) {
          ...
          // files.
          ARROW_CHECK_OK(CompareFuzzBatches(maybe_file_result->batches,
                                            (**maybe_stream_result).batches));
        } else {
