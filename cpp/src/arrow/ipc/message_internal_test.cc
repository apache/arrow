...
TEST(TestMessageInternal, TestByteIdentical) {
  ...
  // Canonical output captured from current s390x build (big-endian ensures endianness shims)
  ...
  if (out_buffer->size() != expected_buffer.size()) {
    ADD_FAILURE() << "Schema message bytes size mismatch: expected "
                  << expected_buffer.size() << " got " << out_buffer->size()
                  << "\nexpected hex: "
                  << arrow::HexEncode(expected_buffer.data(), expected_buffer.size())
                  << "\nactual hex  : "
                  << arrow::HexEncode(out_buffer->data(), out_buffer->size());
    return;
  }
  if (!out_buffer->Equals(expected_buffer)) {
    ADD_FAILURE() << "Schema message bytes differ from canonical reference\nexpected hex: "
                  << arrow::HexEncode(expected_buffer.data(), expected_buffer.size())
                  << "\nactual hex  : "
                  << arrow::HexEncode(out_buffer->data(), out_buffer->size());
    return;
