...
class ARROW_EXPORT RowGroupBuffer : public InputStream, public Seekable {
 public:
  /// Necessary because we hold a std::unique_ptr
  ~RowGroupBuffer() override;
  ...
  /// \param[in] nbytes the extent of bytes to read. The file should have
  /// sufficient bytes available
  static std::shared_ptr<InputStream> GetStream(std::shared_ptr<RowGroupBuffer> file,
                                                int64_t file_offset, int64_t nbytes);
 ...
 private:
  struct ARROW_NO_EXPORT RowGroupBufferImpl;
  std::unique_ptr<RowGroupBufferImpl> interface_impl_;
};
