...
Status GetIsInKernel(FunctionContext* ctx,
...
#define ISIN_CASE(InType)                                                \
  case InType::type_id:                                                               \
    kernel.reset(new                                                                  \
                 typename HashBinaryKernelTraits<InType>::HashBinaryKernelImpl( \
...
Status IsIn(FunctionContext* context, const Datum& left, const Datum& right, Datum* out) {
    ...
    RETURN_NOT_OK (detail::InvokeBinaryArrayKernel(context, kernel.get(), left, right, out));
    return Status::OK();
}
