...
struct XxHashScalar {
  static Status ExecPrimitive(KernelContext* ctx, const ExecSpan& input_arg,
    ...
    out->value =
        ArrayData{uint64(), input_len, {input_data.GetBuffer(0), std::move(hash_buffer)}};
    return Status::OK();
  }
...
std::shared_ptr<ScalarFunction> RegisterKernelsFastHash32() {
  ...
  for (const auto nested_type :
    ...
    DCHECK_OK(fn_fast_hash_32->AddKernel({InputType(nested_type)}, OutputType(uint32()),
                                         FastHashScalar<UInt32Type>::Exec));
  }
