...
struct RunLengthEncodeGenerator {
  ...
  static Status Exec(KernelContext* ctx, const ExecBatch& batch, Datum* output) {
    ...
    ARROW_ASSIGN_OR_RAISE(auto values_buffer, AllocateBuffer(num_values_output * sizeof(CType), pool));
    ARROW_ASSIGN_OR_RAISE(auto indices_buffer, AllocateBuffer(num_values_output * sizeof(uint64_t), pool));

    auto output_array_data = ArrayData::Make(/* TODO */ input_data->type, input_data->length);
    ...
    for (int64_t input_position = 0; input_position < input_data->length; input_position++) {
      Element previous_element = element;
...
void RegisterVectorRunLengthEncode(FunctionRegistry* registry) {
  ...
  for (const auto& ty : NumericTypes()) {
    ...
    auto sig = KernelSignature::Make({InputType(ty, ValueDescr::ARRAY)}, OutputType([](KernelContext*, const std::vector<ValueDescr>& descrs) -> Result<ValueDescr> {
                                       return descrs[0];
                                   }));
