 ...
 private:
  ...
  Status SortInternal() {
    ...
    if (num_chunks == 0) {
      ...
      *output_ =
          NullLikePartition::fromCounts(indices_, 0, 0, 0, null_placement_);
      return Status::OK();
    }
    ...
    if (sorted.size() > 1) {
      ...
      // Reverse everything
      sorted.resize(1);
      sorted[0] = chunk_sorted[0].TranslateTo(chunked_indices_begin, indices_.data());
...
class RecordBatchColumnSorter {
 public:
  ...
  virtual NullLikePartition SortRange(std::span<uint64_t> indices,
                                                  int64_t offset) = 0;
...
class ConcreteRecordBatchColumnSorter : public RecordBatchColumnSorter {
 public:
  ...
  NullLikePartition SortRange(std::span<uint64_t> indices,
...
class ConcreteRecordBatchColumnSorter<NullType> : public RecordBatchColumnSorter {
 public:
  ...
  NullLikePartition SortRange(std::span<uint64_t> indices, int64_t offset) {
    ...
    return NullLikePartition::fromCounts(indices, 0, 0, indices.size(),
                                                     null_placement_);
  }
...
#undef VISIT
 ...
 private:
  ...
  template <typename Type>
  NullLikePartition PartitionNullsInternal(
