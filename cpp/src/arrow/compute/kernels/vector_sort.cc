...
struct LocationContainer {
 public:
  LocationContainer() {}
  LocationContainer(uint64_t index) : index_(index), chunk_location_({-1, -1}) {}
  uint64_t index_;
  ...
  struct ResolvedSortKey {
    ...
    ResolvedChunk<ArrayType> GetChunk(LocationContainer& location_container) const {
      if (location_container.chunk_location_.chunk_index == -1){
        location_container.chunk_location_ = resolver.ResolveChunkLocation(location_container.index_);
      }
...
#undef VISIT
  ...
  Status SelectKthInternal() {
    ...
    ARROW_ASSIGN_OR_RAISE(auto row_indices,
    ...
    ARROW_ASSIGN_OR_RAISE(auto take_indices, MakeMutableUInt64Array(uint64(), k_,
                                                                    ctx_->memory_pool()));
