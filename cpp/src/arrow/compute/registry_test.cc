...
TEST_F(TestRegistry, Basics) {
  ...
  std::shared_ptr<Function> func =
      std::make_shared<ScalarFunction>("f1", Arity::Unary(), /*doc=*/FunctionDoc::Empty());
  ASSERT_OK(registry_->AddFunction(func));
  ASSERT_EQ(1, registry_->num_functions());

  func = std::make_shared<VectorFunction>("f0", Arity::Binary(), /*doc=*/FunctionDoc::Empty());
  ASSERT_OK(registry_->AddFunction(func));
  ...
  func = std::make_shared<ScalarAggregateFunction>("f1", Arity::Unary(), /*doc=*/FunctionDoc::Empty());
