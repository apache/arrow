...
class SwissJoin : public HashJoinImpl {
 ...
 private:
  ...
  Status BuildHashTableAsync(int64_t thread_id) {
    ...
    std::vector<KeyEncoder::KeyColumnMetadata> key_types;
    ...
    std::vector<KeyEncoder::KeyColumnMetadata> payload_types;
  ...
  Status ScanTask(size_t thread_id, int64_t task_id) {
    ...
    for (int64_t mini_batch_start = start_row; mini_batch_start < end_row;) {
      ...
      //
      uint32_t first_key_id = hash_table_.payload_id_to_key_id(mini_batch_start);
      uint32_t last_key_id =
          hash_table_.payload_id_to_key_id(mini_batch_start + mini_batch_size_next - 1);
