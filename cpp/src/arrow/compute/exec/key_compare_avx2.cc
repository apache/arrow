...
uint32_t KeyCompare::CompareFixedLength_UpTo8B_avx2(
  ...
  for (uint32_t i = 0; i < num_rows / unroll; ++i) {
    auto key_left = _mm256_i64gather_epi64(reinterpret_cast<const long long int*>(rows_left),
                                           offset_left, 1);
    ...
    auto key_right = _mm256_i32gather_epi64(reinterpret_cast<const long long int*>(rows_right),
                                            offset_right, 1);
