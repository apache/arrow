...
struct TableSourceNode : ExecNode {
  TableSourceNode(ExecPlan* plan, std::shared_ptr<Schema> output_schema, 
    std::shared_ptr<arrow::Table> table) : ExecNode(plan, {}, {}, 
    std::move(output_schema), /*num_outputs=*/1), table_(std::move(table)) {}

  static Result<ExecNode*> Make(ExecPlan* plan, std::vector<ExecNode*> inputs,
                                const ExecNodeOptions& options) {
    RETURN_NOT_OK(ValidateExecNodeInputs(plan, inputs, 0, "TableSourceNode"));
    const auto& source_options = checked_cast<const TableSourceNodeOptions&>(options);
    return plan->EmplaceNode<TableSourceNode>(plan, source_options.table.schema(),
                                         source_options.table);
  }
