...
struct MemoStore {
  struct Entry {
    ...
    // Batch associated with the entry (perf is probably OK for this; batches change rarely)
  ...
  void store(const std::shared_ptr<RecordBatch> &batch, row_index_t row, int64_t time, KeyType key) {
    auto &e=_entries[key];
...
class InputState
{
  ...
  row_index_t _latest_ref_row=0; // must be < _queue.front()->num_rows() if _queue is non-empty
...
public:
  InputState(const std::shared_ptr<arrow::Schema> &schema,
    ...
    _schema(schema),
    _time_col_index(schema->GetFieldIndex(time_col_name)), //TODO: handle missing field name
    _key_col_index(schema->GetFieldIndex(key_col_name)) //TODO: handle missing field name
  { /*nothing else*/ }
  ...
  row_index_t get_latest_row() const {
    return _latest_ref_row;
  }

  bool empty() const {
    if(_latest_ref_row>0) return false; // cannot be empty if ref row is >0 -- can avoid slow queue lock below
        return _queue.empty();
  }
  ...
  KeyType get_latest_key() const {
    return _queue.unsync_front()->column_data(_key_col_index)->GetValues<KeyType>(1)[_latest_ref_row];
  }
  int64_t get_latest_time()const {
    return _queue.unsync_front()->column_data(_time_col_index)->GetValues<int64_t>(1)[_latest_ref_row];
  }

  bool finished() const {
        return _batches_processed==_total_batches;
    }

    bool advance() {
        // Returns true if able to advance, false if not.
        bool have_active_batch=(_latest_ref_row>0/*short circuit the lock on the queue*/) || !_queue.empty();
        if(have_active_batch) {
            ...
            if(++_latest_ref_row>=_queue.unsync_front()->num_rows()) {
                ...
                if(have_active_batch) assert(_queue.unsync_front()->num_rows()>0); // empty batches disallowed
            }
        }
        return have_active_batch;
    }

    // Advance the data to be immediately past the specified TS, updating latest and latest_ref_row to
    // the latest data prior to that immediate just past
    ...
    bool advance_and_memoize(int64_t ts) {
        ...
        do {
            ...
            if(latest_time<=ts) // if advance() returns true, then the latest_ts must also be valid
  ...
  util::optional<const MemoStore::Entry*> get_memo_entry_for_key(KeyType key) {
    ...
    if(_wildcard_key.has_value())
      r=_memo.get_entry_for_key(*_wildcard_key);
    return r;
  }
  ...
  const std::shared_ptr<Schema> &get_schema() const { return _schema; }
    ...
    void set_total_batches(int n) {
        assert(n>=0); // not sure why arrow uses a signed int for this, but it should be >=0
...
class CompositeReferenceTable {
  ...
  void add_record_batch_ref(const std::shared_ptr<RecordBatch> &ref) {
        if(!_ptr2ref.count((uintptr_t)ref.get()))
...
public:
  ...
  void emplace(std::vector<std::unique_ptr<InputState>> &in,int64_t tolerance) {
    ...
    assert(!in[0]->empty());
    const std::shared_ptr<arrow::RecordBatch> &lhs_latest_batch=in[0]->get_latest_batch();
    ...
    if(0==lhs_latest_row) {
      ...
      assert(lhs_latest_batch->num_rows()<=MAX_ROWS_PER_BATCH); //TODO: better error handling
      row_index_t new_batch_size=lhs_latest_batch->num_rows();
      row_index_t new_capacity=_rows.size()+new_batch_size;
      if(_rows.capacity()<new_capacity)
        _rows.reserve(new_capacity);
      //cerr << "new_batch_size=" << new_batch_size << " old_size=" << _rows.size() << " new_capacity=" << _rows.capacity() << endl;
    }
    ...
    for(size_t i=1;i<in.size();++i) {
      util::optional<const MemoStore::Entry*> opt_entry=in[i]->get_memo_entry_for_key(key);
...
private:
    ...
    std::shared_ptr<Array> materialize_primitive_column(size_t i_table,size_t i_col) {
        ...
        for(row_index_t i_row=0;i_row<_rows.size();++i_row) {
            ...
            if(ref._batch)
                builder[i_row]=ref._batch->column_data(i_col)->template GetValues<PrimitiveType>(1)[ref._row];
            //TODO: set null value if ref._batch is null -- currently we don't due to API limitations
            //of the builders.
        }
...
public:
  ...
  std::shared_ptr<RecordBatch> materialize(const std::shared_ptr<arrow::Schema> &output_schema,
        ...
        // Count output columns (dbg sanitycheck)
        {
            ...
            for(const auto &s:state)
                n_out_cols+=s->get_schema()->num_fields();
            n_out_cols-=(state.size()-1)*2; // remove column indices for key and time cols on RHS
        ...
        for(size_t i_table=0;i_table<_n_tables;++i_table) {
          int n_src_cols=state.at(i_table)->get_schema()->num_fields();
          {
            for(col_index_t i_src_col=0;i_src_col<n_src_cols;++i_src_col) {
              util::optional<size_t> i_dst_col_opt=state[i_table]->map_src_to_dst(i_src_col);
              ...
              if(field_type->Equals(i32_type)) {
                arrays.at(i_dst_col)=materialize_primitive_column<arrow::Int32Builder,int32_t>(i_table,i_src_col);
              } else if(field_type->Equals(i64_type)) {
                arrays.at(i_dst_col)=materialize_primitive_column<arrow::Int64Builder,int64_t>(i_table,i_src_col);
              } else if(field_type->Equals(f64_type)) {
                arrays.at(i_dst_col)=materialize_primitive_column<arrow::DoubleBuilder,double>(i_table,i_src_col);
              } else {
        ...
        std::shared_ptr<arrow::RecordBatch> r=arrow::RecordBatch::Make(output_schema,(int64_t)n_rows,arrays);
...
class AsofJoinNode : public ExecNode {
    ...
    for(size_t i=1;i<_state.size();++i)
      {
        auto &rhs=*_state[i];
        if(!rhs.finished()) {
          // If RHS is finished, then we know it's up to date (but if it isn't, it might be up to date)
          if(rhs.empty()) return false; // RHS isn't finished, but is empty --> not up to date
          if(lhs_ts>=rhs.get_latest_time()) return false; // TS not up to date (and not finished)
        }
        }
    return true;
  }
  ...
  std::shared_ptr<RecordBatch> process_inner() {
    ...
    // Generate rows into the dst table until we either run out of data or hit the row limit, or run out of input
    for(;;) {
      // If LHS is finished or empty then there's nothing we can do here
      if(lhs.finished() || lhs.empty()) break;

      // Advance each of the RHS as far as possible to be up to date for the LHS timestep
    ...
    if(!lhs.empty())
      for(size_t i=1;i<_state.size();++i)
        _state[i]->remove_memo_entries_with_lesser_time(lhs.get_latest_time()-_options._tolerance);

    // Emit the batch
    std::shared_ptr<RecordBatch> r=dst.empty()?nullptr:dst.materialize(output_schema(),_state);
    return r;
  }
