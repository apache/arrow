...
_DEFAULT_MARKET_NUM_IDS = 5000
_DEFAULT_MARKET_NUM_FACTOR_IDS = 100
...
_DEFAULT_DETAIL_TICKER = np.array(
    ["A", "B"], dtype=_DTYPE_STRING)
_DEFAULT_DETAIL_CURRENCY_ID = np.array([10203040, 50607080], dtype=_DTYPE_ID)
_DEFAULT_DETAIL_GICS_CODE = np.array([10203040, 50607080], dtype=_DTYPE_ID)
...
class SimpleMidnightCalendar:
    """
    A simple midnight calendar. All days are trading days, and trading hours
    are midnight only, irrespective of holidays or time zone.
    """

    def is_trading_day(self, curr_date):
        """
        Returns True
        """
        return True

    def trading_begin_ns(self, curr_date):
        """
        Returns midnight
        """
        return 0
...
class SimpleNYSECalendar:
    """
    A simple NYSE calendar. Trading days are Monday through Friday, and trading
...
class SimpleDailyNYSECalendar:
    """
    A simple daily NYSE calendar. Trading days are Monday through Friday, and
    trading hours are midnight only, irrespective of holidays or time zone.
    """
...
def _get_id_dist(ids=_DEFAULT_MARKET_NUM_IDS, dist="all"):
    return _get_dist(dist)(_get_array(ids))

def _get_factor_id_dist(ids=_DEFAULT_MARKET_NUM_FACTOR_IDS, dist="all"):
    return _get_dist(dist)(_get_array(ids))
...
class DataGenerator:
    ...
    def generate(self, begin_date=None, end_date=None, seed=None, calendar=None,
        ...
        when the date range or trading date ends.
...
('value', [0.345584192064786, 0.8216181435011584, 0.33043707618338714])
        ...
        while True:
            if calendar.is_trading_day(curr_date):
                begin_time_ns = (
                        curr_date_ns + calendar.trading_begin_ns(curr_date))
                end_time_ns = (
...
class MarketDataGenerator:
    ...
    """
    A market data generator. See the generate(...) method for details.
    """

    @_doc(ids_default=_DEFAULT_MARKET_NUM_IDS)
    def generate(self, begin_date=None, end_date=None, seed=None, calendar=None,
            freq="200ms", time_dist="constant", ids=_DEFAULT_MARKET_NUM_IDS,
        ...
        Generates market data using these paremeters:
            ...
            seed        the seed to use for pseudorandom generation.
                        Defaults to a random seed.
            calendar    the calendar defining trading dates and hours.
                        Defaults to SimpleNYSECalendar().
            freq        the time frequency of periods to generate with.
...
class ModelStateDataGenerator:
    ...
    def generate(self, begin_date=None, end_date=None, seed=None,
        ...
        """
        Generates model-state data using these paremeters:
        ...
        >>> msdgen = ModelStateDataGenerator()
        >>> for batch in msdgen.generate("2020-01-01", "2020-01-01", seed=1):
        ...     for item in batch.to_pydict().items():
        ...
        ('id', [0])
        ('return_prev_1d_lag', [0.345584192064786])
        ('return_next_1d_lead', [0.599476654534323])
        ('variance', [0.338346476063111])
        ('volume', [484732503.5147238])
        ('market_cap', [-0.43637535848759307])
        ('factor_id', [0])
        ('price', [-0.14101551119463715])
        """
        freq = _max_freq([freq, "30T"])
        dists = [
            ("id", _get_id_dist()),
            ("return_prev_1d_lag", NormalDistribution()),
            ("return_next_1d_lead", NormalDistribution()),
            ("variance", SquaredNormalDistribution()),
            ("volume", SquaredNormalDistribution(0.0, 20000.0)),
            ("market_cap", NormalDistribution()),
            ("factor_id", _get_factor_id_dist()),
            ("price", NormalDistribution()),
        ]
        return self.dgen.generate(begin_date=begin_date, end_date=end_date,
            seed=seed, calendar=SimpleDailyNYSECalendar(), freq=freq,
            time_dist="constant", dists=dists, num_ids=_DEFAULT_MARKET_NUM_IDS,
            batch_size=batch_size)

class GlobalLoadingDataGenerator:
    def __init__(self):
        self.mdgen = MarketDataGenerator()

    def generate(self, begin_date=None, end_date=None, seed=None,
            freq="24h", batch_size=_DEFAULT_BATCH_SIZE):
        """
        Generates global-loading data using these paremeters:
        ...
        >>> gldgen = GlobalLoadingDataGenerator()
        >>> for batch in gldgen.generate("2020-01-01", "2020-01-01", seed=1):
        ...
        return self.mdgen.generate(begin_date=begin_date, end_date=end_date,
            seed=seed, calendar=SimpleDailyNYSECalendar(), freq=freq,
            time_dist="constant", ids=_DEFAULT_MARKET_NUM_IDS, ids_dist="all",
            cols=141, batch_size=batch_size)

class USLoadingDataGenerator:
    def __init__(self):
        self.mdgen = MarketDataGenerator()
    ...
    def generate(self, begin_date=None, end_date=None, seed=None,
            freq="24h", batch_size=_DEFAULT_BATCH_SIZE):
        """
        Generates US-loading data using these paremeters:
        ...
        >>> usldgen = USLoadingDataGenerator()
        >>> for batch in usldgen.generate("2020-01-01", "2020-01-01", seed=1):
        ...     for item in list(batch.to_pydict().items())[-10:]:
        ...         print((item[0], item[1][:1]))
        ...     break
        ... 
        ...
        freq = _max_freq([freq, "24h"])
        return self.mdgen.generate(begin_date=begin_date, end_date=end_date,
            seed=seed, calendar=SimpleDailyNYSECalendar(), freq="24h",
            time_dist="constant", ids=_DEFAULT_MARKET_NUM_IDS, ids_dist="all",
...
class ForecastDataGenerator:
    ...
    def generate(self, begin_date=None, end_date=None, seed=None,
        ...
        >>> fldgen = ForecastDataGenerator()
        >>> for batch in fldgen.generate("2020-01-01", "2020-01-01", seed=1):
        ...     for item in batch.to_pydict().items():
        ...         print((item[0], item[1][:1]))
        ...     break
        ... 
        ...
        ('id', [0])
        ('forecast', [0.345584192064786])
        """
        freq = _max_freq([freq, "30T"])
        return self.mdgen.generate(begin_date=begin_date, end_date=end_date,
            seed=seed, calendar=SimpleNYSECalendar(), freq=freq,
            time_dist="constant", ids=_DEFAULT_MARKET_NUM_IDS, ids_dist="all",
            cols=["forecast"], batch_size=batch_size)

class InstrumentDetailsDataGenerator:
    def __init__(self):
        self.dgen = DataGenerator()
    ...
    def generate(self, begin_date=None, end_date=None, seed=None,
            freq="24h", batch_size=_DEFAULT_BATCH_SIZE):
        """
        Generates instrument-details data using these paremeters:
        ...
        >>> iddgen = InstrumentDetailsDataGenerator()
        >>> for batch in iddgen.generate("2020-01-01", "2020-01-01", seed=1):
        ...     for item in batch.to_pydict().items():
        ...         print((item[0], item[1][:1]))
        ...     break
        ... 
        ('time', [Timestamp('2020-01-01 05:00:00+0000', tz='UTC')])
        ('id', [0])
        ('value', [0.345584192064786])
        ('instrument_exchange', ['N'])
        ('instrument_name', ['American Electric Power'])
        ('instrument_country_name', ['United States'])
        ('instrument_ticker', ['A'])
        ('instrument_currency_id', [10203040])
        ('instrument_gics_code', [10203040])
        ('instrument_country_id', [10203040])
        ('instrument_type_id', [10203040])
        """
        freq = _max_freq([freq, "24h"])
        dists = [
            ("id", _get_id_dist()),
            ...
            ("instrument_country_name",
                _get_id_dist(_DEFAULT_DETAIL_COUNTRY_NAME)),
            ("instrument_ticker",
                _get_id_dist(_DEFAULT_DETAIL_TICKER)),
            ("instrument_currency_id",
                _get_id_dist(_DEFAULT_DETAIL_CURRENCY_ID)),
            ("instrument_gics_code",
                _get_id_dist(_DEFAULT_DETAIL_GICS_CODE)),
...
class ConditionerDataGenerator:
    def __init__(self):
        self.dgen = DataGenerator()
    ...
    def generate(self, begin_date=None, end_date=None, seed=None,
        ...
        >>> cdgen = ConditionerDataGenerator()
