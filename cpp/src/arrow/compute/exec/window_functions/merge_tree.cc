...
void MergeTree::Cascade(int level, int64_t pos, RangeQueryState* result) const {
  ...
  bool has_right_child = (node_begin + (1LL << (level - 1))) < num_rows_;
  result->pos[1] = has_right_child ? RangeQueryState::PosFromNodeAndLength(
                                         level - 1, node * 2 + 1, local_rank)
                                   : RangeQueryState::kEmpty;
}

void MergeTree::Cascade_Begin(int level, int64_t begin, int64_t* lbegin,
                              int64_t* rbegin) const {
  if (begin == num_rows_ || begin == RangeQueryState::kEmpty) {
    *lbegin = *rbegin = RangeQueryState::kEmpty;
  }
  ARROW_DCHECK(begin >= 0 && begin < num_rows_);
  int64_t node_begin_mask = ~((static_cast<int64_t>(1) << level) - 1);
  int64_t half_node_begin = (begin & node_begin_mask) / 2;
  int64_t half_node_length = (1LL << (level - 1));
  int64_t rank = BitVectorNavigator::Rank(begin, level_bitvecs_[level].data(),
                                          level_popcounts_[level].data());
  *lbegin = (begin - rank) + half_node_begin;
  *rbegin = rank + half_node_begin + half_node_length;

  int64_t lnode_end = half_node_begin * 2 + half_node_length;
  int64_t rnode_end = lnode_end + half_node_length;
  if (*lbegin >= lnode_end) {
    *lbegin = RangeQueryState::kEmpty;
  }
  if (*rbegin >= rnode_end || *rbegin >= num_rows_) {
    *rbegin = RangeQueryState::kEmpty;
  }
}

void MergeTree::Cascade_End(int level, int64_t end, int64_t* lend, int64_t* rend) const {
  if (end == 0 || end == RangeQueryState::kEmpty) {
    *lend = *rend = RangeQueryState::kEmpty;
    return;
  }
  ARROW_DCHECK(end > 0 && end <= num_rows_);
  int64_t node_begin_mask = ~((static_cast<int64_t>(1) << level) - 1);
  int64_t half_node_begin = ((end - 1) & node_begin_mask) / 2;
  int64_t half_node_length = (1LL << (level - 1));
  int64_t rank = BitVectorNavigator::RankNext(end - 1, level_bitvecs_[level].data(),
                                              level_popcounts_[level].data());
  *lend = (end - rank) + half_node_begin;
  *rend = rank + half_node_begin + half_node_length;

  int64_t lnode_begin = half_node_begin * 2;
  int64_t rnode_begin = lnode_begin + half_node_length;
  if (*lend == lnode_begin) {
    *lend = RangeQueryState::kEmpty;
  }
  if (*rend == rnode_begin) {
    *rend = RangeQueryState::kEmpty;
  }
}

int64_t MergeTree::Cascade_Pos(int level, int64_t pos) const {
  ARROW_DCHECK(pos >= 0 && pos < num_rows_);
  int64_t node_begin_mask = ~((static_cast<int64_t>(1) << level) - 1);
  int64_t half_node_begin = (pos & node_begin_mask) / 2;
  int64_t half_node_length = (1LL << (level - 1));
  int64_t rank = BitVectorNavigator::Rank(pos, level_bitvecs_[level].data(),
                                          level_popcounts_[level].data());
  bool rchild = bit_util::GetBit(
      reinterpret_cast<const uint8_t*>(level_bitvecs_[level].data()), pos);
  return rchild ? (rank + half_node_begin + half_node_length)
                : (pos - rank + half_node_begin);
}
...
bool MergeTree::NodeFullyInsideRange(int level, int64_t node, int64_t begin,
  ...
  int64_t node_begin = node << level;
  int64_t node_end =
      std::min(num_rows_, node_begin + (static_cast<int64_t>(1LL) << level));
