...
class TpchTableGenerator {
 public:
  ...
  virtual Status Init(std::vector<std::string> columns, float scale_factor,
...
static constexpr uint32_t kCurrentDate =
...
class PartAndPartSupplierGenerator {
 public:
  Status Init(size_t num_threads, int64_t batch_size, float scale_factor) {
    if (!inited_) {
      inited_ = true;
...
class SupplierGenerator : public TpchTableGenerator {
 public:
  Status Init(std::vector<std::string> columns, float scale_factor,
  ...
  Status StartProducing(size_t num_threads, OutputBatchCallback output_callback,
    ...
    arrow_vendored::pcg_extras::seed_seq_from<std::random_device> seq;
    thread_local_data_.resize(num_threads);
    for (ThreadLocalData& tld : thread_local_data_) tld.rng.seed(seq);
...
class NationGenerator : public TpchTableGenerator {
 ...
 private:
  ...
  const int32_t kNationKey[kRowCount] = {0,  1,  2,  3,  4,  5,  6,  7,  8,
  ...
  const int32_t kRegionKey[kRowCount] = {0, 1, 1, 1, 4, 0, 3, 3, 2, 2, 4, 4, 2,
...
class RegionGenerator : public TpchTableGenerator {
 public:
  ...
  const char* kRegionNames[kRowCount] = {"AFRICA", "AMERICA", "ASIA", "EUROPE",
                                         "MIDDLE EAST"};
