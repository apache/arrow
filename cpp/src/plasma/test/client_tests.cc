...
class TestPlasmaStore : public ::testing::Test {
 public:
  ...
  void CreateObject(PlasmaClient& client, const ObjectID& object_id,
    ...
    ASSERT_OK(client.Create(object_id, data.size(), metadata.data(), metadata.size(),
                            &data_buffer));
...
TEST_F(TestPlasmaStore, BatchNotificationTest) {
  ...
  ASSERT_OK(local_client.CreateAndSealBatch(object_ids, data, metadata));

  ObjectID object_id = random_object_id();
  int64_t data_size = 0;
  int64_t metadata_size = 0;
  ASSERT_OK(local_client2.GetNotification(fd, &object_id, &data_size, &metadata_size));
...
TEST_F(TestPlasmaStore, SetQuotaBasicTest) {
  ...
  ASSERT_OK(client_.SetClientOptions("client1", 5 * 1024 * 1024));
  std::vector<uint8_t> big_data(3 * 1024 * 1024, 0);

  // First object fits
  CreateObject(client_, id1, {42}, big_data, true);
  ASSERT_OK(client_.Contains(id1, &has_object));
  ASSERT_TRUE(has_object);

  // Evicts first object
  CreateObject(client_, id2, {42}, big_data, true);
  ASSERT_OK(client_.Contains(id2, &has_object));
  ASSERT_TRUE(has_object);
  ASSERT_OK(client_.Contains(id1, &has_object));
...
TEST_F(TestPlasmaStore, SetQuotaProtectsOtherClients) {
  ...
  ASSERT_OK(client2_.SetClientOptions("client2", 5 * 1024 * 1024));
  for (int i = 0; i < 10; i++) {
    CreateObject(client2_, random_object_id(), {42}, big_data, true);
  }
...
TEST_F(TestPlasmaStore, SetQuotaDemotesPinnedObjectsToGlobalLRU) {
  ...
  ASSERT_OK(client_.Release(id1));
  ASSERT_OK(client_.Release(id2));
...
TEST_F(TestPlasmaStore, SetQuotaDemoteDisconnectToGlobalLRU) {
  ...
  ASSERT_OK(local_client.Connect(store_socket_name_, ""));
  ASSERT_OK(local_client.SetClientOptions("local", 5 * 1024 * 1024));
...
TEST_F(TestPlasmaStore, SetQuotaCleanupObjectMetadata) {
  ...
  // release id0
  ASSERT_OK(local_client.Release(id0));
  ASSERT_TRUE(client_.DebugString().find("(global lru) num objects: 1") !=
              std::string::npos);

  // delete everything
  ASSERT_OK(local_client.Delete(id0));
  ASSERT_OK(local_client.Delete(id2));
  ASSERT_OK(local_client.Delete(id3));
  ASSERT_OK(local_client.Release(id3));
...
TEST_F(TestPlasmaStore, RefreshLRUTest) {
  ...
  for (const auto& object_id : object_ids) {
    ...
    ASSERT_OK(client_.Contains(object_ids[0], &has_object));
    ASSERT_TRUE(has_object);
  }
  ...
  ASSERT_OK(client_.Contains(object_ids[1], &has_object));
  ASSERT_FALSE(has_object);

  ASSERT_OK(client_.Refresh({object_ids[2], object_ids[3]}));
  ...
  ASSERT_OK(client_.Contains(object_ids[2], &has_object));
  ASSERT_TRUE(has_object);
  ASSERT_OK(client_.Contains(object_ids[3], &has_object));
  ASSERT_TRUE(has_object);

  // the next object in LRU order is evicted
  ASSERT_OK(client_.Contains(object_ids[4], &has_object));
  ASSERT_FALSE(has_object);
}
...
TEST_F(TestPlasmaStore, BatchCreateTest) {
  ...
  ASSERT_OK(client_.CreateAndSealBatch(object_ids, data, metadata));
...
TEST_F(TestPlasmaStore, GetGPUTest) {
  ...
  std::shared_ptr<Buffer> data_buffer;
  std::shared_ptr<CudaBuffer> gpu_buffer;
  ASSERT_OK(client_.Create(object_id, data_size, metadata, metadata_size, &data_buffer,
                           kGpuDeviceNumber));
...
TEST_F(TestPlasmaStore, RepeatlyCreateGPUTest) {
  ...
  for (int64_t i = 0; i < object_num; i++) {
    ...
    std::shared_ptr<Buffer> data;
    ASSERT_OK(client_.Create(object_id, data_size, 0, 0, &data, kGpuDeviceNumber));
    ASSERT_OK(client_.Seal(object_id));
    ASSERT_OK(client_.Release(object_id));
  }
...
TEST_F(TestPlasmaStore, GPUBufferLifetime) {
  ...
  ASSERT_OK(client_.Get(&object_id, 1, -1, &get_buff_1));
  ObjectBuffer get_buff_2;
  ASSERT_OK(client_.Get(&object_id, 1, -1, &get_buff_2));
  ASSERT_OK(client_.Release(object_id));
  ASSERT_OK(client_.Release(object_id));

  ObjectBuffer get_buff_3;
  ASSERT_OK(client_.Get(&object_id, 1, -1, &get_buff_3));
  ASSERT_OK(client_.Release(object_id));
