...
class ThriftConverter {
 public:
  ...
  auto ToString(std::string prefix, uint64_t v, int len) {
    uint64_t x = ::arrow::bit_util::FromBigEndian(v);
    return prefix + std::string(reinterpret_cast<const char*>(&x), len);
  }
    ...
    if (type) {
      switch (*type) {
        ...
        case format3::Type::FLOAT:
          if (s->max4()) out.max_value = ToString(*s->max4());
          if (s->min4()) out.min_value = ToString(*s->min4());
          break;
        case format3::Type::INT64:
        case format3::Type::DOUBLE:
          if (s->max8()) out.max_value = ToString(*s->max8());
          if (s->min8()) out.min_value = ToString(*s->min8());
          break;
        default: {
          int min_len = s->lens() & 0xf, max_len = (s->lens() >> 4) & 0xf;
          auto prefix = s->prefix() ? s->prefix()->str() : "";
          out.max_value = ToString(prefix, *s->max8(), max_len);
          out.min_value = ToString(prefix, *s->min8(), min_len);
        }
      }
    }
    return out;
