    ...
    if (fast_copy) {
      ...
      for (int i = 0; i < metadata_->num_columns(); ++i) {
        ...
        if (true) {
          ...
          // TODO(HuaHuaY): rewrite column chunk with new encoding, compression, etc.
        }
      }
    }
  }
...
class SingleFileRewriter {
 public:
  ...
  void WriteData(bool fast_copy, RowGroupMetaDataBuilder* rg_metadata_builder,
    ...
    rewriter.WriteData(fast_copy && row_group_metadata->file_offset() != 0,
                       rg_metadata_builder, page_index_builder, total_bytes_written);
    ++current_row_group_index_;
  }

  bool HasData() { return current_row_group_index_ < metadata_->num_row_groups(); }
...
class ConcatRewriter {
 public:
  explicit ConcatRewriter(std::vector<std::unique_ptr<SingleFileRewriter>> rewriters)
    ...
    auto* schema = rewriters_[0]->schema();
    for (size_t i = 1; i < rewriters_.size(); ++i) {
      if (!schema->Equals(*rewriters_[i]->schema())) {
  ...
  void WriteData(bool fast_copy, RowGroupMetaDataBuilder* rg_metadata_builder,
                 PageIndexBuilder* page_index_builder, int64_t& total_bytes_written) {
    rewriters_[current_rewriter_index_]->WriteData(
  ...
  bool HasData() {
    ...
    return current_rewriter_index_ < rewriters_.size();
  }

  void Close() {
    for (size_t i = current_rewriter_index_; i < rewriters_.size(); ++i) {
      rewriters_[i]->Close();
    }
  }
 ...
 private:
  std::vector<std::unique_ptr<SingleFileRewriter>> rewriters_;
...
class ExtendRewriter {
 public:
  ...
  void WriteData(RowGroupMetaDataBuilder* rg_metadata_builder,
                 PageIndexBuilder* page_index_builder, int64_t& total_bytes_written) {
    if (rewriters_.size() == 1) {
      rewriters_[0]->WriteData(true, rg_metadata_builder, page_index_builder,
                               total_bytes_written);
      return;
    }
    for (auto& rewriter : rewriters_) {
      rewriter->WriteData(false, rg_metadata_builder, page_index_builder,
                          total_bytes_written);
    }
  }

  bool HasData() { return rewriters_[0]->HasData(); }
...
class GeneratedFile : public ParquetFileRewriter::Contents {
 public:
  ...
  void Rewrite() override {
    while (rewriter_->HasData()) {
      auto* rg_metadata_builder = new_file_metadata_->AppendRowGroup();
      page_index_builder_->AppendRowGroup();
      int64_t total_bytes_written = 0;
      rewriter_->WriteData(rg_metadata_builder, page_index_builder_.get(),
                           total_bytes_written);
    ...
    new_file_metadata_->SetPageIndexLocation(location);

    auto file_metadata = new_file_metadata_->Finish(sink_metadata_);
 ...
 private:
  ...
  std::unique_ptr<FileMetaDataBuilder> new_file_metadata_;
