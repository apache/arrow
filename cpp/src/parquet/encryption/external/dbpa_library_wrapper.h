...
class PARQUET_EXPORT DBPALibraryWrapper : public DataBatchProtectionAgentInterface {
 ...
 public:
  ...
  // While the wrapped_agent_ would automatically be destroyed when this object is
  // destroyed, we need to explicitly destroy **before** we are able to close the shared
  // library. Doing it in a different order, may cause issues, as by unloading the
  // library may cause the class definition to be unloaded before the destructor
  // completes, and that is likely to cause issues (such as a segfault).
  ~DBPALibraryWrapper();

  // Decorator implementation of init method
  inline void init(std::string column_name,
