...
TEST_F(DBPAExecutorTest, InitForwardsToWrappedAgent) {
  ...
  std::map<std::string, std::string> connection_config = {{"key", "value"},
                                                          {"server", "localhost"}};
  ...
  EXPECT_NO_THROW(executor_->init(column_name, connection_config, app_context,
                                  column_key_id, data_type, std::nullopt,
                                  compression_type, column_encryption_metadata));
  ...
  EXPECT_EQ(mock_agent_ptr_->last_init_column_encryption_metadata_.value().at("metaKey"),
            "metaValue");
}

TEST_F(DBPAExecutorTest, EncryptForwardsToWrappedAgent) {
  // Initialize the executor first
  executor_->init("test_column", {}, "test_context", "test_key_id", Type::type::INT32,
                  std::nullopt, CompressionCodec::type::UNCOMPRESSED, std::nullopt);
...
TEST_F(DBPAExecutorTest, MultipleCallsAreProperlyForwarded) {
  ...
  executor_->init("column1", {{"key1", "value1"}}, "context1", "key1", Type::type::INT32,
                  std::nullopt, CompressionCodec::type::UNCOMPRESSED, std::nullopt);
  ...
  EXPECT_EQ(mock_agent_ptr_->last_init_compression_type_,
            CompressionCodec::type::UNCOMPRESSED);
