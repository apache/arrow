...
WrappedKeyManager::WrappedKeyManager(std::shared_ptr<KmsClient> kms_client, bool wrapLocally,
  ...
  if (!wrapLocally && !kms_client->supportsServerSideWrapping()) {
    throw UnsupportedOperationException("KMS client doesn't support server-side wrapping");
  }
}

WrappedKeyManager::WrappedKeyRetriever::WrappedKeyRetriever(std::shared_ptr<KmsClient> kms_client,
							    bool unwrap_locally,
...
const std::string& WrappedKeyManager::WrappedKeyRetriever::GetKey(const std::string& key_metadata) {
  ...
  if (unwrap_locally_) {
    std::string wrapped_data_key = boost::beast::detail::base64_decode(encoded_wrapped_data_key);
    ...
    std::string master_key = boost::beast::detail::base64_decode(encoded_master_key);
    ...
    int64_t data_key_size = (int64_t)encryption->CalculatePlainSize((uint32_t)wrapped_data_key.size());
    std::shared_ptr<ResizableBuffer> data_key_buffer;
    PARQUET_THROW_NOT_OK(arrow::AllocateResizableBuffer(pool, data_key_size, &data_key_buffer));
    ...
    parquet_encryption::Decrypt(ParquetCipher::AES_GCM_V1, false,
    ...
    data_key_.assign(reinterpret_cast<char const*>(data_key_buffer->mutable_data()), DATA_KEY_SIZE);    
  } else {
...
std::shared_ptr<DecryptionKeyRetriever> WrappedKeyManager::getKeyRetriever() {
  return std::shared_ptr<DecryptionKeyRetriever>(new WrappedKeyRetriever(kms_client_,
									 wrap_locally_,
									 wrapped_key_store_,
									 file_id_));
}
...
// The metadata is comprised of the wrapped data key (encrypted with master key),
// and the identity of the master key.
std::unique_ptr<ParquetKey> WrappedKeyManager::generateKey(std::string &master_key_id) {
  ...
  if (wrap_locally_) {
    ...
    std::shared_ptr<EncryptionProperties> encryption =
      std::make_shared<EncryptionProperties>(ParquetCipher::AES_GCM_V1, "", "", "");
    auto pool = ::arrow::default_memory_pool();
    int64_t wrapped_data_key_buffer_size = (int64_t)encryption->CalculateCipherSize(DATA_KEY_SIZE);
    ...
    PARQUET_THROW_NOT_OK(arrow::AllocateResizableBuffer(pool, wrapped_data_key_buffer_size,
							&wrapped_data_key_buffer));
    
    wrapped_data_key_buffer_size = parquet_encryption::Encrypt(ParquetCipher::AES_GCM_V1, false,
    ...
    std::string wrapped_data_key(reinterpret_cast<char const*>(wrapped_data_key_buffer->mutable_data()),
				 wrapped_data_key_buffer_size) ;
    
    encoded_wrapped_data_key = boost::beast::detail::base64_encode(wrapped_data_key);
  } else { 
    std::string encoded_data_key = boost::beast::detail::base64_encode(data_key, DATA_KEY_SIZE);
    try {
  ...
  std::string wrapped_key_material = encoded_wrapped_data_key + ":" + master_key_id;
  arrow::util::InitializeUTF8();
  ...
  std::string data_key_str(reinterpret_cast<char const*>(data_key), DATA_KEY_SIZE) ;
  std::unique_ptr<ParquetKey> key(new ParquetKey(data_key_str, wrapped_key_material));
  return key;
}
