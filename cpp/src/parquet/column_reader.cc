 ...
 public:
  ...
  void FillOutData(size_t row_group_idx, int64_t records_read) override {
    for (size_t i = 0; i < destinations[row_group_idx].size(); ++i) {
      ...
      auto dictionary =
            reinterpret_cast<T*>(dictionary_ptrs[row_group_idx][i]->mutable_data());
      ...
      if (status != QPL_STS_OK) {
        throw ParquetException("An error acquired during qpl job wait finished." +
                                std::to_string(status));
      }
      status = qpl_fini_job(job);
      if (status != QPL_STS_OK) {
        throw ParquetException("An error acquired during finish qpl job." +
                                std::to_string(status));
      }
  ...
  virtual void ReadValuesDense(int64_t values_to_read) {
    ...
    T* out_ptr = nullptr;
    int64_t num_decoded =
        this->current_decoder_->DecodeWithIAA(ValuesHead<T>(),
        static_cast<int>(values_to_read), &qpl_job_id, &job, &destination, &out_ptr);
