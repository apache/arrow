...
ColumnEncryptionProperties::Builder* ColumnEncryptionProperties::Builder::withKeyID(
    std::string key_id) {
...
const std::string& FileDecryptionProperties::getColumnKey(
    const std::shared_ptr<schema::ColumnPath>& column_path) {
...
FileEncryptionProperties::Builder* FileEncryptionProperties::Builder::withFooterKeyID(
    std::string key_id) {
  //key_id is expected to be in UTF8 encoding
  ::arrow::util::InitializeUTF8();
  const uint8_t* data = reinterpret_cast<const uint8_t*>(key_id.c_str());
...
std::shared_ptr<ColumnEncryptionProperties> FileEncryptionProperties::getColumnProperties(
    const std::shared_ptr<schema::ColumnPath>& column_path) {
