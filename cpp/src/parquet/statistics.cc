...
template <>
void TypedStatisticsImpl<FLBAType>::PlainDecode(const std::string& src, T* dst) const {
  if (src.size() != static_cast<size_t>(descr_->type_length())) {
    throw ParquetException("Size of encoded FLBA does not match descriptor");
  }
