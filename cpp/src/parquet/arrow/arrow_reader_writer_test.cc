...
TEST(TestArrowWriteNestedDictionary, TestNullCount) {
  std::shared_ptr<::arrow::Array> dictionary = ArrayFromJSON(::arrow::utf8(), R"(["z"])");
  ...
  ASSERT_OK_AND_ASSIGN(
      std::shared_ptr<Array> values,
      ::arrow::ListArray::FromArrays(*offsets, *dict_encoded, default_memory_pool()));
  std::shared_ptr<::arrow::Schema> schema =
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        StatisticsTestParam{/*table=*/Table::Make(
                                ...
                                {ArrayFromJSON(::arrow::utf8(), R"(["1", null, "3"])")}),
                            /*expected=*/1, /* empty list counts as null as well */
