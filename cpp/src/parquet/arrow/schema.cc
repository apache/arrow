...
Status FieldToNode(const std::string& name, const std::shared_ptr<Field>& field,
  ...
  switch (field->type()->id()) {
    ...
    case ArrowTypeId::TIME32:
      type = ParquetType::INT32;
      logical_type =
          LogicalType::Time(/*is_adjusted_to_utc=*/arrow_properties.write_time_adjusted_to_utc(), LogicalType::TimeUnit::MILLIS);
      break;
    case ArrowTypeId::TIME64: {
      ...
      if (time_type->unit() == ::arrow::TimeUnit::NANO) {
        logical_type =
            LogicalType::Time(/*is_adjusted_to_utc=*/arrow_properties.write_time_adjusted_to_utc(), LogicalType::TimeUnit::NANOS);
      } else {
        logical_type =
            LogicalType::Time(/*is_adjusted_to_utc=*/arrow_properties.write_time_adjusted_to_utc(), LogicalType::TimeUnit::MICROS);
      }
    } break;
