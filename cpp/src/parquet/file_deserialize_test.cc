...
namespace parquet {
  ...
  TEST_F(EncodingPropertiesSerdeTest, CapturesDictionaryPageEncodingProperties) {
    ...
    ctx.data_decryptor_factory = [sink, descr]() {
      ...
      return std::make_unique<Decryptor>(std::move(iface), /*key*/ ::arrow::util::SecureString(),
  ...
  TEST(PlaintextFooter_EncryptionAlgorithmsSetCorrectly, ExternalAndAES) {
    ...
    ::arrow::util::SecureString footer_key(std::string("1234567890123456"));  // 16 bytes for tests
