...
GDV_FN_CAST_VARCHAR_INTEGER(date64, Int64Type)
GDV_FN_CAST_VARCHAR_INTEGER(day_time_interval, Int64Type)
...
void ExportedStubFunctions::AddMappings(Engine* engine) const {
  ...
  // gdv_fn_castVARCHAR_day_time_interval_int64
  args = {types->i64_type(),       // int64_t execution_context
          types->i64_type(),       // int64_t value
          types->i64_type(),       // int64_t len
          types->i32_ptr_type()};  // int32_t* out_len
  engine->AddGlobalMappingForFunc(
      "gdv_fn_castVARCHAR_day_time_interval_int64", types->i8_ptr_type() /*return_type*/,
      args, reinterpret_cast<void*>(gdv_fn_castVARCHAR_day_time_interval_int64));
