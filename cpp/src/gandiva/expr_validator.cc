...
Status ExprValidator::Visit(const InExpressionNode<gandiva::BasicDecimalScalar128>& node) {
  int32_t scale = 0;
  int32_t precision=0;

  if (node.values().size() > 0){
    scale = node.values().begin()->scale();
    precision = node.values().begin()->precision();
  }
