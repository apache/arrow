...
#define CAST_FROM_STRING(OUT_TYPE, ARROW_TYPE, TYPE_NAME)  \
  FORCE_INLINE \
  gdv_##OUT_TYPE cast##TYPE_NAME##_utf8(int64_t context, const char * data, int32_t len) { \
    ...
    if (!converter_(data, len, &val)) { \
      gdv_fn_context_set_error_msg(context, "Failed parsing the string to required format"); \
    } \
    return val; \
  }

CAST_FROM_STRING(int32, arrow::Int32Type, INT)
CAST_FROM_STRING(int64, arrow::Int64Type, BIGINT)
CAST_FROM_STRING(float32, arrow::FloatType, FLOAT4)
CAST_FROM_STRING(float64, arrow::DoubleType, FLOAT8)
