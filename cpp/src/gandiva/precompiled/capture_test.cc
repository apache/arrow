...
TEST(TestCapture, capture) {
  ...
  gdv_int32 out_len;
  const char* ret = capture_utf8_utf8(
      ctx_ptr, source_string.data(), static_cast<int>(source_string.length()), true,
      pattern_string.data(), static_cast<int>(pattern_string.length()), true, &out_len);
