...
FORCE_INLINE
gdv_date64 convertTimezone(gdv_int64 context, const char* source, gdv_int32 source_length,
  ...
  // Parses the timestamp to time fields
  int time_fields[9] = {0, 0, 0, 0, 0, 0, 0, 0, 0};
  time_fields[TimeFields::kYear] = extractYear_timestamp(timestamp_in_millis);
  time_fields[TimeFields::kMonth] = extractMonth_timestamp(timestamp_in_millis);
  time_fields[TimeFields::kDay] = extractDay_timestamp(timestamp_in_millis);
  time_fields[TimeFields::kHours] = extractHour_timestamp(timestamp_in_millis);
  time_fields[TimeFields::kMinutes] = extractMinute_timestamp(timestamp_in_millis);
  ...
  if (source_length == 6) {
    ...
    // Updates time_fields with the UTC values
    time_fields[TimeFields::kYear] = extractYear_timestamp(ret_time);
    time_fields[TimeFields::kMonth] = extractMonth_timestamp(ret_time);
    time_fields[TimeFields::kDay] = extractDay_timestamp(ret_time);
    time_fields[TimeFields::kHours] = extractHour_timestamp(ret_time);
    time_fields[TimeFields::kMinutes] = extractMinute_timestamp(ret_time);
    time_fields[TimeFields::kSeconds] = extractSecond_timestamp(ret_time);
  }
  ...
  int destination_values[2] = {0, 0};
  gdv_boolean destination_add_displacement = false;
  ...
  year_month_day date = year(time_fields[TimeFields::kYear]) /
                        month(time_fields[TimeFields::kMonth]) /
                        day(time_fields[TimeFields::kDay]);

  auto date_time = sys_days(date) + hours(time_fields[TimeFields::kHours]) +
