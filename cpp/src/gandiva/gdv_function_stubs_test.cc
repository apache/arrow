...
TEST(TestGdvFnStubs, TestCastFloat8) {
  ...
  ctx.Reset();
    ...
    TEST(TestGdvFnStubs, TestCastVARCHARFromInt32) {
        ...
        out_str = gdv_fn_to_char_int32_int64(ctx_ptr, 34567, 3, "%1", &out_len);
        EXPECT_EQ(std::string(out_str, out_len), "345");
        EXPECT_FALSE(ctx.has_error());

        out_str = gdv_fn_to_char_int32_int64(ctx_ptr, 347, 0, "%1", &out_len);
    ...
    TEST(TestGdvFnStubs, TestCastVARCHARFromInt64) {
        ...
        out_str =
                gdv_fn_to_char_int64_int64(ctx_ptr, -9223372036854775807LL - 1, 100, "%1", &out_len);
        EXPECT_EQ(std::string(out_str, out_len), "-9223372036854775808");
        EXPECT_FALSE(ctx.has_error());

        out_str = gdv_fn_to_char_int64_int64(ctx_ptr, 0, 100, "%1", &out_len);
        EXPECT_EQ(std::string(out_str, out_len), "0");
        EXPECT_FALSE(ctx.has_error());

        // test with required length less than actual buffer length
        out_str = gdv_fn_to_char_int64_int64(ctx_ptr, 12345, 3, "%1", &out_len);
    ...
    TEST(TestGdvFnStubs, TestCastVARCHARFromFloat) {
        ...
        const char* out_str = gdv_fn_to_char_float32_int64(ctx_ptr, 4.567f, 100, "%1", &out_len);
        EXPECT_EQ(std::string(out_str, out_len), "4.567");
        EXPECT_FALSE(ctx.has_error());

        out_str = gdv_fn_to_char_float32_int64(ctx_ptr, -3.4567f, 100, "%1", &out_len);
        EXPECT_EQ(std::string(out_str, out_len), "-3.4567");
        EXPECT_FALSE(ctx.has_error());

        out_str = gdv_fn_to_char_float32_int64(ctx_ptr, 0.00001f, 100, "%1", &out_len);
        EXPECT_EQ(std::string(out_str, out_len), "0.00001");
        EXPECT_FALSE(ctx.has_error());

        out_str = gdv_fn_to_char_float32_int64(ctx_ptr, 0.0f, 100, "%1", &out_len);
        EXPECT_EQ(std::string(out_str, out_len), "0");
        EXPECT_FALSE(ctx.has_error());

        // test with required length less than actual buffer length
        out_str = gdv_fn_to_char_float32_int64(ctx_ptr, 1.2345f, 3, "%1", &out_len);
    ...
    TEST(TestGdvFnStubs, TestCastVARCHARFromDouble) {
        ...
        const char* out_str = gdv_fn_to_char_float64_int64(ctx_ptr, 4.567, 100, "%1", &out_len);
        EXPECT_EQ(std::string(out_str, out_len), "4.567");
        EXPECT_FALSE(ctx.has_error());

        out_str = gdv_fn_to_char_float64_int64(ctx_ptr, -3.4567, 100, "%1", &out_len);
        EXPECT_EQ(std::string(out_str, out_len), "-3.4567");
        EXPECT_FALSE(ctx.has_error());

        out_str = gdv_fn_to_char_float64_int64(ctx_ptr, 0.00001, 100, "%1", &out_len);
        EXPECT_EQ(std::string(out_str, out_len), "0.00001");
        EXPECT_FALSE(ctx.has_error());

        out_str = gdv_fn_to_char_float64_int64(ctx_ptr, 0.0, 100, "%1", &out_len);
        EXPECT_EQ(std::string(out_str, out_len), "0");
        EXPECT_FALSE(ctx.has_error());

        // test with required length less than actual buffer length
        out_str = gdv_fn_to_char_float64_int64(ctx_ptr, 1.2345, 3, "%1", &out_len);
        EXPECT_EQ(std::string(out_str, out_len), "1.2");
        EXPECT_FALSE(ctx.has_error());
    }
