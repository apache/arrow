...
// static void TimedTestAdd3BatchVersion(benchmark::State& state) {
...
//     batches[i] = rag.BatchOf(schema->fields(), num_batches, num_batches);
//   }
//
//   ProjectEvaluator evaluator(projector);
//
//   Status status = TimedEvaluateRecordBatch<arrow::Int64Type, int64_t>(
//       schema, evaluator, batches, pool_, num_records, num_batches, state);
//   ASSERT_OK(status);
// }

static void TimedTestAdd3ArrayVersion(benchmark::State& state) {
  // schema for input fields
  auto field0 = field("f0", int64());
  auto field1 = field("f1", int64());
  auto field2 = field("f2", int64());
...
//   for (int i = 0; i < NUM_BATCHES; i++) {
//     for (int col = 0; col < num_fields; col++) {
//       arrays[col * NUM_BATCHES + i] = std::make_shared<ArrayPtr>(rag.Int64(num_batches,
//       0, 250, 0));
//     }
//   }
//   ProjectEvaluator evaluator(projector);
...
//   auto array = rag.Int64(array_size, int64_t_min, int64_t_max, 0);
//   ProjectEvaluator evaluator(projector);
//
//   Status status = TimedEvaluate<arrow::Date64Type, int64_t>(
//       schema, evaluator, array, pool_, num_records, num_batches, state);
//   ASSERT_TRUE(status.ok());
// }
//
...
//    auto array = rag.String(array_size, 0, 32, 0);
//    FilterEvaluator evaluator(filter);
//
...
//   auto array = rag.String(array_size, 1, 32, 0);
//   ProjectEvaluator evaluator(projector);
//
//   Status status = TimedEvaluate<arrow::StringType, std::string>(
//       schema, evaluator, array, pool, num_records, num_batches, state);
//   ASSERT_TRUE(status.ok());
// }
//
...
//
//   Status status = TimedEvaluate<arrow::StringType, std::string>(
//       schema, evaluator, data_generator, pool, num_records, num_batches, state);
//   ASSERT_TRUE(status.ok());
// }
//
...
//  auto array = rag.String(array_size, 0, 64, 0);
//  ProjectEvaluator evaluator(projector);
//
//  Status status = TimedEvaluate<arrow::StringType, std::string>(
//      schema, evaluator, array, pool_, num_records, num_batches, state);
...
//      schema, evaluator, data_generator, pool_, num_records, num_batches, state)));
//}
...
//    auto condition = TreeExprBuilder::MakeFunction("equal", {node_a, literal},
//    boolean()); boolean_functions.push_back(condition);
//  }
//
...
//  auto array = rag.Decimal128(decimal_type, array_size, 0);
//  ProjectEvaluator evaluator(projector);
//
//  status = TimedEvaluate<arrow::Decimal128Type, arrow::Decimal128>(
//      schema, evaluator, array, arrow::default_memory_pool(), 1 * MILLION, 16 *
//      THOUSAND, state);
//  ASSERT_OK(status);
//}
//
...
// BENCHMARK(TimedTestAdd3BatchVersion)->MinTime(1.0)->Unit(benchmark::kMicrosecond);
BENCHMARK(TimedTestAdd3ArrayVersion)->MinTime(1.0)->Unit(benchmark::kMicrosecond);
