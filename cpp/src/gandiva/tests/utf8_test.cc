...
//  auto expr0 = TreeExprBuilder::MakeExpression(literal0, res);
  ...
  auto field_b = field("b", arrow::utf8());
  auto field_c = field("c", arrow::utf8());
  auto schema = arrow::schema({field_a, field_b, field_c});

  // output fields
  auto res = field("replace", arrow::utf8());

  // Build expression
  auto replace_expr = TreeExprBuilder::MakeExpression("regexp_replace", {field_a, field_b, field_c}, res);

  std::shared_ptr<Projector> projector;
  auto status = Projector::Make(schema, {replace_expr}, TestConfiguration(), &projector);
  EXPECT_TRUE(status.ok()) << status.message();

  // Create a row-batch with some sample data
  int num_records = 2;
  auto array0 = MakeArrowArrayUtf8({"123456789", "asdf1asdf"}, {true, true});
  auto array1 = MakeArrowArrayUtf8({"3", "1"}, {true, true});
  auto array2 = MakeArrowArrayUtf8({"9", "0"}, {true, true});

  // expected output
  auto exp_replace = MakeArrowArrayUtf8({"129456789", "asdf0asdf"}, {true, true});
  ...
  // Validate results
  EXPECT_ARROW_ARRAY_EQUALS(exp_replace, outputs.at(0));
}
