...
TEST_F(TestProjector, TestTime) {
  ...
  auto time2hour_expr =
      TreeExprBuilder::MakeExpression("extractHour", {field1}, field_hour);
  ...
  std::vector<int64_t> field0_data = {MillisSince(epoch, 2000, 1, 1, 5, 0, 0),
  ...
  std::vector<int64_t> field1_data = {
      MillisSince(epoch, 2000, 1, 1, 5, 0, 0), MillisSince(epoch, 1999, 12, 31, 4, 0, 0),
      MillisSince(epoch, 2015, 6, 30, 20, 0, 0), MillisSince(epoch, 2015, 7, 3, 3, 0, 0)};

  auto array1 = MakeArrowTypeArray<arrow::Time32Type, int64_t>(
      time32(arrow::TimeUnit::MILLI), field1_data, validity);

  std::vector<int64_t> field2_data = {
      MillisSince(epoch, 1999, 12, 31, 5, 0, 0), MillisSince(epoch, 2000, 1, 2, 5, 0, 0),
      MillisSince(epoch, 2015, 7, 1, 1, 0, 0), MillisSince(epoch, 2015, 6, 29, 23, 0, 0)};
