...
Physically, a struct type has one child array for each field. Although each child array must be a byte-aligned contiguous memory buffer, there is no requirement that arrays be aligned with each other.
